...
Profiles are `mcp`, `mcp-component`, `codec-json`, `jsonrpc`, `interop` (the
non-TUI runtime and interoperability contracts), `agent` (provider-neutral
conversations), `agent-ui`, `desktop` (Desk with all five applets),
`desktop-agent`, `pad`, `fexplorer`, `daybook`, and `grid`.
