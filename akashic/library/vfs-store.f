...
\ Adversarial fixture seam: retain the validated Library generation but
...
\ Adversarial fixture seam: install a checksummed but stale/wrong hint so the
\ direct verifier, rather than the certificate CRC, must reject the frame.
...
: _LIBMA-CLASSIFY-HEAD  ( -- status )
    _LIBMA-FILE-U @ VFSNAP-HEADER-SIZE < IF _LIBMA-CORRUPT EXIT THEN
    _LIBVP-SECTOR VFSNAP-HEADER-SIZE 0 _LIBMA-READ-AT
        DUP IF EXIT THEN DROP
    _LIBVP-SECTOR _VFSN-H-MAGIC + 8
        _LIBVFS-HEAD-RECORD-MAGIC 8 COMPARE IF
        _LIBMA-CORRUPT EXIT
    THEN
    _LIBVP-SECTOR _VFSNAP-HEADER-CRC
        _LIBVP-SECTOR _VFSN-H-HEADER-CRC + @ <> IF
        _LIBMA-CORRUPT EXIT
    THEN
    _LIBVP-SECTOR _VFSN-H-FORMAT + @ DUP
        _LIBMA-COMPONENT @ LIBEO.ENVELOPE-FORMAT !
    DUP _LIBVFS-HEAD-ENVELOPE-FORMAT-V1 <> IF
        DUP _LIBVFS-HEAD-ENVELOPE-FORMAT-V1 > IF
            DROP _LIBMA-FUTURE
        ELSE
            DROP _LIBMA-CORRUPT
        ...
        EXIT
    THEN DROP
    _LIBVP-SECTOR _VFSN-H-HEADER-SIZE + @ VFSNAP-HEADER-SIZE <> IF
        _LIBMA-CORRUPT EXIT
    THEN
    _LIBVP-SECTOR _VFSN-H-FLAGS + @ IF _LIBMA-CORRUPT EXIT THEN
    _LIBVP-SECTOR _VFSN-H-GENERATION + @ DUP 0> 0= IF
        DROP _LIBMA-CORRUPT EXIT
    THEN
    _LIBMA-COMPONENT @ LIBEO.GENERATION !
    _LIBVP-SECTOR _VFSN-H-PAYLOAD-SIZE + @
        LIB-HEAD-PAYLOAD-SIZE <> IF _LIBMA-CORRUPT EXIT THEN
    _LIBMA-FILE-U @ VFSNAP-HEADER-SIZE LIB-HEAD-PAYLOAD-SIZE + <> IF
        _LIBMA-CORRUPT EXIT
    THEN
    _LIBVP-FRAME LIB-HEAD-PAYLOAD-SIZE VFSNAP-HEADER-SIZE
        _LIBMA-READ-AT DUP IF EXIT THEN DROP
    _LIBVP-FRAME LIB-HEAD-PAYLOAD-SIZE _VFSNAP-PAYLOAD-CRC
        _LIBVP-SECTOR _VFSN-H-PAYLOAD-CRC + @ <> IF
        _LIBMA-CORRUPT EXIT
    THEN
    _LIBVP-FRAME LIB-HEAD-PAYLOAD-SIZE
        _LIBMA-COMPONENT @ LIBEO.GENERATION @
        LIB-HEAD-PAYLOAD-VALIDATE
    DUP LIB-S-UNSUPPORTED = IF
        DROP _LIBVP-FRAME _LIBSH-FORMAT + @
...
: _LIBMA-LOAD-CANDIDATE-HEAD  ( object-id -- status )
    ...
    _LIBMA-FILE-U @ VFSNAP-HEADER-SIZE LIB-HEAD-PAYLOAD-SIZE + <> IF
        LIBSTORE-S-CORRUPT EXIT
    THEN
    _LIBVP-FRAME VFSNAP-HEADER-SIZE LIB-HEAD-PAYLOAD-SIZE + 0
        _LIBMA-READ-AT DUP IF EXIT THEN DROP
    _LIBVP-CLOSE-NOW DUP IF EXIT THEN DROP
    _LIBVP-HEAD-FACT LIB-HEAD-FACT-INIT
    _LIBVP-FRAME VFSNAP-HEADER-SIZE + LIB-HEAD-PAYLOAD-SIZE
    _LIBVP-FRAME _VFSN-H-GENERATION + @ _LIBVP-HEAD-FACT
