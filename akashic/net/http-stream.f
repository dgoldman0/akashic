...
: HSTR-PUMP  ( parser port buffer capacity -- pump-status )
    ...
    _HSPM-PORT @ NIO-POLL
    _HSPM-BUFFER @ _HSPM-CAPACITY @ _HSPM-PORT @ NIO-RECV
    _HSPM-IO ! _HSPM-N !
    _HSPM-IO @ NIO-S-CANCELLED = IF
        _HSPM-PARSER @ HSTR-CANCEL HSTR-PUMP-CANCELLED EXIT
    THEN
    _HSPM-IO @ NIO-S-EOF = IF
        ...
        EXIT
    THEN
    _HSPM-IO @ NIO-S-OK <> IF HSTR-PUMP-TRANSPORT-ERROR EXIT THEN
