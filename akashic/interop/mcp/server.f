...
VARIABLE _MSCU-M
VARIABLE _MSCU-COUNT

: _MS-CURSOR  ( message count -- start ior )
    _MSCU-COUNT ! _MSCU-M !
    _MSCU-M @ S" cursor" _MS-PARAM-FIELD
    DUP IF >R DROP 2DROP R> DROP 0 JRPC-E-INVALID-PARAMS EXIT THEN DROP
    0= IF 2DROP 0 0 EXIT THEN
    _MS-TEXT-BUF 32 JRPC-DECODE-STRING
    DUP IF 2DROP 0 JRPC-E-INVALID-PARAMS EXIT THEN
    DROP
    _MS-TEXT-BUF SWAP STR>NUM 0= IF
        DROP 0 JRPC-E-INVALID-PARAMS EXIT
    THEN
    DUP 0< OVER _MSCU-COUNT @ > OR IF
        DROP 0 JRPC-E-INVALID-PARAMS
    ELSE
        0
    THEN ;
...
: _MS-HANDLE-TOOLS-LIST  ( -- output-len status )
    ...
    DUP IF 2DROP JRPC-E-INVALID-PARAMS S" Invalid cursor" _MS-ERROR EXIT THEN DROP
