...
: _VFS-DENTRY-RELEASE  ( dentry drop-link? vfs -- )
    ...
    _VDF-D @ D.NAME @ _VDF-V @ _VFS-STR-RELEASE
...
: VFS-RENAME-AT  ( new-a new-u inode new-parent flags vfs -- ior )
    ...
    THEN
    _VRN-IN @ IN.NAME @ _VRN-V @ _VFS-STR-RELEASE
...
: VFS-LINK  ( name-a name-u target parent vfs -- dentry ior )
    ...
    _VLN-TARGET @ D.FLAGS @ VFS-DF-UNLINKED AND IF
        0 VFS-E-NOENT EXIT
    THEN
    _VLN-PARENT @ D.FLAGS @ VFS-DF-UNLINKED AND IF
        0 VFS-E-NOENT EXIT
    THEN
...
: VFS-LOOKUP  ( name-a name-u parent vfs -- dentry ior )
    ...
    VFS-OP-LOOKUP _VLK-V @ _VFS-HAS-OP? 0= IF
        _VLK-PARENT @ _VLK-V @ _VFS-ENSURE-CHILDREN?
        ?DUP IF 0 SWAP EXIT THEN
        _VLK-A @ _VLK-U @ _VLK-PARENT @ _VFS-FIND-CHILD
        DUP IF 0 EXIT THEN
        DROP 0 VFS-E-NOENT EXIT
    THEN
