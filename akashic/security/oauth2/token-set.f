...
\
\  Refresh launch is a generation-bound lease transaction:
\    O2TOK-REFRESH-BEGIN
...
2 CONSTANT _O2TOK-MODE-UPDATE
3 CONSTANT _O2TOK-MODE-COMMIT
...
: _O2TOK-STATE-VALID?  ( tokens -- flag )
    ...
    DUP O2TOK.REFRESH-U @ O2TOK-REFRESH-CAPACITY
        _O2TOK-BOUNDED-LENGTH? 0= IF DROP 0 EXIT THEN
    ...
    DUP _O2TOK.LEASE-ID @ 0= IF
        ...
        SWAP _O2TOK.LEASE-USED @ 0= AND EXIT
    THEN
...
: O2TOK-PRESENT?  ( tokens -- flag )
    DUP _O2TOK-OBJECT-STATUS ?DUP IF 2DROP 0 EXIT THEN
    DUP _O2TOK-LOCK ?DUP IF 2DROP 0 EXIT THEN
...
: _O2TOK-UPDATE-GEOMETRY
  ( id-a id-u access-a access-u refresh-a refresh-u expires tokens
    ...
    7 PICK 7 PICK O2TOK-ID-CAPACITY 0 4 PICK
        ...
        >R _O2TOK-DROP8 R> EXIT
    THEN
    5 PICK 5 PICK O2TOK-ACCESS-CAPACITY 0 4 PICK
        _O2TOK-SOURCE-STATUS ?DUP IF
        >R _O2TOK-DROP8 R> EXIT
    THEN
    3 PICK 3 PICK O2TOK-REFRESH-CAPACITY 0 4 PICK
...
: _O2TOK-STAGE-REFRESH  ( tokens -- )
    ...
    ELSE
        R@ _O2TOK.OP-MODE @ _O2TOK-MODE-SET <> IF
            R@ O2TOK.REFRESH R@ _O2TOK.STAGE-REFRESH
            R@ O2TOK.REFRESH-U @ MOVE
            R@ O2TOK.REFRESH-U @ R@ _O2TOK.STAGE-REFRESH-U !
        THEN
    THEN
    R> DROP ;
...
: _O2TOK-PUBLISH-STAGE  ( tokens -- )
    ...
    R@ _O2TOK.STAGE-REFRESH R@ O2TOK.REFRESH
    ...
    R@ _O2TOK.STAGE-REFRESH-U @ R@ O2TOK.REFRESH-U !
...
: _O2TOK-UPDATE-BODY
  ( id-a id-u access-a access-u refresh-a refresh-u expires tokens
    ...
    DUP _O2TOK-PRESENT-LOCKED? 0= IF
        _O2TOK-DROP8 O2TOK-S-ABSENT EXIT
    THEN
    _O2TOK-LOAD-OP8
    _O2TOK-MODE-UPDATE OVER _O2TOK.OP-MODE !
    DUP _O2TOK-STAGE-ALL
    DUP _O2TOK.STAGE-ACCESS-U @ 0= IF
        O2TOK-S-ABSENT _O2TOK-MUTATE-FAIL EXIT
    THEN
...
: O2TOK-UPDATE
  ( id-a id-u access-a access-u refresh-a refresh-u expires-ms tokens
    -- status )
    _O2TOK-8DUP _O2TOK-UPDATE-GEOMETRY ?DUP IF
        >R _O2TOK-DROP8 R> EXIT
    THEN
    DUP _O2TOK-LOCK ?DUP IF
        >R _O2TOK-DROP8 R> EXIT
    THEN
    ['] _O2TOK-UPDATE-BODY _O2TOK-CALL8 ;
...
: O2TOK-WITH-REFRESH
  ( callback callback-context tokens -- status )
    ...
    DUP _O2TOK.LEASE-ID @ IF
        DUP _O2TOK-UNLOCK
        _O2TOK-DROP3 O2TOK-S-BUSY EXIT
    THEN
    DUP O2TOK.REFRESH
    OVER O2TOK.REFRESH-U @
    _O2TOK-WITH-AT ;
...
: O2TOK-REFRESH-BEGIN  ( tokens -- lease status )
    DUP _O2TOK-OBJECT-STATUS ?DUP IF
        NIP 0 SWAP EXIT
    THEN
    DUP _O2TOK-LOCK ?DUP IF
    ...
    DUP O2TOK.BORROWED @ IF
        DUP _O2TOK-UNLOCK
        DROP 0 O2TOK-S-BUSY EXIT
    THEN
    DUP _O2TOK.LEASE-ID @ IF
        DUP _O2TOK-UNLOCK
        DROP 0 O2TOK-S-BUSY EXIT
    THEN
    DUP O2TOK.REFRESH-U @ 0= IF
        DUP _O2TOK-UNLOCK
        DROP 0 O2TOK-S-ABSENT EXIT
    THEN
...
: O2TOK-WITH-REFRESH-LEASE
  ( callback callback-context lease tokens -- status )
    ...
    1 PICK 0= IF _O2TOK-DROP4 O2TOK-S-INVALID EXIT THEN
    DUP _O2TOK-OBJECT-STATUS ?DUP IF
...
: O2TOK-REFRESH-ABORT  ( lease tokens -- status )
    OVER 0= IF 2DROP O2TOK-S-INVALID EXIT THEN
    DUP _O2TOK-OBJECT-STATUS ?DUP IF
        >R 2DROP R> EXIT
    THEN
    DUP _O2TOK-LOCK ?DUP IF
