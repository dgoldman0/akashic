...
\    DESK-SANDBOX-CONFIGURE ( owner|0 capacity -- status )
\                                      Borrow modules and select slots before run
...
VARIABLE _DESK-PENDING-SBOX-OWNER
...
\ Desk owns one modest immutable pure-compute policy.  These are independent
\ of Agent model/token policy and remain below the reusable pure profile.
100000 CONSTANT _DESK-SBOX-INSTRUCTION-BUDGET
...
_DESK-CURRENT-STATE CMP-CELL: _DESK-SBOX-OWNER
...
: _DESK-SBOX-LIVE?  ( -- flag )
    _DESK-SANDBOX @ ?DUP IF
        DESK-SBOX-JOB-SERVICE-STATE@
        DUP DESK-SBOX-JOB-SERVICE-STATE-OPEN =
        SWAP DESK-SBOX-JOB-SERVICE-STATE-CLOSING = OR
    ELSE
        0
    THEN ;
...
: _DHR-REMEMBER  ( status -- )
    ?DUP IF
        _DHR-FIRST @ 0= IF _DHR-FIRST ! ELSE DROP THEN
    THEN ;
...
VARIABLE _DSBI-OWNER
VARIABLE _DSBI-CAPACITY
VARIABLE _DSBI-SERVICE
VARIABLE _DSBI-SERVICE-U
VARIABLE _DSBI-STATUS

: _DESK-SBOX-LIMITS-INIT  ( -- status )
    _DESK-SBOX-LIMITS SBOX-VALUE-LIMITS-SIZE 0 FILL
    _DESK-SBOX-LIMITS SBOX-VALUE-LIMITS-BEGIN
        DUP IF EXIT THEN DROP
    8 SBOX-VALUE-LIMIT-DEPTH _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    65536 SBOX-VALUE-LIMIT-BLOB-BYTES _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    64 SBOX-VALUE-LIMIT-LIST-COUNT _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    32 SBOX-VALUE-LIMIT-MAP-COUNT _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    256 SBOX-VALUE-LIMIT-INPUT-NODES _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    65536 SBOX-VALUE-LIMIT-INPUT-BYTES _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    256 SBOX-VALUE-LIMIT-OUTPUT-ARENA-NODES _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    65536 SBOX-VALUE-LIMIT-OUTPUT-ARENA-BYTES _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    256 SBOX-VALUE-LIMIT-OUTPUT-RESULT-NODES _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    131072 SBOX-VALUE-LIMIT-OUTPUT-RESULT-BYTES _DESK-SBOX-LIMITS
        SBOX-VALUE-LIMIT! DUP IF EXIT THEN DROP
    _DESK-SBOX-LIMITS SBOX-VALUE-LIMITS-SEAL ;
...
: _DESK-SBOX-INIT  ( -- status )
    _DESK-SBOX-OWNER @ DUP _DSBI-OWNER !
    _DESK-SBOX-CAPACITY @ _DSBI-CAPACITY !
    0 _DESK-SANDBOX !
    0 _DESK-SANDBOX-U !
    0= IF
        _DSBI-CAPACITY @ IF
            _DESK-SBOX-STAGING-CLEAR DESK-SBOX-S-INVALID EXIT
        THEN
        _DESK-SBOX-STAGING-CLEAR DESK-SBOX-S-OK EXIT
    THEN
    _DSBI-CAPACITY @ DESK-SBOX-JOB-SERVICE-MEASURE
    _DSBI-STATUS ! _DSBI-SERVICE-U !
    _DSBI-STATUS @ IF
        _DESK-SBOX-STAGING-CLEAR _DSBI-STATUS @ EXIT
    THEN
    _DSBI-OWNER @ SBOX-MODULE-OWNER-SEALED? 0= IF
        _DESK-SBOX-STAGING-CLEAR DESK-SBOX-S-OWNER EXIT
    THEN
    _DESK-SBOX-LIMITS-INIT DUP IF
        DROP
        _DESK-SBOX-STAGING-CLEAR DESK-SBOX-S-INVALID EXIT
    THEN
    DROP
    ...
    _DSBI-STATUS @ IF
        _DESK-SBOX-STAGING-CLEAR DESK-SBOX-S-NOMEM EXIT
    THEN
    ...
    _DINI-CONTEXT @
    _DESK-SBOX-LIMITS
    _DESK-SBOX-INSTRUCTION-BUDGET
    _DESK-SBOX-VALUE-OP-BUDGET
    _DESK-SBOX-COPY-BUDGET
    _DESK-SBOX-SLICE-STEPS
    _DINI-INST @ CINST.ID @
    _DESK-SBOX-CAPACITY @
    _DESK-SANDBOX @
    _DESK-SANDBOX-U @
    DESK-SBOX-JOB-SERVICE-INIT _DSBI-STATUS !
    _DSBI-STATUS @ IF
        _DSBI-SERVICE @ FREE
        0 _DESK-SANDBOX !
        0 _DESK-SANDBOX-U !
...
: _DESK-SBOX-FINI  ( -- status )
    ...
    THEN
    _DESK-SANDBOX-U @ DESK-SBOX-JOB-SERVICE-RELEASE
    DUP IF
        DROP
...
: _DESK-INTEROP-INIT  ( desk-instance -- )
    ...
    _DESK-SBOX-INIT DESK-SBOX-S-OK <>
...
: _DESK-INTEROP-FINI-QUIESCED  ( -- )
    _DESK-SBOX-FINI DESK-SBOX-S-OK <>
...
: DESK-TICK-CB  ( instance -- )
    ...
    _DESK-SBOX-READY? IF
        _DESK-SANDBOX @ DESK-SBOX-JOB-SERVICE-TICK DROP
    THEN
...
VARIABLE _DSCFG-OWNER
VARIABLE _DSCFG-CAPACITY

: DESK-SANDBOX-CONFIGURE  ( owner|0 capacity -- status )
    _DSCFG-CAPACITY ! _DSCFG-OWNER !
    _DESK-CURRENT-STATE @ IF
        DESK-SBOX-S-STATE EXIT
    THEN
    _DSCFG-OWNER @ 0= IF
        _DSCFG-CAPACITY @ IF DESK-SBOX-S-INVALID EXIT THEN
        0 _DESK-PENDING-SBOX-CAPACITY !
        0 _DESK-PENDING-SBOX-OWNER !
        DESK-SBOX-S-OK EXIT
    THEN
    _DSCFG-CAPACITY @ DESK-SBOX-JOB-SERVICE-MEASURE
    DUP IF NIP EXIT THEN
    2DROP
    _DSCFG-OWNER @ SBOX-MODULE-OWNER-SEALED? 0= IF
        DESK-SBOX-S-OWNER EXIT
    THEN
    _DSCFG-CAPACITY @ _DESK-PENDING-SBOX-CAPACITY !
    _DSCFG-OWNER @ _DESK-PENDING-SBOX-OWNER !
    DESK-SBOX-S-OK ;
...
' DESK-SANDBOX-CONFIGURE CONSTANT _desk-sandbox-configure-xt
...
: DESK-SANDBOX-CONFIGURE
    _desk-sandbox-configure-xt _desk-guard WITH-GUARD ;
