...
\ Layout + Regions
VARIABLE _FEXP-LAYOUT     \ layout engine handle
...
: FEXP-INIT-CB  ( -- )
    ...
    \ Get VFS (shell must have initialised it via _ASHELL-VFS-INIT)
    VFS-CUR DUP 0= ABORT" fexplorer: no VFS available"
    _FEXP-VFS !

    \ Build layout: menu(1 row) | body(expand) | status(1 row)
    ASHELL-REGION LAY-VERTICAL 0 LAY-NEW  _FEXP-LAYOUT !
    _FEXP-LAYOUT @  1 1  LAY-ADD  _FEXP-MENU-RGN !    \ fixed 1 row
    _FEXP-LAYOUT @  0 1  LAY-ADD  _FEXP-BODY-RGN !    \ auto-expand
    _FEXP-LAYOUT @  1 1  LAY-ADD  _FEXP-STAT-RGN !    \ fixed 1 row
    LAY-F-EXPAND _FEXP-LAYOUT @ LAY-FLAGS!
    _FEXP-LAYOUT @ LAY-COMPUTE
...
: FEXP-SHUTDOWN-CB  ( -- )
    ...
    0 _FEXP-GOTO-W !
    \ Free layout
    _FEXP-LAYOUT @ ?DUP IF LAY-FREE THEN
    0 _FEXP-LAYOUT !
    \ Free widgets
...
: FEXP-ENTRY  ( desc -- )
    ...
    0                    OVER APP.WIDTH !
    0                    OVER APP.HEIGHT !
    S" File Explorer"    ROT DUP >R APP.TITLE-U !
                         R> APP.TITLE-A !
;
