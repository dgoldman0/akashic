...
\ A Daybook instance is either a standalone VFS client or a session onto the
...
_DB-CURRENT-STATE RSES-SIZE CMP-FIELD: _DB-RESOURCE-SESSION
...
\ the retained discovery, exact binding, and request lifecycle live in RSES.
: _DB-RESOURCE-MODE       ( -- a ) _DB-RESOURCE-SESSION RSES.MODE ;
: _DB-SHARED-REF          ( -- ref ) _DB-RESOURCE-SESSION RSES.REF ;
: _DB-SHARED-BIND         ( -- bind ) _DB-RESOURCE-SESSION RSES.BIND ;
: _DB-SHARED-SNAPSHOT-CAP ( -- a ) _DB-RESOURCE-SESSION RSES.SNAPSHOT ;
: _DB-SHARED-REPLACE-CAP  ( -- a ) _DB-RESOURCE-SESSION RSES.REPLACE ;
: _DB-SHARED-REQUEST      ( -- a ) _DB-RESOURCE-SESSION RSES.REQUEST ;

VARIABLE _DB-SHARED-ADVANCE-XT
' LBIND-ADVANCE _DB-SHARED-ADVANCE-XT !

: _DB-SHARED-ADVANCE  ( request context binding -- status )
    _DB-SHARED-ADVANCE-XT @ EXECUTE ;
...
: _DB-SHARED-BLOCK!  ( -- )
    _DB-RESOURCE-SESSION RSES-HELD? IF
        RSES-M-STALE
    ELSE
        RSES-M-BLOCKED
    THEN _DB-RESOURCE-MODE !
    -1 _DB-SOURCE-BLOCKED ! ;

: _DB-SHARED-REFRESH  ( -- status )
    _DB-RESOURCE-SESSION RSES-REFRESH ;

: _DB-SHARED-INIT  ( -- )
    0 _DB-CAPTURE-HOP !
    S" org.akashic.resource.daybook" _DB-CURRENT-INSTANCE @
        _DB-RESOURCE-SESSION RSES-INIT DROP
    ['] _DB-SHARED-ADVANCE _DB-RESOURCE-SESSION RSES.ADVANCE-XT !
    _DB-RESOURCE-MODE @ DUP RSES-M-BLOCKED =
...
: _DB-SHARED-FINI  ( -- )
    ...
    BEGIN
        _DB-RESOURCE-SESSION RSES-FINI
        DUP RSES-S-RELEASE =
    WHILE
        DROP YIELD?
    REPEAT
...
: _DB-PARSE-DATE  ( addr len -- epoch flag )
    ...
    _DB-P-Y @ _DB-P-M @ DT-MONTH-DAYS
    DUP IF 2DROP 0 0 EXIT THEN DROP
    _DB-P-D @ SWAP > IF 0 0 EXIT THEN
    _DB-P-Y @ _DB-P-M @ _DB-P-D @ DT-YMD>EPOCH-S
    DUP IF 2DROP 0 0 EXIT THEN DROP -1 ;
...
: _DB-SHARED-REQUEST!  ( capability principal -- status )
    ...
    _DB-SRQ-CAP @ _DB-SRQ-PRINCIPAL @ _DB-RESOURCE-SESSION RSES-PREPARE ;
...
: _DB-SHARED-SNAPSHOT  ( -- status )
    ...
    _DB-RESOURCE-SESSION RSES-DISPATCH
...
: _DB-LOAD  ( -- status )
    _DB-RESOURCE-MODE @ CASE
        RSES-M-ACTIVE OF _DB-LOAD-SHARED ENDOF
        RSES-M-STALE OF _DB-LOAD-SHARED ENDOF
        RSES-M-BLOCKED OF
...
: _DB-SERIALIZE-ENTRY  ( entry -- )
    ...
    ENDCASE
    _DB-SER-E @ _DB-E-DATE + @ _DB-DATE-BUF 16 DT-DATE-S DROP
...
: _DB-WRITE-SHARED  ( -- ior )
    ...
    _DB-RESOURCE-SESSION RSES-ADVANCE DUP IF
        ...
        \ local session, preserve the committed model, and require an explicit
        \ reload before another write.
        DROP -1 _DB-SOURCE-BLOCKED ! 0 EXIT
    THEN DROP
    0 ;
...
: _DB-WRITE  ( -- ior )
    0 _DB-SAVE-STALE !
    _DB-RESOURCE-MODE @ CASE
        RSES-M-ACTIVE OF _DB-WRITE-SHARED ENDOF
        RSES-M-STALE OF _DB-W-S-SHARED ENDOF
        RSES-M-BLOCKED OF _DB-W-S-SHARED ENDOF
...
: _DB-UPDATE-STATUS  ( -- )
    _DB-SELECTED-DATE @ _DB-DATE-BUF 16 DT-DATE-S 2DROP
    ...
    _DB-RESOURCE-MODE @ RSES-M-BLOCKED = IF
...
: _DB-SAVE-ERROR-TOAST  ( -- )
    _DB-SAVE-STALE @ IF
        ...
        3000 ASHELL-TOAST
    ELSE _DB-RESOURCE-MODE @ RSES-M-BLOCKED = IF
...
: _DB-DRAW-CALENDAR  ( -- )
    _DB-SELECTED-DATE @ DT-EPOCH-S>YMD DROP _DB-DD ! _DB-DM ! _DB-DY !
    255 23 0 DRW-STYLE!
    ...
    _DB-DY @ _DB-DM @ 1 DT-YMD>EPOCH-S DROP DUP _DB-DMONTH-EPOCH !
    _DB-SECONDS-DAY / 3 + 7 MOD _DB-DWDAY !
    _DB-DY @ _DB-DM @ DT-MONTH-DAYS DROP 1+ 1 DO
...
: _DB-DRAW-AGENDA  ( -- )
    ...
    255 234 1 DRW-STYLE!
    _DB-SELECTED-DATE @ _DB-DATE-BUF 16 DT-DATE-S DROP
...
: _DB-PANEL-INIT  ( rgn -- )
    DUP _DB-PANEL-RGN !
    _DB-PANEL 30 ROT
    ['] _DB-PANEL-DRAW ['] _DB-PANEL-HANDLE WDG-INIT ;
...
: _DB-SOURCE-VALUE!  ( value -- status )
    ...
    _DB-RESOURCE-MODE @ CASE
        ...
        ENDOF
        RSES-M-STALE OF IRES-S-INVALID ENDOF
        RSES-M-BLOCKED OF IRES-S-INVALID ENDOF
...
: DAYBOOK-INIT-CB  ( instance -- )
    ...
    _DB-SHARED-INIT
    _DB-RESOURCE-MODE @ RSES-M-DIRECT = IF
...
: DAYBOOK-SHUTDOWN-CB  ( instance -- )
    _DB-ACTIVATE
    \ The exact pool lease lives inside this component state.  Release it
    \ before any UI teardown that can throw during close or launch rollback.
    _DB-SHARED-FINI
...
CREATE _DB-CAP-BUILDER CAPB-SIZE ALLOT
...
: _DB-CAP-SETUP  ( -- )
    ...
    _DB-CAP-BUILDER CAPB-INIT ?DUP IF THROW THEN

    _DB-CAP-BUILDER CAPB-BEGIN ?DUP IF THROW THEN
    CAP-K-COMMAND S" daybook.task.capture" S" Capture task"
        S" Add a task to Daybook's selected date and persist it"
        _DB-CAP-BUILDER CAPB-META ?DUP IF THROW THEN
    _DB-TEXT-SCHEMA _DB-INT-SCHEMA
        CAP-E-MUTATE CAP-E-PERSIST OR CAP-F-NEEDS-TARGET
        0 CCLASS-OWNER-COMMIT _DB-CAP-BUILDER CAPB-CONTRACT
        ?DUP IF THROW THEN
    ['] _DB-CAP-CAPTURE-HANDLER 0 0 _DB-CAP-BUILDER CAPB-CALLBACKS
        ?DUP IF THROW THEN
    DAYBOOK-CAP-CAPTURE _DB-CAP-BUILDER CAPB-SEAL ?DUP IF THROW THEN

    _DB-CAP-BUILDER CAPB-BEGIN ?DUP IF THROW THEN
    CAP-K-RESOURCE S" daybook.source" S" Daybook source"
        S" Read the durable Daybook source resource"
        _DB-CAP-BUILDER CAPB-META ?DUP IF THROW THEN
    0 _DB-RESOURCE-SCHEMA CAP-E-OBSERVE
        CAP-F-IDEMPOTENT CAP-F-NEEDS-TARGET OR CAP-F-CONTEXT-DEFAULT OR
        0 CCLASS-OWNER-COMMIT _DB-CAP-BUILDER CAPB-CONTRACT
        ?DUP IF THROW THEN
    ['] _DB-CAP-SOURCE-HANDLER 0 0 _DB-CAP-BUILDER CAPB-CALLBACKS
        ?DUP IF THROW THEN
    DAYBOOK-CAP-SOURCE _DB-CAP-BUILDER CAPB-SEAL ?DUP IF THROW THEN

    _DB-CAP-BUILDER CAPB-BEGIN ?DUP IF THROW THEN
    CAP-K-RESOURCE S" daybook.agenda.markdown" S" Daybook agenda"
        S" Read the bounded task, event, and note source"
        _DB-CAP-BUILDER CAPB-META ?DUP IF THROW THEN
    0 _DB-AGENDA-SCHEMA CAP-E-OBSERVE
        CAP-F-IDEMPOTENT CAP-F-NEEDS-TARGET OR
        0 CCLASS-OWNER-COMMIT _DB-CAP-BUILDER CAPB-CONTRACT
        ?DUP IF THROW THEN
    ['] _DB-CAP-AGENDA-HANDLER 0 0 _DB-CAP-BUILDER CAPB-CALLBACKS
        ?DUP IF THROW THEN
    DAYBOOK-CAP-AGENDA _DB-CAP-BUILDER CAPB-SEAL ?DUP IF THROW THEN ;
