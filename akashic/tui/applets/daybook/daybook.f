...
\ Gregorian months carry one title, seven weekday headers, and at most 31
\ dates.  A 32-byte title covers the longest month plus a signed cell year.
32 USCOL-TEXT-ITEM-BYTES CONSTANT _DB-GRID-TITLE-ITEM-CAP
 2 USCOL-TEXT-ITEM-BYTES CONSTANT _DB-GRID-SHORT-ITEM-CAP
USCOL-TEXT-FIXED-SIZE _DB-GRID-TITLE-ITEM-CAP +
    38 _DB-GRID-SHORT-ITEM-CAP * + CONSTANT _DB-GRID-BANK-CAP
39 8 * CONSTANT _DB-GRID-WORK-CAP
...
_DB-CURRENT-STATE _DB-GRID-BANK-CAP CMP-FIELD: _DB-GRID-BANK-A
_DB-CURRENT-STATE _DB-GRID-BANK-CAP CMP-FIELD: _DB-GRID-BANK-B
_DB-CURRENT-STATE USCOL-BUILDER-SIZE CMP-FIELD: _DB-GRID-BUILDER
_DB-CURRENT-STATE _DB-GRID-WORK-CAP CMP-FIELD: _DB-GRID-WORK
_DB-CURRENT-STATE USCOL-SUMMARY-SIZE CMP-FIELD: _DB-GRID-SUMMARY
_DB-CURRENT-STATE _DB-GRID-TITLE-CAP CMP-FIELD: _DB-GRID-TITLE
...
\ =====================================================================
\  Canonical calendar TEXT_GRID model
\ =====================================================================

VARIABLE _DB-G-DEST
VARIABLE _DB-G-U
VARIABLE _DB-G-YEAR
VARIABLE _DB-G-MONTH
VARIABLE _DB-G-DAY
VARIABLE _DB-G-DAYS
VARIABLE _DB-G-FIRST-EPOCH
VARIABLE _DB-G-WEEKDAY
VARIABLE _DB-G-ROW
VARIABLE _DB-G-COLUMN
VARIABLE _DB-G-EPOCH
VARIABLE _DB-G-STATE
VARIABLE _DB-G-TEXT-A
VARIABLE _DB-G-TEXT-U
VARIABLE _DB-G-TITLE-U
VARIABLE _DB-G-YEAR-A
VARIABLE _DB-G-YEAR-U
VARIABLE _DB-G-LAYOUT-ROW
VARIABLE _DB-G-LAYOUT-COLUMN

: _DB-WIDE-LAYOUT?  ( width height -- flag )
    SWAP _DB-WIDE-MIN-WIDTH >=
    SWAP _DB-WIDE-MIN-HEIGHT >= AND ;

: _DB-GRID-DATE-KEY  ( epoch -- key )
    _DB-SECONDS-DAY / _DB-GRID-DATE-KEY-BIAS + ;

: _DB-GRID-WEEKDAY  ( column -- address bytes )
    CASE
        0 OF S" Mo" ENDOF
        1 OF S" Tu" ENDOF
        2 OF S" We" ENDOF
...
: _DB-GRID-TITLE!  ( -- flag )
    _DB-G-MONTH @ _DB-MONTH-NAME _DB-G-TEXT-U ! _DB-G-TEXT-A !
    _DB-G-YEAR @ NUM>STR _DB-G-YEAR-U ! _DB-G-YEAR-A !
    _DB-G-TEXT-U @ 1+ _DB-G-YEAR-U @ +
        DUP _DB-GRID-TITLE-CAP > IF DROP 0 EXIT THEN
    _DB-G-TITLE-U !
    _DB-G-TEXT-A @ _DB-GRID-TITLE _DB-G-TEXT-U @ MOVE
    BL _DB-GRID-TITLE _DB-G-TEXT-U @ + C!
    _DB-G-YEAR-A @ _DB-GRID-TITLE _DB-G-TEXT-U @ 1+ +
        _DB-G-YEAR-U @ MOVE
    -1 ;

: _DB-GRID-MONTH-METRICS?  ( -- flag )
    _DB-SELECTED-DATE @ DT-EPOCH-S>YMD
    DUP IF 2DROP 2DROP 0 EXIT THEN DROP
    _DB-G-DAY ! _DB-G-MONTH ! _DB-G-YEAR !
    _DB-G-YEAR @ _DB-G-MONTH @ DT-MONTH-DAYS
    DUP IF 2DROP 0 EXIT THEN DROP _DB-G-DAYS !
    _DB-G-YEAR @ _DB-G-MONTH @ 1 DT-YMD>EPOCH-S
    DUP IF 2DROP 0 EXIT THEN DROP DUP _DB-G-FIRST-EPOCH !
    _DB-SECONDS-DAY / 3 + 7 MOD _DB-G-WEEKDAY !
    _DB-GRID-TITLE! ;

: _DB-GRID-INACTIVE  ( -- address )
    _DB-GRID-ACTIVE-A @ _DB-GRID-BANK-A = IF
        _DB-GRID-BANK-B
    ELSE
        _DB-GRID-BANK-A
    THEN ;

: _DB-GRID-EMIT-TITLE  ( -- status )
    _DB-GRID-TITLE-KEY 0 0 1 _DB-GRID-COLUMNS
        USCOL-ROLE-ROW-HEADER 0
        _DB-GRID-TITLE _DB-G-TITLE-U @ _DB-GRID-BUILDER
        USCOL-TEXT-ITEM ;

: _DB-GRID-EMIT-WEEKDAYS  ( -- status )
    _DB-GRID-COLUMNS 0 DO
        I _DB-GRID-WEEKDAY-KEY-BASE + 1 I 1 1
            USCOL-ROLE-COLUMN-HEADER 0
            I _DB-GRID-WEEKDAY _DB-GRID-BUILDER
            USCOL-TEXT-ITEM
        DUP USCOL-S-OK <> IF UNLOOP EXIT THEN DROP
    LOOP
    USCOL-S-OK ;

: _DB-GRID-EMIT-DATES  ( -- status )
    _DB-G-DAYS @ 1+ 1 DO
        _DB-G-WEEKDAY @ I 1- + DUP
        7 / 2 + _DB-G-ROW !
        7 MOD _DB-G-COLUMN !
        _DB-G-FIRST-EPOCH @ I 1- _DB-SECONDS-DAY * +
            DUP _DB-G-EPOCH !
        _DB-TODAY-CACHE @ = IF USCOL-ITEM-CURRENT ELSE 0 THEN
            _DB-G-STATE !
        I NUM>STR _DB-G-TEXT-U ! _DB-G-TEXT-A !
        _DB-G-EPOCH @ _DB-GRID-DATE-KEY
            _DB-G-ROW @ _DB-G-COLUMN @ 1 1
            USCOL-ROLE-CONTENT _DB-G-STATE @
            _DB-G-TEXT-A @ _DB-G-TEXT-U @ _DB-GRID-BUILDER
            USCOL-TEXT-ITEM
        DUP USCOL-S-OK <> IF UNLOOP EXIT THEN DROP
    LOOP
    USCOL-S-OK ;

: _DB-GRID-REBUILD  ( -- status )
    _DB-GRID-WIDGET @ 0= IF USCOL-S-UNAVAILABLE EXIT THEN
    _DB-GRID-MONTH-METRICS? 0= IF USCOL-S-INVALID EXIT THEN
    _DB-GRID-INACTIVE DUP _DB-G-DEST !
        _DB-GRID-BANK-CAP _DB-GRID-BUILDER USCOL-BUILDER-INIT
    DUP USCOL-S-OK <> IF EXIT THEN DROP
    USCOL-F-TEXT-GRID _DB-GRID-ROOT-KEY
        0 0 _DB-CALENDAR-HEIGHT _DB-CALENDAR-WIDTH
        USCOL-STATE-VISIBLE USCOL-STATE-ENABLED OR
        _DB-GRID-BUILDER USCOL-TEXT-BEGIN
    DUP USCOL-S-OK <> IF EXIT THEN DROP
    USCOL-CONTENT-READ-ONLY _DB-GRID-ROWS _DB-GRID-COLUMNS
        0 0 _DB-GRID-ROWS _DB-GRID-COLUMNS
        _DB-GRID-BUILDER USCOL-TEXT-SHAPE
    DUP USCOL-S-OK <> IF EXIT THEN DROP
    _DB-SELECTED-DATE @ _DB-GRID-DATE-KEY 0 0 0
        _DB-GRID-BUILDER USCOL-TEXT-POSITIONS
    DUP USCOL-S-OK <> IF EXIT THEN DROP
    _DB-GRID-EMIT-TITLE DUP USCOL-S-OK <> IF EXIT THEN DROP
    _DB-GRID-EMIT-WEEKDAYS DUP USCOL-S-OK <> IF EXIT THEN DROP
    _DB-GRID-EMIT-DATES DUP USCOL-S-OK <> IF EXIT THEN DROP
    _DB-GRID-BUILDER USCOL-TEXT-END
    DUP USCOL-S-OK <> IF EXIT THEN DROP
    _DB-GRID-BUILDER USCOL-BUILDER-FINISH
    DUP USCOL-S-OK <> IF NIP EXIT THEN DROP _DB-G-U !
    _DB-G-DEST @ _DB-G-U @
        _DB-GRID-WORK _DB-GRID-WORK-CAP _DB-GRID-SUMMARY
        _DB-GRID-WIDGET @ TGRID-BIND
    DUP USCOL-S-OK <> IF EXIT THEN DROP
    _DB-G-DEST @ _DB-GRID-ACTIVE-A !
    _DB-G-U @ _DB-GRID-ACTIVE-U !
    USCOL-S-OK ;

: _DB-GRID-REBUILD!  ( -- )
    _DB-GRID-WIDGET @ ?DUP IF
        DROP _DB-GRID-REBUILD USCOL-S-OK <>
            ABORT" daybook: calendar grid rebuild"
    THEN ;

' _DB-GRID-REBUILD! IS _DB-GRID-REBUILD-D

: _DB-GRID-LAYOUT  ( -- )
    _DB-PANEL-RGN @ DUP RGN-ROW _DB-G-LAYOUT-ROW !
    RGN-COL _DB-G-LAYOUT-COLUMN !
    _DB-G-LAYOUT-ROW @ _DB-G-LAYOUT-COLUMN @
        _DB-CALENDAR-HEIGHT _DB-CALENDAR-WIDTH _DB-GRID-RGN @
        RGN-BOUNDS! ;

: _DB-GRID-SELECTED  ( item-key widget -- )
    DROP _DB-GRID-DATE-KEY-BIAS - _DB-SECONDS-DAY *
    _DB-SELECTED-DATE !
...
: _DB-PANEL-DRAW  ( widget -- )
    ...
    _DB-DW @ _DB-DH @ _DB-WIDE-LAYOUT? IF
        _DB-CALENDAR-WIDTH 1+ _DB-AGENDA-COL !
        _DB-DW @ _DB-AGENDA-COL @ - _DB-AGENDA-W !
        _DB-GRID-LAYOUT
        _DB-GRID-WIDGET @ WDG-DRAW
        _DB-PANEL-RGN @ RGN-USE
        239 234 0 DRW-STYLE!
        9474 0 _DB-CALENDAR-WIDTH _DB-DH @ DRW-VLINE
    ELSE
...
: _DB-GRID-HANDLE?  ( event -- consumed? )
    ...
    _DB-PANEL-RGN @ RGN-W _DB-PANEL-RGN @ RGN-H
        _DB-WIDE-LAYOUT? 0= IF DROP 0 EXIT THEN
    DUP @ KEY-T-SPECIAL <> IF DROP 0 EXIT THEN
    DUP 8 + @ DUP KEY-LEFT = SWAP KEY-RIGHT = OR 0= IF DROP 0 EXIT THEN
    _DB-GRID-WIDGET @ WDG-HANDLE ;
