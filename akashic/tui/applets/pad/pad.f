...
_PAD-CURRENT-STATE SDLENS-SIZE CMP-FIELD: _PAD-SHARED-LENS
...
\ SDLENS owns the shared service, exact-binding, and request plumbing.
: _PAD-RESOURCE-MODE       ( -- a ) _PAD-SHARED-LENS SDLENS.MODE ;
: _PAD-SHARED-REF          ( -- ref ) _PAD-SHARED-LENS SDLENS.REF ;
: _PAD-SHARED-BIND         ( -- bind ) _PAD-SHARED-LENS SDLENS.BIND ;
: _PAD-SHARED-REQUEST      ( -- a ) _PAD-SHARED-LENS SDLENS.REQUEST ;
...
: _PAD-SHARED-REQUEST!  ( binding capability -- ior )
    ...
    _PSRQ-BIND @ _PAD-SHARED-LENS SDLENS.CONTEXT @ _PAD-SHARED-REQUEST @
...
: _PAD-SHARED-SNAPSHOT-CANDIDATE  ( -- ior )
    0 _PAD-SHARED-SNAPSHOT-A ! 0 _PAD-SHARED-SNAPSHOT-U !
    _PAD-SHARED-CANDIDATE-BIND _PAD-SHARED-LENS SDLENS.SNAPSHOT-CAP @
        _PAD-SHARED-REQUEST! DUP IF EXIT THEN DROP
    _PAD-SHARED-REQUEST @ CBR.ARGS CV-NULL!
    _PAD-SHARED-REQUEST @ _PAD-SHARED-LENS SDLENS.BUS @ CBUS-DISPATCH
...
: _PAD-SHARED-OPEN-CANDIDATE  ( -- ior )
    _PAD-RESOURCE-MODE @ SDLENS-M-SHARED <> IF
    ...
    _PAD-SHARED-CANDIDATE-REF RREF.ID
        _PAD-SHARED-LENS SDLENS.RID RID= 0= IF
        -1 EXIT
    THEN
    _PAD-SHARED-CANDIDATE-REF _PAD-SHARED-LENS SDLENS.CONTEXT @
        _PAD-SHARED-LENS SDLENS.RREG @
        _PAD-SHARED-CANDIDATE-BIND LBIND-ATTACH
    DUP IF _PAD-LBIND-IOR EXIT THEN DROP
    \ Revision zero is resolved only during attach; Pad retains exact state.
...
: _PAD-OPEN-SHARED-LATEST  ( -- ior )
    _PAD-RESOURCE-MODE @ SDLENS-M-BLOCKED = IF
        _PAD-E-SHARED-UNAVAILABLE EXIT
    THEN
    _PAD-RESOURCE-MODE @ SDLENS-M-SHARED <> IF
        _PAD-E-SHARED-FAILED EXIT
    THEN
    3 0 DO
        _PAD-SHARED-LENS SDLENS.RID
            _PAD-SHARED-LENS SDLENS.CONTEXT @
            _PAD-SHARED-CANDIDATE-REF
            _PAD-SHARED-LENS SDLENS.RREG @ RREG-REF DUP IF
...
: _PAD-SHARED-SAVE  ( -- ior )
    ...
    THEN
    _PAD-SHARED-BIND _PAD-SHARED-LENS SDLENS.REPLACE-CAP @
        _PAD-SHARED-REQUEST!
    ...
    _PAD-SHARED-REQUEST @ _PAD-SHARED-LENS SDLENS.CONTEXT @
        _PAD-SHARED-BIND
...
: _PAD-OPEN-PATH  ( fname-a fname-u -- ior )
    ...
    _PIO-NAME-U ! _PIO-NAME-A !
    _PIO-NAME-A @ _PIO-NAME-U @ _PAD-DAYBOOK-PATH? IF
        _PAD-RESOURCE-MODE @ SDLENS-M-DIRECT <> IF
