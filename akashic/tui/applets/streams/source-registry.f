...
5 CONSTANT SSOURCE-KIND-ATPROTO-AUTHENTICATED
...
: STREAMS-SOURCE-INIT  ( source -- )
    ...
    DUP STREAMS-SOURCE-OBSERVATION-DEFAULT
        SWAP SSOURCE.OBSERVATION-MAX !
    DUP STREAMS-SOURCE-REVISION-DEFAULT SWAP SSOURCE.REVISION-MAX !
...
: _SSOURCE-KIND-FORMAT?  ( kind format -- flag )
    ...
    THEN
    _SSKF-KIND @ SSOURCE-KIND-ATPROTO-AUTHENTICATED = IF
        _SSKF-FORMAT @ SSOURCE-FORMAT-ATPROTO-JSON = EXIT
    THEN
    0 ;
...
: _SSOURCE-CONFIG-VALID?  ( source -- flag )
    ...
    DUP STREAMS-SOURCE-SIZE MSPAN-NONWRAPPING? 0= IF DROP 0 EXIT THEN
    _SSCV-S !
