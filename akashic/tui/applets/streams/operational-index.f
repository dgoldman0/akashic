...
\  replay from a mismatched request with one logarithmic lookup.  The shared
\  usage tree instead carries exact connector and flow admission counters.
\
...
7 CONSTANT STREAMS-OI-TREE-OPERATIONAL-USAGE
8 CONSTANT STREAMS-OI-TREE-COUNT
...
0x53524F4955534147 CONSTANT STREAMS-OI-SCOPE-OPERATIONAL-USAGE \ "SROIUSAG"

\ Family bytes remain globally distinct where ready/active families share the
\ dispatch tree and connector/flow families share the usage tree.
...
0x81 CONSTANT STREAMS-OI-F-CONNECTOR-USAGE
0x82 CONSTANT STREAMS-OI-F-FLOW-USAGE
...
1 RID-SIZE +
    CONSTANT STREAMS-OI-CONNECTOR-USAGE-KEY-SIZE
STREAMS-OI-FAMILY-PREFIX-SIZE
    CONSTANT STREAMS-OI-CONNECTOR-USAGE-PREFIX-SIZE

1 RID-SIZE +
    CONSTANT STREAMS-OI-FLOW-USAGE-KEY-SIZE
STREAMS-OI-FAMILY-PREFIX-SIZE
    CONSTANT STREAMS-OI-FLOW-USAGE-PREFIX-SIZE
...
2 STREAMS-OI-ORDER-SIZE *
    CONSTANT STREAMS-OI-CONNECTOR-USAGE-VALUE-SIZE
STREAMS-OI-ORDER-SIZE
    CONSTANT STREAMS-OI-FLOW-USAGE-VALUE-SIZE
...
STREAMS-OI-IDEMPOTENCY-VALUE-SIZE PBTREE-VALUE-MAX <> [IF]
    _SOI-GEOMETRY-ABORT
[THEN]
STREAMS-OI-CONNECTOR-USAGE-VALUE-SIZE PBTREE-VALUE-MAX > [IF]
    _SOI-GEOMETRY-ABORT
[THEN]
STREAMS-OI-FLOW-USAGE-VALUE-SIZE PBTREE-VALUE-MAX > [IF]
    _SOI-GEOMETRY-ABORT
[THEN]
...
: STREAMS-OI-FAMILY-VALID?  ( family -- flag )
    ...
    OVER STREAMS-OI-F-IDEMPOTENCY = OR
    OVER STREAMS-OI-F-CONNECTOR-USAGE = OR
    SWAP STREAMS-OI-F-FLOW-USAGE = OR ;
...
: STREAMS-OI-TREE-SCOPE  ( tree-index -- scope|0 )
    CASE
        ...
        ENDOF
        STREAMS-OI-TREE-OPERATIONAL-USAGE OF
            STREAMS-OI-SCOPE-OPERATIONAL-USAGE
        ENDOF
        0 SWAP
    ENDCASE ;
...
: STREAMS-OI-SCOPE-TREE  ( scope -- tree-index|-1 )
    ...
    THEN
    DUP STREAMS-OI-SCOPE-IDEMPOTENCY = IF
        DROP STREAMS-OI-TREE-IDEMPOTENCY EXIT
    THEN
    STREAMS-OI-SCOPE-OPERATIONAL-USAGE = IF
...
: STREAMS-OI-FAMILY-TREE  ( family -- tree-index|-1 )
    ...
    DUP STREAMS-OI-F-TERMINAL-RETENTION = IF
        DROP STREAMS-OI-TREE-TERMINAL-RETENTION EXIT
    THEN
    DUP STREAMS-OI-F-IDEMPOTENCY = IF
        DROP STREAMS-OI-TREE-IDEMPOTENCY EXIT
    THEN
    DUP STREAMS-OI-F-CONNECTOR-USAGE =
    SWAP STREAMS-OI-F-FLOW-USAGE = OR IF
        STREAMS-OI-TREE-OPERATIONAL-USAGE
    ELSE
        -1
    THEN ;
...
: STREAMS-OI-DISPATCH-READY-ORDER-PREFIX
  ( ready-sequence destination -- status )
...
: STREAMS-OI-DISPATCH-READY-KEY
  ( ready-sequence attempt-rid destination -- status )
    2 PICK 0> 0= IF
...
: STREAMS-OI-DISPATCH-READY-KEY-SEQUENCE@
  ( key-a key-u -- ready-sequence flag )
...
\ ---------------------------------------------------------------------
\ Per-connector and per-flow operational usage
\ ---------------------------------------------------------------------

: STREAMS-OI-CONNECTOR-USAGE-PREFIX  ( destination -- status )
    STREAMS-OI-F-CONNECTOR-USAGE SWAP _SOI-FAMILY-PREFIX ;

: STREAMS-OI-CONNECTOR-USAGE-PREFIX-VALID?
  ( prefix-a prefix-u -- flag )
    STREAMS-OI-F-CONNECTOR-USAGE _SOI-FAMILY-PREFIX? ;

: STREAMS-OI-CONNECTOR-USAGE-KEY
  ( connector-rid destination -- status )
    STREAMS-OI-F-CONNECTOR-USAGE SWAP _SOI-ONE-RID-KEY ;

: STREAMS-OI-CONNECTOR-USAGE-KEY-VALID?  ( key-a key-u -- flag )
    STREAMS-OI-F-CONNECTOR-USAGE _SOI-ONE-RID-KEY? ;

: STREAMS-OI-CONNECTOR-USAGE-KEY-ID@  ( key-a key-u -- rid|0 )
    STREAMS-OI-F-CONNECTOR-USAGE _SOI-ONE-RID-KEY-ID@ ;

: STREAMS-OI-FLOW-USAGE-PREFIX  ( destination -- status )
    STREAMS-OI-F-FLOW-USAGE SWAP _SOI-FAMILY-PREFIX ;

: STREAMS-OI-FLOW-USAGE-PREFIX-VALID?  ( prefix-a prefix-u -- flag )
    STREAMS-OI-F-FLOW-USAGE _SOI-FAMILY-PREFIX? ;

: STREAMS-OI-FLOW-USAGE-KEY  ( flow-rid destination -- status )
    STREAMS-OI-F-FLOW-USAGE SWAP _SOI-ONE-RID-KEY ;

: STREAMS-OI-FLOW-USAGE-KEY-VALID?  ( key-a key-u -- flag )
    STREAMS-OI-F-FLOW-USAGE _SOI-ONE-RID-KEY? ;

: STREAMS-OI-FLOW-USAGE-KEY-ID@  ( key-a key-u -- rid|0 )
    STREAMS-OI-F-FLOW-USAGE _SOI-ONE-RID-KEY-ID@ ;
...
: STREAMS-OI-KEY-VALID?  ( key-a key-u -- flag )
    ...
    THEN
    DUP STREAMS-OI-F-IDEMPOTENCY = IF
        DROP STREAMS-OI-IDEMPOTENCY-KEY-VALID? EXIT
    THEN
    DUP STREAMS-OI-F-CONNECTOR-USAGE = IF
        DROP STREAMS-OI-CONNECTOR-USAGE-KEY-VALID? EXIT
    THEN
    STREAMS-OI-F-FLOW-USAGE = IF
        STREAMS-OI-FLOW-USAGE-KEY-VALID?
    ELSE
        2DROP 0
    THEN ;
...
: STREAMS-OI-CONNECTOR-USAGE-VALUE
  ( retained-item-count retained-payload-bytes destination -- status )
    >R
    OVER _SOI-NONNEGATIVE? 0= IF
        2DROP R> DROP STREAMS-OI-S-INVALID EXIT
    THEN
    DUP _SOI-NONNEGATIVE? 0= IF
        2DROP R> DROP STREAMS-OI-S-INVALID EXIT
    THEN
    OVER 0= OVER 0<> AND IF
        2DROP R> DROP STREAMS-OI-S-INVALID EXIT
    THEN
    R@ STREAMS-OI-CONNECTOR-USAGE-VALUE-SIZE
        _SOI-VALUE-DEST? 0= IF
        2DROP R> DROP STREAMS-OI-S-INVALID EXIT
    THEN
    R@ STREAMS-OI-CONNECTOR-USAGE-VALUE-SIZE 0 FILL
    OVER R@ _SOI-U64-BE!
    DUP R@ STREAMS-OI-ORDER-SIZE + _SOI-U64-BE!
    2DROP R> DROP STREAMS-OI-S-OK ;

: STREAMS-OI-CONNECTOR-USAGE-VALUE-VALID?
  ( value-a value-u -- flag )
    2DUP _SOI-SPAN? 0= IF 2DROP 0 EXIT THEN
    STREAMS-OI-CONNECTOR-USAGE-VALUE-SIZE <> IF DROP 0 EXIT THEN
    DUP _SOI-U64-BE-NONNEGATIVE? 0= IF DROP 0 EXIT THEN
    DUP STREAMS-OI-ORDER-SIZE + _SOI-U64-BE-NONNEGATIVE? 0=
        IF DROP 0 EXIT THEN
    DUP _SOI-U64-BE@ 0= IF
        STREAMS-OI-ORDER-SIZE + _SOI-U64-BE@ 0=
    ELSE
        DROP -1
    THEN ;

: STREAMS-OI-CONNECTOR-USAGE-VALUE-ITEM-COUNT@
  ( value-a value-u -- retained-item-count flag )
    2DUP STREAMS-OI-CONNECTOR-USAGE-VALUE-VALID? IF
        DROP _SOI-U64-BE@ -1
    ELSE
        2DROP 0 0
    THEN ;
...
: STREAMS-OI-CONNECTOR-USAGE-VALUE-PAYLOAD-BYTES@
  ( value-a value-u -- retained-payload-bytes flag )
    2DUP STREAMS-OI-CONNECTOR-USAGE-VALUE-VALID? IF
        DROP STREAMS-OI-ORDER-SIZE + _SOI-U64-BE@ -1
    ELSE
        2DROP 0 0
    THEN ;

: STREAMS-OI-FLOW-USAGE-VALUE  ( active-count destination -- status )
    >R
    DUP _SOI-NONNEGATIVE? 0= IF
        DROP R> DROP STREAMS-OI-S-INVALID EXIT
    THEN
    R@ STREAMS-OI-FLOW-USAGE-VALUE-SIZE _SOI-VALUE-DEST? 0= IF
        DROP R> DROP STREAMS-OI-S-INVALID EXIT
    THEN
    R@ STREAMS-OI-FLOW-USAGE-VALUE-SIZE 0 FILL
    DUP R@ _SOI-U64-BE!
    DROP R> DROP STREAMS-OI-S-OK ;

: STREAMS-OI-FLOW-USAGE-VALUE-VALID?  ( value-a value-u -- flag )
    2DUP _SOI-SPAN? 0= IF 2DROP 0 EXIT THEN
    STREAMS-OI-FLOW-USAGE-VALUE-SIZE <> IF DROP 0 EXIT THEN
    _SOI-U64-BE-NONNEGATIVE? ;

: STREAMS-OI-FLOW-USAGE-VALUE-ACTIVE-COUNT@
  ( value-a value-u -- active-count flag )
    2DUP STREAMS-OI-FLOW-USAGE-VALUE-VALID? IF
        DROP _SOI-U64-BE@ -1
    ELSE
        2DROP 0 0
    THEN ;
...
: STREAMS-OI-FAMILY-VALUE-VALID?
  ( value-a value-u family -- flag )
    ...
    THEN
    DUP STREAMS-OI-F-IDEMPOTENCY = IF
        DROP STREAMS-OI-IDEMPOTENCY-VALUE-VALID? EXIT
    THEN
    DUP STREAMS-OI-F-CONNECTOR-USAGE = IF
        DROP STREAMS-OI-CONNECTOR-USAGE-VALUE-VALID? EXIT
    THEN
    STREAMS-OI-F-FLOW-USAGE = IF
        STREAMS-OI-FLOW-USAGE-VALUE-VALID?
    ELSE
