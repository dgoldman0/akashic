...
\ The instrument schema is fixed: one header, eight READOUTs, two METERs,
\ and three STATUS objects.  This is the exact caller-owned graph size, not
\ a renderer reservation.
1960 CONSTANT _SL-DGRAPH-CAP
   1 CONSTANT _SL-DGRAPH-ROOT-KEY
  13 CONSTANT _SL-DGRAPH-OBJECT-COUNT

0xEEEEEEFF CONSTANT _SL-DG-TEXT
0x00000000 CONSTANT _SL-DG-TRANSPARENT
0x3A3A3AFF CONSTANT _SL-DG-TRACK
0x5FAF5FFF CONSTANT _SL-DG-PEAK
0x00AFAFFF CONSTANT _SL-DG-RMS
0x666666FF CONSTANT _SL-DG-INACTIVE
0xFFFF00FF CONSTANT _SL-DG-DEVICE
...
_SL-CURRENT-STATE CMP-CELL: _SL-DGRAPH-WIDGET
_SL-CURRENT-STATE CMP-CELL: _SL-DGRAPH-ACTIVE-A
_SL-CURRENT-STATE CMP-CELL: _SL-DGRAPH-ACTIVE-U
_SL-CURRENT-STATE _SL-DGRAPH-CAP CMP-FIELD: _SL-DGRAPH-BANK-A
_SL-CURRENT-STATE _SL-DGRAPH-CAP CMP-FIELD: _SL-DGRAPH-BANK-B
_SL-CURRENT-STATE UDG-BUILDER-SIZE CMP-FIELD: _SL-DGRAPH-BUILDER
_SL-CURRENT-STATE UDG-SUMMARY-SIZE CMP-FIELD: _SL-DGRAPH-SUMMARY
_SL-CURRENT-STATE CMP-CELL: _SL-DGRAPH-BOUND-H
_SL-CURRENT-STATE CMP-CELL: _SL-DGRAPH-BOUND-W
_SL-CURRENT-STATE CMP-CELL: _SL-DGRAPH-OWNED-ACTIVE
_SL-CURRENT-STATE CMP-CELL: _SL-DGRAPH-AUDIO-PRESENT
...
: _SL-DGRAPH-INVALIDATE  ( -- )
    _SL-DGRAPH-REBUILD-D
    _SL-INVALIDATE ;
...
: _SL-RENDER  ( -- ior )
    ...
    ['] _SL-RENDER-BODY CATCH DUP IF
        ...
        0 _SL-RENDER-VALID !
        _SL-RENDER-CLEANUP
        _SL-DGRAPH-INVALIDATE EXIT
    THEN
    DROP _SL-COMMIT-RENDER _SL-DGRAPH-INVALIDATE 0 ;
...
VARIABLE _SL-METER-W

: _SL-LAYOUT  ( -- )
    _SL-PANEL-RGN @ ?DUP IF
        DUP RGN-W _SL-DW ! RGN-H _SL-DH !
    ELSE
        0 _SL-DW ! 0 _SL-DH !
    THEN
    ...
    _SL-DH @ 18 >= IF
        ...
        _SL-PLOT-ROW @ _SL-PLOT-H @ + 1+ _SL-ANALYSIS-ROW !
    ELSE
        0 _SL-PLOT-ROW ! 0 _SL-PLOT-H ! 0 _SL-PLOT-MID !
        10 _SL-ANALYSIS-ROW !
    THEN
    _SL-DW @ 16 - 4 MAX 52 MIN _SL-METER-W ! ;

\ ---------------------------------------------------------------------
\ Renderer-neutral analysis instrument
\ ---------------------------------------------------------------------

VARIABLE _SL-DG-DEST
VARIABLE _SL-DG-U
VARIABLE _SL-DG-FLAGS

: _SL-DGRAPH-INACTIVE  ( -- address )
    _SL-DGRAPH-ACTIVE-A @ _SL-DGRAPH-BANK-A = IF
        _SL-DGRAPH-BANK-B
    ELSE
        _SL-DGRAPH-BANK-A
    THEN ;

: _SL-DGRAPH-REBUILD  ( -- status )
    _SL-DGRAPH-WIDGET @ 0= IF DGRAPH-S-OK EXIT THEN
    _SL-LAYOUT
    _SL-DW @ 0> _SL-DH @ 0> AND 0= IF DGRAPH-S-INVALID EXIT THEN
    _SL-DGRAPH-INACTIVE DUP _SL-DG-DEST !
        _SL-DGRAPH-CAP _SL-DGRAPH-BUILDER UDG-BUILDER-INIT
    DUP IF EXIT THEN DROP
    _SL-DGRAPH-ROOT-KEY 0 0 _SL-DH @ _SL-DW @
        UDG-STATE-VISIBLE UDG-STATE-ENABLED OR
        _SL-DGRAPH-BUILDER UDG-BEGIN
    DUP IF EXIT THEN DROP

    _SL-RENDER-VALID @ IF UDG-OBJECT-VISIBLE ELSE 0 THEN
        _SL-DG-FLAGS !

    10 _SL-ANALYSIS-ROW @ 7 1 5 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-PERCENT 0
        _SL-M-PEAK @ _SL-FP16-PERCENT 0 MAX 100 MIN 100
        0 0 _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP
    11 _SL-ANALYSIS-ROW @ 21 1 7 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-INTEGER 0 _SL-M-PEAK-FRAME @ 1
        0 0 _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP
    12 _SL-ANALYSIS-ROW @ 35 1 5 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-PERCENT 0
        _SL-M-RMS @ _SL-FP16-PERCENT 0 MAX 100 MIN 100
        0 0 _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP
    13 _SL-ANALYSIS-ROW @ 46 1 11 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-INTEGER 0 _SL-M-DC @ _SL-FP16-PERMILLE 1
        S"  ppt" _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP
    14 _SL-ANALYSIS-ROW @ 1+ 8 1 10 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-INTEGER 0 _SL-M-PITCH @ FP16>INT 1
        S"  Hz" _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP
    15 _SL-ANALYSIS-ROW @ 1+ 30 1 11 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-INTEGER 0 _SL-M-CENTROID @ FP16>INT 1
        S"  Hz" _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP
    16 _SL-ANALYSIS-ROW @ 1+ 53 1 9 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-INTEGER 0 _SL-M-ZC @ 1
        0 0 _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP
    17 _SL-ANALYSIS-ROW @ 1+ 70 1 8 1 _SL-DG-FLAGS @
        _SL-DG-TEXT _SL-DG-TRANSPARENT
        UDG-READOUT-INTEGER 0 _SL-M-CLIPS @ 1
        0 0 _SL-DGRAPH-BUILDER UDG-READOUT
    DUP IF EXIT THEN DROP

    20 _SL-ANALYSIS-ROW @ 2 + 14 1 _SL-METER-W @ 0 _SL-DG-FLAGS @
        _SL-DG-PEAK _SL-DG-TRACK UDG-METER-HORIZONTAL 0
        0 100 _SL-M-PEAK @ _SL-FP16-PERCENT 0 MAX 100 MIN
        _SL-DGRAPH-BUILDER UDG-METER
    DUP IF EXIT THEN DROP
    21 _SL-ANALYSIS-ROW @ 3 + 14 1 _SL-METER-W @ 0 _SL-DG-FLAGS @
        _SL-DG-RMS _SL-DG-TRACK UDG-METER-HORIZONTAL 0
        0 100 _SL-M-RMS @ _SL-FP16-PERCENT 0 MAX 100 MIN
        _SL-DGRAPH-BUILDER UDG-METER
    DUP IF EXIT THEN DROP

    30 8 2 1 1 2 UDG-OBJECT-VISIBLE
        _SL-DG-INACTIVE _SL-DG-RMS _SL-RENDER-VALID @
        UDG-STATUS-CIRCLE _SL-DGRAPH-BUILDER UDG-STATUS
    DUP IF EXIT THEN DROP
    31 9 4 1 1 2 UDG-OBJECT-VISIBLE
        _SL-DG-INACTIVE _SL-DG-PEAK _SL-OWNED-ACTIVE?
        UDG-STATUS-DIAMOND _SL-DGRAPH-BUILDER UDG-STATUS
    DUP IF EXIT THEN DROP
    32 9 2 1 1 2 UDG-OBJECT-VISIBLE
        _SL-DG-INACTIVE _SL-DG-DEVICE AUDIO-OUT-PRESENT?
        UDG-STATUS-SQUARE _SL-DGRAPH-BUILDER UDG-STATUS
    DUP IF EXIT THEN DROP

    _SL-DGRAPH-BUILDER UDG-END
    DUP IF EXIT THEN DROP
    _SL-DGRAPH-BUILDER UDG-BUILDER-FINISH
    DUP IF NIP EXIT THEN DROP
    DUP _SL-DGRAPH-CAP <> IF DROP DGRAPH-S-INVALID EXIT THEN
    _SL-DG-U !
    _SL-DG-DEST @ UDG-OBJECT-COUNT@
        _SL-DGRAPH-OBJECT-COUNT <> IF DGRAPH-S-INVALID EXIT THEN
    _SL-DG-DEST @ _SL-DG-U @ _SL-DGRAPH-SUMMARY
        _SL-DGRAPH-WIDGET @ DGRAPH-BIND
    DUP IF EXIT THEN DROP
    _SL-DG-DEST @ _SL-DGRAPH-ACTIVE-A !
    _SL-DG-U @ _SL-DGRAPH-ACTIVE-U !
    _SL-DH @ _SL-DGRAPH-BOUND-H !
    _SL-DW @ _SL-DGRAPH-BOUND-W !
    _SL-OWNED-ACTIVE? _SL-DGRAPH-OWNED-ACTIVE !
    AUDIO-OUT-PRESENT? _SL-DGRAPH-AUDIO-PRESENT !
    _SL-DGRAPH-WIDGET @ WDG-SHOW
    DGRAPH-S-OK ;

: _SL-DGRAPH-REBUILD!  ( -- )
    _SL-DGRAPH-WIDGET @ 0= IF EXIT THEN
    _SL-DGRAPH-REBUILD DGRAPH-S-OK <>
        ABORT" soundlab: instrument graph rebuild" ;

: _SL-DGRAPH-ENSURE-LAYOUT  ( -- )
    _SL-DGRAPH-WIDGET @ 0= IF EXIT THEN
    _SL-DH @ _SL-DGRAPH-BOUND-H @ <>
    _SL-DW @ _SL-DGRAPH-BOUND-W @ <> OR IF
        _SL-DGRAPH-REBUILD!
    THEN ;

' _SL-DGRAPH-REBUILD! IS _SL-DGRAPH-REBUILD-D
...
: _SL-DRAW-ANALYSIS  ( -- )
    ...
    250 234 0 DRW-STYLE!
    S" Peak" _SL-ANALYSIS-ROW @ 2 DRW-TEXT
    S" @ frame" _SL-ANALYSIS-ROW @ 13 DRW-TEXT
    S" RMS" _SL-ANALYSIS-ROW @ 30 DRW-TEXT
    S" DC" _SL-ANALYSIS-ROW @ 43 DRW-TEXT
    S" Pitch" _SL-ANALYSIS-ROW @ 1+ 2 DRW-TEXT
    S" centroid" _SL-ANALYSIS-ROW @ 1+ 21 DRW-TEXT
    S" crossings" _SL-ANALYSIS-ROW @ 1+ 43 DRW-TEXT
    S" clips" _SL-ANALYSIS-ROW @ 1+ 64 DRW-TEXT
    S" Peak level" _SL-ANALYSIS-ROW @ 2 + 2 DRW-TEXT
    S" RMS level" _SL-ANALYSIS-ROW @ 3 + 2 DRW-TEXT

    _SL-ANALYSIS-ROW @ 6 + _SL-DH @ < IF
        _SL-DW @ 6 - 4 MAX _SL-LANDMARK-W !
        244 234 CELL-A-DIM DRW-STYLE!
        S" Frequency landmarks: 0 Hz" _SL-ANALYSIS-ROW @ 5 + 2 DRW-TEXT
        S" 4 kHz" _SL-ANALYSIS-ROW @ 5 + 2 _SL-LANDMARK-W @ DRW-TEXT-RIGHT
        9472 _SL-ANALYSIS-ROW @ 6 + 2 _SL-LANDMARK-W @ DRW-HLINE
        _SL-M-PITCH @ FP16>INT 0 MAX 4000 MIN _SL-LANDMARK-W @ 1- * 4000 /
        2 + _SL-PITCH-X !
        _SL-M-CENTROID @ FP16>INT 0 MAX 4000 MIN _SL-LANDMARK-W @ 1- * 4000 /
        2 + _SL-CENTROID-X !
        81 234 CELL-A-BOLD DRW-STYLE!
        [CHAR] P _SL-ANALYSIS-ROW @ 6 + _SL-PITCH-X @ DRW-CHAR
        220 DRW-FG!
        _SL-CENTROID-X @ _SL-PITCH-X @ = IF [CHAR] * ELSE [CHAR] C THEN
        _SL-ANALYSIS-ROW @ 6 + _SL-CENTROID-X @ DRW-CHAR
    THEN ;

: _SL-PANEL-DRAW  ( widget -- )
    DROP _SL-LAYOUT _SL-DGRAPH-ENSURE-LAYOUT
    253 234 0 DRW-STYLE!
    ...
    THEN THEN
    _SL-DW @ 8 - 0 MAX MIN 9 6 DRW-TEXT
    _SL-DRAW-WAVEFORM
    _SL-DRAW-ANALYSIS
    _SL-DGRAPH-WIDGET @ ?DUP IF
        _SL-PANEL-RGN @ WDG-DRAW-IN
    THEN
    DRW-STYLE-RESET ;
...
: SOUNDLAB-INIT-CB  ( instance -- )
    ...
    0 _SL-DGRAPH-WIDGET !
    0 _SL-DGRAPH-ACTIVE-A ! 0 _SL-DGRAPH-ACTIVE-U !
    0 _SL-DGRAPH-BOUND-H ! 0 _SL-DGRAPH-BOUND-W !
    0 _SL-DGRAPH-OWNED-ACTIVE !
    0 _SL-DGRAPH-AUDIO-PRESENT !
    _SL-PM-NONE _SL-PROMPT-MODE !
    ...
    _SL-E-BODY @ ?DUP IF
        ...
        _SL-PANEL-RGN @ DGRAPH-NEW _SL-DGRAPH-WIDGET !
        _SL-DGRAPH-REBUILD DGRAPH-S-OK <>
            ABORT" soundlab: initial instrument graph"
...
: SOUNDLAB-TICK-CB  ( instance -- )
    _SL-ACTIVATE
    _SL-DGRAPH-WIDGET @ IF
        _SL-OWNED-ACTIVE? _SL-DGRAPH-OWNED-ACTIVE @ <>
        AUDIO-OUT-PRESENT? _SL-DGRAPH-AUDIO-PRESENT @ <> OR IF
            _SL-DGRAPH-INVALIDATE
        THEN
    THEN ;
...
: SOUNDLAB-SHUTDOWN-CB  ( instance -- )
    ...
    _SL-DGRAPH-WIDGET @ ?DUP IF DGRAPH-FREE THEN
