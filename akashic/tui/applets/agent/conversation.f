...
1  CONSTANT AMSG-F-TOOL
2  CONSTANT AMSG-F-AUDIT
4  CONSTANT AMSG-F-APPROVED
8  CONSTANT AMSG-F-DENIED
16 CONSTANT AMSG-F-RECOVERED
...
40 CONSTANT _AC-MODEL-CONTEXT
48 CONSTANT _AC-MESSAGES
...
: ACONV.MODEL-CONTEXT ( conversation -- a ) _AC-MODEL-CONTEXT + ;
...
: ACONV-NEW  ( -- conversation ior )
    ...
    DUP IF EXIT THEN
    DROP DUP _ACN-C ! AGENT-CONVERSATION-SIZE 0 FILL
    ACTX-NEW DUP IF
        NIP _ACN-C @ FREE 0 SWAP EXIT
    THEN
    DROP _ACN-X !
    _ACN-X @ _ACN-C @ ACONV.MODEL-CONTEXT !
    1 _ACN-C @ ACONV.ID ! 1 _ACN-C @ ACONV.REVISION !
    _ACN-C @ 0 ;
...
: ACONV-ROOM?  ( needed conversation -- flag )
    ACONV.COUNT @ + ACONV-MAX-MESSAGES <= ;
...
: ACONV-FREE  ( conversation -- )
    ...
    _ACF-C @ ACONV.MODEL-CONTEXT @ ACTX-FREE
    FREE ;
...
: ACONV-DROP-LAST  ( conversation -- )
    DUP ACONV.COUNT @ DUP 0= IF 2DROP EXIT THEN
    1- OVER ACONV-NTH AMSG-FREE
    -1 OVER ACONV.COUNT +!
    1 SWAP ACONV.REVISION +! ;
...
: ACONV-CLEAR  ( conversation -- )
    _ACF-C !
    _ACF-C @ ACONV.COUNT @ 0 ?DO I _ACF-C @ ACONV-NTH AMSG-FREE LOOP
    _ACF-C @ ACONV.MODEL-CONTEXT @ ACTX-CLEAR
