...
\
\  Public frozen-slice validation API:
\    UCSN-FROZEN-VALIDATE
\      ( validation-a validation-u work-a work-u
\        descriptors-a descriptors-u native-a native-u -- status )
\
\  The frozen validator owns no live UIDL observation.  It deep-validates
\  every pointer-free native entry, correlates the exact regenerated summary
\  with its descriptor, and proves that the descriptor-local native offsets
\  cover the passed native slice exactly.  Descriptor identity order and
\  native visit order are independent, so caller WORK holds one native offset
\  per descriptor for an unsigned O(n log n) coverage proof.
\
...
: UCSN-FROZEN-WORK-BYTES  ( descriptor-count -- bytes|0 )
    8 _UCSN-SIZED-BYTES ;
...
VARIABLE _UCSN-DIRTY-VALIDATION-U
VARIABLE _UCSN-DIRTY-WORK-U
...
CREATE _UCSN-PROOF-SPANS 5 MSPAN-SET-BYTES ALLOT

\ Collect the five caller spans for one enclosed authority proof.  Their
\ shapes are admitted first, and the prover skips empty optional spans.
: _UCSN-PROOF-SPANS?  ( -- flag )
    5 _UCSN-PROOF-SPANS MSPAN-SET-INIT MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UCSN-BUILDER @ USCOL-BUILDER-SIZE _UCSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UCSN-VALIDATION-A @ _UCSN-VALIDATION-U @
        _UCSN-PROOF-SPANS MSPAN-SET-PUSH MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UCSN-WORK-A @ _UCSN-WORK-U @ _UCSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UCSN-DESCRIPTORS-A @ _UCSN-DESCRIPTORS-U @
        _UCSN-PROOF-SPANS MSPAN-SET-PUSH MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UCSN-NATIVE-A @ _UCSN-NATIVE-U @ _UCSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK = ;
...
: _UCSN-RANGES?  ( -- flag )
    ...
    \ Each authority query walks the document's storage.  Clustered caller
    \ spans need one enclosed query instead of one per span.
    _UCSN-PROOF-SPANS? 0= IF 0 EXIT THEN
    _UCSN-PROOF-SPANS ['] _UCSN-AUTHORITY-DISJOINT?
        MSPAN-SET-PROVE-DISJOINT? 0= IF 0 EXIT THEN
...
: _UCSN-CLEAR-SCRATCH  ( -- )
    ...
    _UCSN-DIRTY-VALIDATION-U @ ?DUP IF
        _UCSN-VALIDATION-A @ SWAP 0 FILL
    THEN
    _UCSN-DIRTY-WORK-U @ ?DUP IF
        _UCSN-WORK-A @ SWAP 0 FILL
    THEN ;
...
: _UCSN-V-CAPACITY?  ( -- flag )
    ...
    _UCSN-COUNT @ _UCSN-WORK-NODE-AT _UCSN-V-WORK !
    \ Dense nodes are claimed only at COUNT.  Initialize the selected node
    \ locally instead of clearing the entire capacity-sized node arena, and
    \ advance the dirty prefix before FILL for caught-write cleanup.
    _UCSN-WORK-SOURCE-U @
    _UCSN-COUNT @ 1+ UCSN-WORK-NODE-SIZE * +
    DUP _UCSN-WORK-U @ U> IF DROP _UCSN-SET-INVALID 0 EXIT THEN
    _UCSN-DIRTY-WORK-U !
...
: _UCSN-V-VALIDATE?  ( -- flag )
    ...
    DUP USCOL-S-OK <> IF
        >R DROP R> _UCSN-MAP-USCOL 0 EXIT
    THEN
    DROP DUP _UCSN-VALIDATION-U @ U> IF
        DROP _UCSN-SET-CAPACITY 0 EXIT
    THEN
    DUP _UCSN-V-VALIDATION-U !
    DUP _UCSN-DIRTY-VALIDATION-U @ U> IF
        DUP _UCSN-DIRTY-VALIDATION-U !
    THEN
    DROP
    _UCSN-V-ENTRY @ _UCSN-V-ENTRY-U @
    _UCSN-V-VALIDATION-U @ ?DUP IF
        _UCSN-VALIDATION-A @ SWAP
    ELSE
        0 0
    THEN
...
: _UCSN-CAPTURE-OBSERVED  ( -- descriptor-count native-used status )
    ...
    0 _UCSN-DIRTY-VALIDATION-U ! 0 _UCSN-DIRTY-WORK-U !
...
: _UCSN-SCRUB  ( -- )
    ...
    0 _UCSN-V-ENTRY-U ! 0 _UCSN-V-VALIDATION-U !
    0 _UCSN-V-NEXT-NATIVE !
    0 _UCSN-V-REMAINING !
...
\ =====================================================================
\  Complete validation of one frozen descriptor/native slice
\ =====================================================================

VARIABLE _UCSN-F-VALIDATION-A
VARIABLE _UCSN-F-VALIDATION-U
VARIABLE _UCSN-F-WORK-A
VARIABLE _UCSN-F-WORK-U
VARIABLE _UCSN-F-DESCRIPTORS-A
VARIABLE _UCSN-F-DESCRIPTORS-U
VARIABLE _UCSN-F-NATIVE-A
VARIABLE _UCSN-F-NATIVE-U
VARIABLE _UCSN-F-RANGES-VALID
VARIABLE _UCSN-F-STATUS
VARIABLE _UCSN-F-COUNT
VARIABLE _UCSN-F-WORK-NEED
VARIABLE _UCSN-F-DIRTY-VALIDATION-U
VARIABLE _UCSN-F-DIRTY-WORK-U

VARIABLE _UCSN-F-I
VARIABLE _UCSN-F-DESCRIPTOR
VARIABLE _UCSN-F-ENTRY
VARIABLE _UCSN-F-ENTRY-U
VARIABLE _UCSN-F-NATIVE-O
VARIABLE _UCSN-F-VALIDATION-NEED
VARIABLE _UCSN-F-INDEX
VARIABLE _UCSN-F-GENERATION
VARIABLE _UCSN-F-ROOT-KEY
VARIABLE _UCSN-F-PRIOR-INDEX
VARIABLE _UCSN-F-PRIOR-GENERATION
VARIABLE _UCSN-F-PRIOR-ROOT
VARIABLE _UCSN-F-HAVE-PRIOR
VARIABLE _UCSN-F-ROOT-BOTTOM
VARIABLE _UCSN-F-ROOT-RIGHT
VARIABLE _UCSN-F-CLIP-BOTTOM
VARIABLE _UCSN-F-CLIP-RIGHT
VARIABLE _UCSN-F-CURSOR
...
\ sorting offsets is therefore required before exact coverage can be proved.
VARIABLE _UCSN-FS-BASE
VARIABLE _UCSN-FS-COUNT
VARIABLE _UCSN-FS-ROOT
VARIABLE _UCSN-FS-CHILD
VARIABLE _UCSN-FS-LARGEST
VARIABLE _UCSN-FS-START
VARIABLE _UCSN-FS-END
VARIABLE _UCSN-FS-TEMP

: _UCSN-FS-AT  ( index -- address )
    8 * _UCSN-FS-BASE @ + ;

: _UCSN-FS-SWAP  ( index-a index-b -- )
    _UCSN-FS-AT SWAP _UCSN-FS-AT
    OVER @ _UCSN-FS-TEMP !
    DUP @ ROT !
    _UCSN-FS-TEMP @ SWAP ! ;

: _UCSN-FS-SIFT  ( root count -- )
    _UCSN-FS-COUNT ! _UCSN-FS-ROOT !
    BEGIN
        _UCSN-FS-ROOT @ 2* 1+ DUP _UCSN-FS-CHILD !
        _UCSN-FS-COUNT @ U<
    WHILE
        _UCSN-FS-CHILD @ _UCSN-FS-LARGEST !
        _UCSN-FS-CHILD @ 1+ _UCSN-FS-COUNT @ U< IF
            _UCSN-FS-CHILD @ _UCSN-FS-AT @
            _UCSN-FS-CHILD @ 1+ _UCSN-FS-AT @ U< IF
                _UCSN-FS-CHILD @ 1+ _UCSN-FS-LARGEST !
            THEN
        THEN
        _UCSN-FS-ROOT @ _UCSN-FS-AT @
        _UCSN-FS-LARGEST @ _UCSN-FS-AT @ U< IF
            _UCSN-FS-ROOT @ _UCSN-FS-LARGEST @ _UCSN-FS-SWAP
            _UCSN-FS-LARGEST @ _UCSN-FS-ROOT !
        ELSE
            EXIT
        THEN
    REPEAT ;

: _UCSN-FS-SORT  ( address count -- )
    _UCSN-FS-COUNT ! _UCSN-FS-BASE !
    _UCSN-FS-COUNT @ 2 U< IF EXIT THEN
    _UCSN-FS-COUNT @ 2 / 1- _UCSN-FS-START !
    BEGIN _UCSN-FS-START @ 0>= WHILE
        _UCSN-FS-START @ _UCSN-FS-COUNT @ _UCSN-FS-SIFT
        -1 _UCSN-FS-START +!
    REPEAT
    _UCSN-FS-COUNT @ 1- _UCSN-FS-END !
    BEGIN _UCSN-FS-END @ 0> WHILE
        0 _UCSN-FS-END @ _UCSN-FS-SWAP
        0 _UCSN-FS-END @ _UCSN-FS-SIFT
        -1 _UCSN-FS-END +!
    REPEAT ;

: _UCSN-F-DESCRIPTOR-AT  ( ordinal -- descriptor )
    UCSN-DESCRIPTOR-SIZE * _UCSN-F-DESCRIPTORS-A @ + ;

: _UCSN-F-OFFSET-AT  ( ordinal -- address )
    8 * _UCSN-F-WORK-A @ + ;

: _UCSN-F-SET-CAPACITY  ( -- )
    _UCSN-F-STATUS @ UCSN-S-OK = IF
        UCSN-S-CAPACITY _UCSN-F-STATUS !
    THEN ;

: _UCSN-F-SET-INVALID  ( -- )
    UCSN-S-INVALID _UCSN-F-STATUS ! ;

: _UCSN-F-MAP-USCOL  ( status -- )
    DUP USCOL-S-OK = IF DROP EXIT THEN
    USCOL-S-CAPACITY = IF
        _UCSN-F-SET-CAPACITY
    ELSE
...
: _UCSN-F-RANGES?  ( -- flag )
    _UCSN-F-VALIDATION-A @ _UCSN-F-VALIDATION-U @
        _UCSN-OPTIONAL-SPAN? 0= IF 0 EXIT THEN
    _UCSN-F-VALIDATION-U @ IF
        _UCSN-F-VALIDATION-A @ _UCSN-ALIGNED? 0= IF 0 EXIT THEN
    THEN
    _UCSN-F-WORK-A @ _UCSN-F-WORK-U @
        _UCSN-OPTIONAL-SPAN? 0= IF 0 EXIT THEN
    _UCSN-F-WORK-U @ IF
        _UCSN-F-WORK-A @ _UCSN-ALIGNED? 0= IF 0 EXIT THEN
    THEN
    _UCSN-F-DESCRIPTORS-A @ _UCSN-F-DESCRIPTORS-U @
        _UCSN-OPTIONAL-SPAN? 0= IF 0 EXIT THEN
    _UCSN-F-DESCRIPTORS-U @ UCSN-DESCRIPTOR-SIZE MOD IF 0 EXIT THEN
    _UCSN-F-DESCRIPTORS-U @ IF
        _UCSN-F-DESCRIPTORS-A @ _UCSN-ALIGNED? 0= IF 0 EXIT THEN
    THEN
    _UCSN-F-NATIVE-A @ _UCSN-F-NATIVE-U @
        _UCSN-OPTIONAL-SPAN? 0= IF 0 EXIT THEN
    _UCSN-F-NATIVE-U @ 7 AND IF 0 EXIT THEN
    _UCSN-F-NATIVE-U @ IF
        _UCSN-F-NATIVE-A @ _UCSN-ALIGNED? 0= IF 0 EXIT THEN
    THEN
    _UCSN-F-DESCRIPTORS-U @ 0=
        _UCSN-F-NATIVE-U @ 0= <> IF 0 EXIT THEN

    _UCSN-F-VALIDATION-A @ _UCSN-F-VALIDATION-U @
        _UCSN-OWNED-DISJOINT? 0= IF 0 EXIT THEN
    _UCSN-F-WORK-A @ _UCSN-F-WORK-U @
        _UCSN-OWNED-DISJOINT? 0= IF 0 EXIT THEN
    _UCSN-F-DESCRIPTORS-A @ _UCSN-F-DESCRIPTORS-U @
        _UCSN-OWNED-DISJOINT? 0= IF 0 EXIT THEN
    ...
    _UCSN-F-VALIDATION-A @ _UCSN-F-VALIDATION-U @
        USCOL-STORAGE-DISJOINT? 0= IF 0 EXIT THEN
    _UCSN-F-WORK-A @ _UCSN-F-WORK-U @
        USCOL-STORAGE-DISJOINT? 0= IF 0 EXIT THEN
    _UCSN-F-DESCRIPTORS-A @ _UCSN-F-DESCRIPTORS-U @
        USCOL-STORAGE-DISJOINT? 0= IF 0 EXIT THEN
    ...
    _UCSN-F-WORK-A @ _UCSN-F-WORK-U @
        ...
        _UCSN-DISJOINT? 0= IF 0 EXIT THEN
    _UCSN-F-DESCRIPTORS-A @ _UCSN-F-DESCRIPTORS-U @
        _UCSN-F-NATIVE-A @ _UCSN-F-NATIVE-U @ _UCSN-DISJOINT? ;

: _UCSN-F-CLEAR-SCRATCH  ( -- )
    _UCSN-F-RANGES-VALID @ IF
        _UCSN-F-DIRTY-VALIDATION-U @ ?DUP IF
            _UCSN-F-VALIDATION-A @ SWAP 0 FILL
        THEN
        _UCSN-F-DIRTY-WORK-U @ ?DUP IF
            _UCSN-F-WORK-A @ SWAP 0 FILL
        THEN
    THEN
    _UCSN-F-SUMMARY USCOL-SUMMARY-SIZE 0 FILL ;

: _UCSN-F-GEOMETRY?  ( -- flag )
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-ROW@ DUP 0< IF
        DROP 0 EXIT
    THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-HEIGHT@ DUP 0> 0= IF
        2DROP 0 EXIT
    THEN
    _UCSN-IADD-NONNEG? 0= IF DROP 0 EXIT THEN
        _UCSN-F-ROOT-BOTTOM !

    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-COLUMN@ DUP 0< IF
        DROP 0 EXIT
    THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-WIDTH@ DUP 0> 0= IF
        2DROP 0 EXIT
    THEN
    _UCSN-IADD-NONNEG? 0= IF DROP 0 EXIT THEN
        _UCSN-F-ROOT-RIGHT !

    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-CLIP-ROW@ DUP 0< IF
        DROP 0 EXIT
    THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-CLIP-HEIGHT@ DUP 0> 0= IF
        2DROP 0 EXIT
    THEN
    _UCSN-IADD-NONNEG? 0= IF DROP 0 EXIT THEN
        _UCSN-F-CLIP-BOTTOM !

    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-CLIP-COLUMN@ DUP 0< IF
        DROP 0 EXIT
    THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-CLIP-WIDTH@ DUP 0> 0= IF
        2DROP 0 EXIT
    THEN
    _UCSN-IADD-NONNEG? 0= IF DROP 0 EXIT THEN
        _UCSN-F-CLIP-RIGHT !

    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-CLIP-ROW@
        _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-ROW@ < IF 0 EXIT THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-CLIP-COLUMN@
        _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-COLUMN@ < IF 0 EXIT THEN
    _UCSN-F-CLIP-BOTTOM @ _UCSN-F-ROOT-BOTTOM @ > IF 0 EXIT THEN
    _UCSN-F-CLIP-RIGHT @ _UCSN-F-ROOT-RIGHT @ > IF 0 EXIT THEN
    _UCSN-F-ENTRY @ USCOL-ROOT-HEIGHT@
        _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-HEIGHT@ =
    _UCSN-F-ENTRY @ USCOL-ROOT-WIDTH@
        _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-WIDTH@ = AND ;

: _UCSN-F-ORDER?  ( -- flag )
    _UCSN-F-HAVE-PRIOR @ IF
        _UCSN-F-INDEX @ _UCSN-F-PRIOR-INDEX @ U< IF 0 EXIT THEN
        _UCSN-F-INDEX @ _UCSN-F-PRIOR-INDEX @ = IF
            _UCSN-F-GENERATION @ _UCSN-F-PRIOR-GENERATION @ <> IF
                0 EXIT
            THEN
            _UCSN-F-PRIOR-ROOT @ _UCSN-F-ROOT-KEY @ U< 0= IF
                0 EXIT
            THEN
        THEN
    THEN
    _UCSN-F-INDEX @ _UCSN-F-PRIOR-INDEX !
    _UCSN-F-GENERATION @ _UCSN-F-PRIOR-GENERATION !
    _UCSN-F-ROOT-KEY @ _UCSN-F-PRIOR-ROOT !
    -1 _UCSN-F-HAVE-PRIOR !
    -1 ;

: _UCSN-F-VALIDATION-SPAN  ( -- work-a work-u )
    _UCSN-F-VALIDATION-NEED @ ?DUP IF
        _UCSN-F-VALIDATION-A @ SWAP
    ELSE
        0 0
    THEN ;

: _UCSN-F-ONE?  ( -- flag )
    _UCSN-F-I @ _UCSN-F-DESCRIPTOR-AT DUP _UCSN-F-DESCRIPTOR !
    UCSN-DESCRIPTOR-SOURCE@ UCSN-SOURCE-UIDL <> IF
        _UCSN-F-SET-INVALID 0 EXIT
    THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-SOURCE-INDEX@
        DUP _UCSN-F-INDEX ! 0< IF _UCSN-F-SET-INVALID 0 EXIT THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-SOURCE-GENERATION@
        DUP _UCSN-F-GENERATION ! -1 = IF
        _UCSN-F-SET-INVALID 0 EXIT
    THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-ROOT-KEY@
        DUP _UCSN-F-ROOT-KEY ! 0= IF _UCSN-F-SET-INVALID 0 EXIT THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-FAMILY@
        _UCSN-COLLECTION-FAMILY? 0= IF _UCSN-F-SET-INVALID 0 EXIT THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-Z@ 255 U> IF
        _UCSN-F-SET-INVALID 0 EXIT
    THEN
    _UCSN-F-ORDER? 0= IF _UCSN-F-SET-INVALID 0 EXIT THEN

    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-NATIVE-OFFSET@
        DUP _UCSN-F-NATIVE-O !
    DUP 0< SWAP 7 AND 0<> OR IF _UCSN-F-SET-INVALID 0 EXIT THEN
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-ENTRY-BYTES@
        DUP _UCSN-F-ENTRY-U !
    DUP 0> 0= SWAP 7 AND 0<> OR IF _UCSN-F-SET-INVALID 0 EXIT THEN
    _UCSN-F-NATIVE-O @ _UCSN-F-NATIVE-U @ U> IF
        _UCSN-F-SET-INVALID 0 EXIT
    THEN
    _UCSN-F-ENTRY-U @
        _UCSN-F-NATIVE-U @ _UCSN-F-NATIVE-O @ - U> IF
        _UCSN-F-SET-INVALID 0 EXIT
    THEN
    _UCSN-F-NATIVE-A @ _UCSN-F-NATIVE-O @ + _UCSN-F-ENTRY !

    _UCSN-F-ENTRY @ _UCSN-F-ENTRY-U @ USCOL-VALIDATION-WORK-BYTES
    DUP USCOL-S-OK <> IF
        >R DROP R> _UCSN-F-MAP-USCOL 0 EXIT
    THEN
    DROP DUP _UCSN-F-VALIDATION-NEED !
    _UCSN-F-VALIDATION-U @ U> IF _UCSN-F-SET-CAPACITY 0 EXIT THEN
    _UCSN-F-VALIDATION-NEED @ _UCSN-F-DIRTY-VALIDATION-U @ U> IF
        _UCSN-F-VALIDATION-NEED @ _UCSN-F-DIRTY-VALIDATION-U !
    THEN
    _UCSN-F-ENTRY @ _UCSN-F-ENTRY-U @
        _UCSN-F-VALIDATION-SPAN _UCSN-F-SUMMARY
        USCOL-ENTRY-VALIDATE
    DUP USCOL-S-OK <> IF _UCSN-F-MAP-USCOL 0 EXIT THEN DROP
    _UCSN-F-SUMMARY USCOL-SUMMARY-SIZE
    _UCSN-F-DESCRIPTOR @ UCSN-DESCRIPTOR-SUMMARY USCOL-SUMMARY-SIZE
        COMPARE IF _UCSN-F-SET-INVALID 0 EXIT THEN
    _UCSN-F-GEOMETRY? 0= IF _UCSN-F-SET-INVALID 0 EXIT THEN
    _UCSN-F-NATIVE-O @ _UCSN-F-I @ _UCSN-F-OFFSET-AT !
    -1 ;

: _UCSN-F-COVERAGE?  ( -- flag )
    _UCSN-F-WORK-A @ _UCSN-F-COUNT @ _UCSN-FS-SORT
    0 _UCSN-F-CURSOR ! 0 _UCSN-F-I !
    BEGIN _UCSN-F-I @ _UCSN-F-COUNT @ U< WHILE
        _UCSN-F-I @ _UCSN-F-OFFSET-AT @
            _UCSN-F-CURSOR @ <> IF 0 EXIT THEN
        _UCSN-F-NATIVE-A @ _UCSN-F-CURSOR @ + USCOL-ENTRY-BYTES@
            DUP 0> 0= IF DROP 0 EXIT THEN
        DUP 7 AND IF DROP 0 EXIT THEN
        _UCSN-F-CURSOR @ _UCSN-F-NATIVE-U @ U> IF DROP 0 EXIT THEN
        DUP _UCSN-F-NATIVE-U @ _UCSN-F-CURSOR @ - U> IF
            DROP 0 EXIT
        THEN
        _UCSN-F-CURSOR +!
        1 _UCSN-F-I +!
    REPEAT
    _UCSN-F-CURSOR @ _UCSN-F-NATIVE-U @ = ;

: _UCSN-F-BODY  ( -- status )
    UCSN-S-OK _UCSN-F-STATUS !
    0 _UCSN-F-RANGES-VALID !
    0 _UCSN-F-DIRTY-VALIDATION-U ! 0 _UCSN-F-DIRTY-WORK-U !
    _UCSN-F-SUMMARY USCOL-SUMMARY-SIZE 0 FILL
    _UCSN-F-RANGES? 0= IF UCSN-S-INVALID EXIT THEN
    -1 _UCSN-F-RANGES-VALID !
    _UCSN-F-DESCRIPTORS-U @ UCSN-DESCRIPTOR-SIZE /
        DUP _UCSN-F-COUNT !
    DUP 0= IF DROP UCSN-S-OK EXIT THEN
    UCSN-FROZEN-WORK-BYTES DUP 0= IF
        DROP UCSN-S-INVALID EXIT
    THEN
    DUP _UCSN-F-WORK-NEED !
    _UCSN-F-WORK-U @ U> IF UCSN-S-CAPACITY EXIT THEN
    _UCSN-F-WORK-NEED @ _UCSN-F-DIRTY-WORK-U !
    0 _UCSN-F-HAVE-PRIOR ! 0 _UCSN-F-I !
    BEGIN _UCSN-F-I @ _UCSN-F-COUNT @ U< WHILE
        _UCSN-F-ONE? 0= IF _UCSN-F-STATUS @ EXIT THEN
        1 _UCSN-F-I +!
    REPEAT
    _UCSN-F-COVERAGE? 0= IF UCSN-S-INVALID EXIT THEN
    UCSN-S-OK ;

: _UCSN-F-SCRUB  ( -- )
    0 _UCSN-F-VALIDATION-A ! 0 _UCSN-F-VALIDATION-U !
    0 _UCSN-F-WORK-A ! 0 _UCSN-F-WORK-U !
    0 _UCSN-F-DESCRIPTORS-A ! 0 _UCSN-F-DESCRIPTORS-U !
    0 _UCSN-F-NATIVE-A ! 0 _UCSN-F-NATIVE-U !
    0 _UCSN-F-RANGES-VALID ! 0 _UCSN-F-STATUS !
    0 _UCSN-F-COUNT ! 0 _UCSN-F-WORK-NEED !
    0 _UCSN-F-DIRTY-VALIDATION-U ! 0 _UCSN-F-DIRTY-WORK-U !
    0 _UCSN-F-I ! 0 _UCSN-F-DESCRIPTOR ! 0 _UCSN-F-ENTRY !
    0 _UCSN-F-ENTRY-U ! 0 _UCSN-F-NATIVE-O !
    0 _UCSN-F-VALIDATION-NEED ! 0 _UCSN-F-INDEX !
    0 _UCSN-F-GENERATION ! 0 _UCSN-F-ROOT-KEY !
    0 _UCSN-F-PRIOR-INDEX ! 0 _UCSN-F-PRIOR-GENERATION !
    0 _UCSN-F-PRIOR-ROOT ! 0 _UCSN-F-HAVE-PRIOR !
    0 _UCSN-F-ROOT-BOTTOM ! 0 _UCSN-F-ROOT-RIGHT !
    0 _UCSN-F-CLIP-BOTTOM ! 0 _UCSN-F-CLIP-RIGHT !
    0 _UCSN-F-CURSOR !
    0 _UCSN-FS-BASE ! 0 _UCSN-FS-COUNT ! 0 _UCSN-FS-ROOT !
    0 _UCSN-FS-CHILD ! 0 _UCSN-FS-LARGEST !
    0 _UCSN-FS-START ! 0 _UCSN-FS-END ! 0 _UCSN-FS-TEMP !
    _UCSN-F-SUMMARY USCOL-SUMMARY-SIZE 0 FILL ;
...
: UCSN-FROZEN-VALIDATE
    ...
    _UCSN-F-VALIDATION-U ! _UCSN-F-VALIDATION-A !
    0 _UCSN-F-RANGES-VALID !
    ['] _UCSN-F-BODY CATCH ?DUP IF
        DROP UCSN-S-INVALID
    THEN
    >R _UCSN-F-CLEAR-SCRATCH _UCSN-F-SCRUB R> ;
