...
\ canonical semantic widgets reached beneath those composites and associates
...
\ matches the canonical widget's (kind, process-lifetime instance) identity,
\ then assigns a root key within the current mounted-source generation.
\ Pointer fields below are UI-owner-private and are discarded before
\ detach/free.
...
48 CONSTANT _UTUI-MCR-O-KIND

40 CONSTANT _UTUI-MC-STAGE-SIZE
...
\ Zero is the reserved, never-activated generation and -1 is the exhausted
\ sentinel.  Every other cell value can identify one lifecycle epoch,
\ including values after signed wrap; attachment is proven separately and
\ ordering is never inferred from a generation value.
: _UTUI-MC-GENERATION-VALID?  ( generation -- flag )
    DUP 0<> SWAP -1 <> AND ;
...
: _UTUI-MCR-KIND@       ( relation -- value ) _UTUI-MCR-O-KIND + @ ;
...
: _UTUI-MCT-KIND@       ( stage -- value ) _UTUI-MCT-O-KIND + @ ;
...
: _UTUI-MC-KIND?  ( kind -- flag )
    ...
    SWAP _UTUI-MC-K-DATA-GRAPHICS = OR ;
...
VARIABLE _UTUI-MC-CAN-CURRENT
VARIABLE _UTUI-MC-CAN-SEEN
VARIABLE _UTUI-MC-CAN-PRIOR-SOURCE
VARIABLE _UTUI-MC-CAN-PRIOR-KEY
VARIABLE _UTUI-MC-CAN-HAVE-PRIOR

\ The live relation chain is the canonical private index used by upper
\ semantic snapshots.  Keep its exact count and strict unsigned
\ (source-index, root-key) order as one invariant instead of sorting borrowed
\ widget pointers in the collector.
: _UTUI-MC-RELATIONS-CANONICAL?  ( -- flag )
    _UTUI-MC-HEAD @ _UTUI-MC-COUNT @ _UTUI-MC-REL-SIZE
        _UTUI-MC-CHAIN-WELL-FORMED? 0= IF 0 EXIT THEN
    _UTUI-MC-HEAD @ _UTUI-MC-CAN-CURRENT !
    0 _UTUI-MC-CAN-SEEN ! 0 _UTUI-MC-CAN-HAVE-PRIOR !
    BEGIN _UTUI-MC-CAN-CURRENT @ DUP WHILE
        1 _UTUI-MC-CAN-SEEN +!
        DUP _UTUI-MCR-SOURCE@ DUP 0< IF 2DROP 0 EXIT THEN
        _UTUI-MAX-ELEMS U< 0= IF DROP 0 EXIT THEN
        DUP _UTUI-MCR-GENERATION@ _UTUI-MC-GENERATION-VALID?
            0= IF DROP 0 EXIT THEN
        DUP _UTUI-MCR-KIND@ _UTUI-MC-KIND?
            0= IF DROP 0 EXIT THEN
        ...
        _UTUI-MC-CAN-HAVE-PRIOR @ IF
            ...
            OVER _UTUI-MCR-SOURCE@ _UTUI-MC-CAN-PRIOR-SOURCE @ = IF
                _UTUI-MC-CAN-PRIOR-KEY @ OVER U< 0= IF
                    2DROP 0 EXIT
                THEN
            THEN
        THEN
        SWAP _UTUI-MCR-SOURCE@ _UTUI-MC-CAN-PRIOR-SOURCE !
        _UTUI-MC-CAN-PRIOR-KEY !
        -1 _UTUI-MC-CAN-HAVE-PRIOR !
        _UTUI-MC-CAN-CURRENT @ _UTUI-MCR-NEXT@
            _UTUI-MC-CAN-CURRENT !
    REPEAT DROP
...
: _UTUI-MC-WIDGET-KIND@  ( widget -- kind|0 )
    ...
    DUP _WDG-HDR-SIZE MSPAN-NONWRAPPING? 0= IF DROP 0 EXIT THEN
    DUP _WDG-O-TYPE + @
    DUP WDG-T-TEXTAREA = IF
        DROP _UTUI-MC-GENUINE-TEXTAREA?
        IF _UTUI-MC-K-TEXT-AREA ELSE 0 THEN EXIT
    THEN
    DUP WDG-T-TEXTGRID = IF
        DROP _UTUI-MC-GENUINE-TEXTGRID?
        IF _UTUI-MC-K-TEXT-GRID ELSE 0 THEN EXIT
    THEN
    DUP WDG-T-TABS = IF
    ...
    WDG-T-DATA-GRAPHICS = IF
        ...
        IF _UTUI-MC-K-DATA-GRAPHICS ELSE 0 THEN EXIT
    THEN
    DROP 0 ;

: _UTUI-MC-GENUINE-SEMANTIC?  ( widget -- flag )
    _UTUI-MC-WIDGET-KIND@ 0<> ;

: _UTUI-MC-WIDGET-INSTANCE@  ( widget -- instance|0 )
    DUP _UTUI-MC-WIDGET-KIND@
    DUP _UTUI-MC-K-DATA-GRAPHICS = IF
        DROP DGRAPH-INSTANCE@ EXIT
    THEN
    _UTUI-MC-COLLECTION-KIND? IF
        _UTUI-MC-COLLECTION-INSTANCE@
    ELSE
        DROP 0
    THEN ;
                ...
                _UTUI-MC-A-WIDGET @ _WDG-HDR-SIZE
                    ...
                    _UTUI-MC-A-WIDGET @ _WDG-O-REGION + @
                        ...
                        I _UTUI-MC-SOURCE _UTUI-MCS-GENERATION@
                            DUP _UTUI-MC-GENERATION-VALID? 0= IF
...
: _UTUI-MC-ASSOCIATE  ( widget -- status )
    DUP _UTUI-MC-GENUINE-SEMANTIC? 0= IF
...
VARIABLE _UTUI-MC-V-REL
VARIABLE _UTUI-MC-V-WIDGET
VARIABLE _UTUI-MC-V-INSTANCE
VARIABLE _UTUI-MC-V-KIND
VARIABLE _UTUI-MC-V-SOURCE
VARIABLE _UTUI-MC-V-GENERATION
VARIABLE _UTUI-MC-V-ROOT-KEY
VARIABLE _UTUI-MC-V-ELEM
VARIABLE _UTUI-MC-V-SC
VARIABLE _UTUI-MC-V-ROOT-WIDGET
VARIABLE _UTUI-MC-V-SOURCE-STATE
VARIABLE _UTUI-MC-V-NEXT-KEY
VARIABLE _UTUI-MC-V-OUTER
VARIABLE _UTUI-MC-V-INNER
VARIABLE _UTUI-MC-V-SEEN

\ Revalidate one borrowed relation against all current UIDL, widget, source
\ generation, instance-token, and region-ancestry authority.  The relation
\ ledger is an observation cache, never an authority that can bless a stale
\ pointer by itself.
: _UTUI-MC-RELATION-VALID?  ( relation -- flag )
    DUP _UTUI-MC-V-REL !
    DUP 7 AND IF DROP 0 EXIT THEN
    DUP _UTUI-MC-REL-SIZE MSPAN-NONWRAPPING? 0= IF DROP 0 EXIT THEN
    DUP _UTUI-MCR-WIDGET@ _UTUI-MC-V-WIDGET !
    DUP _UTUI-MCR-INSTANCE@ _UTUI-MC-V-INSTANCE !
    DUP _UTUI-MCR-KIND@ _UTUI-MC-V-KIND !
    DUP _UTUI-MCR-SOURCE@ _UTUI-MC-V-SOURCE !
    DUP _UTUI-MCR-GENERATION@ _UTUI-MC-V-GENERATION !
    _UTUI-MCR-ROOT-KEY@ _UTUI-MC-V-ROOT-KEY !

    _UTUI-MC-V-SOURCE @ DUP 0< IF DROP 0 EXIT THEN
    UIDL-ELEM-COUNT DUP 0> 0= IF 2DROP 0 EXIT THEN
    DUP _UTUI-MAX-ELEMS U> IF 2DROP 0 EXIT THEN
    U< 0= IF 0 EXIT THEN
    _UTUI-MC-V-SOURCE @ _UDL-ELEMSZ * _UDL-ELEMS +
        DUP _UTUI-MC-V-ELEM !
    UE.TYPE @ 0= IF 0 EXIT THEN
    _UTUI-MC-V-ELEM @ _UTUI-SIDECAR DUP _UTUI-MC-V-SC !
    DUP _UTUI-SC-FLAGS@ _UTUI-SCF-HAS AND 0= IF DROP 0 EXIT THEN
    DUP _UTUI-SC-WOWNER@ _UTUI-WOWNER-CALLER <> IF DROP 0 EXIT THEN
    _UTUI-SC-WPTR@ DUP _UTUI-MC-V-ROOT-WIDGET !
    DUP 0= IF DROP 0 EXIT THEN
    DUP 7 AND IF DROP 0 EXIT THEN
    DUP _WDG-HDR-SIZE MSPAN-NONWRAPPING? 0= IF DROP 0 EXIT THEN
    _WDG-O-REGION + @ DUP _UTUI-MC-RGN-VALID? 0= IF DROP 0 EXIT THEN
    DROP

    _UTUI-MC-V-WIDGET @ DUP _UTUI-MC-GENUINE-SEMANTIC? 0= IF
        DROP 0 EXIT
    THEN
    DUP _UTUI-MC-WIDGET-KIND@
        _UTUI-MC-V-KIND @ <> IF DROP 0 EXIT THEN
    DUP _UTUI-MC-WIDGET-INSTANCE@
        _UTUI-MC-V-INSTANCE @ <> IF DROP 0 EXIT THEN
    DROP
    _UTUI-MC-V-KIND @ _UTUI-MC-KIND? 0= IF 0 EXIT THEN
    _UTUI-MC-V-INSTANCE @ 0= IF 0 EXIT THEN
    _UTUI-MC-V-GENERATION @ _UTUI-MC-GENERATION-VALID? 0= IF
        0 EXIT
    THEN
    _UTUI-MC-V-ROOT-KEY @ 0= IF 0 EXIT THEN

    _UTUI-MC-V-SOURCE @ _UTUI-MC-SOURCE DUP
        _UTUI-MC-V-SOURCE-STATE !
    DUP _UTUI-MCS-GENERATION@ _UTUI-MC-V-GENERATION @ <> IF
        DROP 0 EXIT
    THEN
    _UTUI-MCS-NEXT-KEY@ DUP _UTUI-MC-V-NEXT-KEY !
    DUP 0= IF DROP 0 EXIT THEN
    _UTUI-MC-V-ROOT-KEY @ SWAP U< 0= IF 0 EXIT THEN

    _UTUI-MC-V-WIDGET @ _UTUI-MC-ASSOCIATE
        _UTUI-MC-S-OK <> IF 0 EXIT THEN
    _UTUI-MC-A-SOURCE @ _UTUI-MC-V-SOURCE @ <> IF 0 EXIT THEN
    _UTUI-MC-A-GENERATION @ _UTUI-MC-V-GENERATION @ <> IF 0 EXIT THEN
    -1 ;

\ Heap records are allocator-owned but the chain itself is mutable state.
\ Pairwise rejection prevents a corrupted interior pointer or duplicated
\ widget/token identity from being accepted merely because NEXT reaches zero.
: _UTUI-MC-RELATION-DISTINCT?  ( relation -- flag )
    DUP _UTUI-MC-V-OUTER ! _UTUI-MCR-NEXT@ _UTUI-MC-V-INNER !
    BEGIN _UTUI-MC-V-INNER @ DUP WHILE
        DUP _UTUI-MC-REL-SIZE MSPAN-NONWRAPPING? 0= IF DROP 0 EXIT THEN
        _UTUI-MC-V-OUTER @ _UTUI-MC-REL-SIZE
        2 PICK _UTUI-MC-REL-SIZE MSPAN-OVERLAP? IF DROP 0 EXIT THEN
        DUP _UTUI-MCR-WIDGET@
            _UTUI-MC-V-OUTER @ _UTUI-MCR-WIDGET@ = IF DROP 0 EXIT THEN
        DUP _UTUI-MCR-KIND@
            _UTUI-MC-V-OUTER @ _UTUI-MCR-KIND@ = IF
            DUP _UTUI-MCR-INSTANCE@
                _UTUI-MC-V-OUTER @ _UTUI-MCR-INSTANCE@ = IF
                DROP 0 EXIT
            THEN
        THEN
        _UTUI-MCR-NEXT@ _UTUI-MC-V-INNER !
    REPEAT DROP -1 ;
...
: _UTUI-MC-STAGE-ADD  ( widget -- status )
    ...
    DUP _UTUI-MC-WIDGET-KIND@ DUP 0= IF
        2DROP _UTUI-MC-S-INVALID EXIT
    THEN
    _UTUI-MC-SA-KIND !
    _UTUI-MC-WIDGET-INSTANCE@ DUP 0= IF
        DROP _UTUI-MC-S-INVALID EXIT
    THEN
    ...
    BEGIN _UTUI-MC-SA-CURRENT @ DUP WHILE
        ...
        THEN
        1 _UTUI-MC-SA-SEEN +!
        DUP _UTUI-MCT-KIND@ _UTUI-MC-SA-KIND @ =
        OVER _UTUI-MCT-INSTANCE@ _UTUI-MC-SA-INSTANCE @ = AND IF
    ...
    _UTUI-MC-SA-KIND @ _UTUI-MC-SA-NEW @ _UTUI-MCT-O-KIND + !
...
: _UTUI-MC-FIND-RELATION  ( -- status )
    ...
    _UTUI-MC-RELATIONS-CANONICAL? 0= IF
        _UTUI-MC-S-INVALID EXIT
    THEN
    ...
    BEGIN _UTUI-MC-F-CURRENT @ DUP WHILE
        ...
        _UTUI-MC-F-SEEN @ _UTUI-MC-COUNT @ U> IF
            DROP _UTUI-MC-S-INVALID EXIT
        THEN
        DUP _UTUI-MCR-KIND@ _UTUI-MC-F-KIND @ =
        OVER _UTUI-MCR-INSTANCE@ _UTUI-MC-F-INSTANCE @ = AND IF
...
: _UTUI-MC-PREPARE-STAGE  ( stage -- status )
    ...
    DUP _UTUI-MCT-KIND@ _UTUI-MC-F-KIND !
    ...
    _UTUI-MC-P-STAGE @ _UTUI-MCT-KIND@
        _UTUI-MC-P-REL @ _UTUI-MCR-O-KIND + !
...
VARIABLE _UTUI-MC-INS-REL
VARIABLE _UTUI-MC-INS-LINK
VARIABLE _UTUI-MC-INS-CURRENT

\ Insert a fully prepared relation into the already validated live chain.
\ Allocation, duplicate identity checks, and key assignment happen before
\ this non-failing link operation.
: _UTUI-MC-REL-INSERT-CANONICAL  ( relation -- )
    _UTUI-MC-INS-REL ! _UTUI-MC-HEAD _UTUI-MC-INS-LINK !
    BEGIN _UTUI-MC-INS-LINK @ @ DUP WHILE
        _UTUI-MC-INS-CURRENT !
        _UTUI-MC-INS-REL @ _UTUI-MCR-SOURCE@
        _UTUI-MC-INS-CURRENT @ _UTUI-MCR-SOURCE@
        2DUP = IF
            2DROP
            _UTUI-MC-INS-REL @ _UTUI-MCR-ROOT-KEY@
            _UTUI-MC-INS-CURRENT @ _UTUI-MCR-ROOT-KEY@ U< IF
                _UTUI-MC-INS-CURRENT @
                    _UTUI-MC-INS-REL @ _UTUI-MCR-NEXT!
                _UTUI-MC-INS-REL @ _UTUI-MC-INS-LINK @ !
                1 _UTUI-MC-COUNT +! EXIT
            THEN
        ELSE
            U< IF
                _UTUI-MC-INS-CURRENT @
            ...
            THEN
        THEN
        _UTUI-MC-INS-CURRENT @ _UTUI-MCR-O-NEXT +
            _UTUI-MC-INS-LINK !
    REPEAT DROP
    0 _UTUI-MC-INS-REL @ _UTUI-MCR-NEXT!
    _UTUI-MC-INS-REL @ _UTUI-MC-INS-LINK @ !
    1 _UTUI-MC-COUNT +! ;
...
: _UTUI-MC-UPSERT  ( widget -- status )
    ...
    _UTUI-MC-U-KIND @ _UTUI-MC-F-KIND !
    ...
    _UTUI-MC-U-KIND @ _UTUI-MC-U-REL @ _UTUI-MCR-O-KIND + !
    ...
    _UTUI-MC-U-REL @ _UTUI-MC-REL-INSERT-CANONICAL
...
: _UTUI-MC-OBS-CANONICAL  ( widget -- )
    DUP _UTUI-MC-GENUINE-SEMANTIC? 0= IF DROP EXIT THEN
...
\ Clear every transient relation/widget pointer once an outer observation or
\ validation query has finished.  The live relation chain and source states
\ remain authoritative; these fields are only scratch and must not retain a
\ borrowed pointer across a UCTX switch.
: _UTUI-MC-SCRATCH-CLEAR  ( -- )
    0 _UTUI-MC-CH-HEAD ! 0 _UTUI-MC-CH-COUNT !
    0 _UTUI-MC-CH-SIZE ! 0 _UTUI-MC-CH-CURRENT !
    0 _UTUI-MC-CH-SEEN !
    0 _UTUI-MC-CAN-CURRENT ! 0 _UTUI-MC-CAN-SEEN !
    0 _UTUI-MC-CAN-PRIOR-SOURCE ! 0 _UTUI-MC-CAN-PRIOR-KEY !
    0 _UTUI-MC-CAN-HAVE-PRIOR !
    0 _UTUI-MC-RCS-SOURCE ! 0 _UTUI-MC-RCS-LINK !
    0 _UTUI-MC-RCS-NODE ! 0 _UTUI-MC-RCS-NEXT !
    0 _UTUI-MC-ACY-SLOW ! 0 _UTUI-MC-ACY-FAST !
    0 _UTUI-MC-A-SOURCE ! 0 _UTUI-MC-A-GENERATION !
    0 _UTUI-MC-A-FOUND ! 0 _UTUI-MC-A-SEEN-DOC !
    ...
    0 _UTUI-MC-V-SOURCE !
    0 _UTUI-MC-V-GENERATION ! 0 _UTUI-MC-V-ROOT-KEY !
    0 _UTUI-MC-V-ELEM ! 0 _UTUI-MC-V-SC !
    0 _UTUI-MC-V-ROOT-WIDGET ! 0 _UTUI-MC-V-SOURCE-STATE !
    0 _UTUI-MC-V-NEXT-KEY ! 0 _UTUI-MC-V-OUTER !
    0 _UTUI-MC-V-INNER ! 0 _UTUI-MC-V-SEEN !
    0 _UTUI-MC-R-WIDGET ! 0 _UTUI-MC-R-SOURCE !
    0 _UTUI-MC-R-GENERATION ! 0 _UTUI-MC-R-FOUND !
    0 _UTUI-MC-SA-WIDGET ! 0 _UTUI-MC-SA-INSTANCE !
    0 _UTUI-MC-SA-KIND !
    0 _UTUI-MC-SA-CURRENT ! 0 _UTUI-MC-SA-SEEN !
    0 _UTUI-MC-SA-NEW !
    0 _UTUI-MC-F-SOURCE ! 0 _UTUI-MC-F-GENERATION !
    0 _UTUI-MC-F-WIDGET ! 0 _UTUI-MC-F-INSTANCE !
    0 _UTUI-MC-F-KIND !
    0 _UTUI-MC-F-CURRENT ! 0 _UTUI-MC-F-SEEN !
    0 _UTUI-MC-F-FOUND ! 0 _UTUI-MC-F-ALLOW-REPLACE !
    0 _UTUI-MC-P-STAGE ! 0 _UTUI-MC-P-REL !
    0 _UTUI-MC-P-KEY ! 0 _UTUI-MC-P-NEXT-KEY !
    0 _UTUI-MC-HR-REL ! 0 _UTUI-MC-HR-STAGE !
    0 _UTUI-MC-HR-SEEN !
    0 _UTUI-MC-C-CURRENT ! 0 _UTUI-MC-C-SEEN !
    0 _UTUI-MC-C-LINK ! 0 _UTUI-MC-C-NEXT !
    0 _UTUI-MC-C-LAST !
    0 _UTUI-MC-INS-REL ! 0 _UTUI-MC-INS-LINK !
    0 _UTUI-MC-INS-CURRENT !
    0 _UTUI-MC-U-WIDGET ! 0 _UTUI-MC-U-INSTANCE !
    0 _UTUI-MC-U-KIND !
    0 _UTUI-MC-U-KEY ! 0 _UTUI-MC-U-NEXT-KEY !
    0 _UTUI-MC-U-REL ! ;
...
: UTUI-DRAW-OBSERVE  ( body-xt -- status )
    ...
    _UTUI-MC-OBS-IOR @ ?DUP IF
        _UTUI-MC-STAGE-CLEAR _UTUI-MC-SCRATCH-CLEAR THROW
    THEN
    ...
    _UTUI-MC-STAGE-ACTIVE @ IF
        _UTUI-MC-STAGE-CLEAR _UTUI-MC-SET-INVALID
    THEN
    _UTUI-MC-STATUS @ _UTUI-MC-SCRATCH-CLEAR ;
...
: _UTUI-MC-RELATIONS-READY?  ( -- flag )
    _UTUI-MC-STATUS @ _UTUI-MC-S-OK <> IF 0 EXIT THEN
    _UCTX-STAGE-QUIESCENT? 0= IF 0 EXIT THEN
    _UTUI-MC-RELATIONS-CANONICAL? 0= IF 0 EXIT THEN
    _UTUI-MC-HEAD @ _UTUI-MC-V-REL ! 0 _UTUI-MC-V-SEEN !
    BEGIN _UTUI-MC-V-REL @ DUP WHILE
        DUP _UTUI-MC-RELATION-VALID? 0= IF DROP 0 EXIT THEN
        DUP _UTUI-MC-RELATION-DISTINCT? 0= IF DROP 0 EXIT THEN
        _UTUI-MCR-NEXT@ _UTUI-MC-V-REL !
        1 _UTUI-MC-V-SEEN +!
        _UTUI-MC-V-SEEN @ _UTUI-MC-COUNT @ U> IF 0 EXIT THEN
    REPEAT DROP
    _UTUI-MC-V-SEEN @ _UTUI-MC-COUNT @ = ;
...
: UCTX-FREE  ( ctx -- )
    ...
    _UCTX-STAGE-QUIESCENT? 0=
        ABORT" UCTX-FREE during draw observation"
...
: UCTX-CLEAR  ( ctx -- )
    ...
    _UCTX-STAGE-QUIESCENT? 0=
        ABORT" UCTX-CLEAR during draw observation"
    DUP _UCTX-FREE-SAVED-RELATIONS
    _UTUI-MC-SCRATCH-CLEAR
    UCTX-TOTAL 0 FILL ;
...
\ and authored-UIDL semantic projection. The current root-region descriptor
\ is borrowed by UIDL-TUI, so it is protected explicitly in addition to
\ neutral UIDL, validation scratch, and state-tree storage.
: _UTUI-STORAGE-DISJOINT-BODY?  ( address length -- flag )
    ...
    ELSE
        DROP
    THEN
    _UCTX-LIVE-OWNER @ DUP IF
        DUP UCTX-TOTAL MSPAN-NONWRAPPING? 0= IF 3DROP 0 EXIT THEN
        >R
        2DUP R> UCTX-TOTAL MSPAN-OVERLAP? IF 2DROP 0 EXIT THEN
    ELSE
        DROP
    THEN
...
\ The tree visitor lends this record to semantic observers, whose public
\ resolved-record ABI requires cell alignment.
CREATE _UTUI-RST-RESOLVED-MEM UTUI-RESOLVED-SIZE 7 + ALLOT
_UTUI-RST-RESOLVED-MEM 7 + -8 AND CONSTANT _UTUI-RST-RESOLVED
...
VARIABLE _UTUI-CG-RGN
VARIABLE _UTUI-CG-SOURCE-A
VARIABLE _UTUI-CG-SOURCE-U
VARIABLE _UTUI-CG-CURRENT
VARIABLE _UTUI-CG-SAW-DOC
VARIABLE _UTUI-CG-ORIGIN-ROW
VARIABLE _UTUI-CG-ORIGIN-COL
VARIABLE _UTUI-CG-EXTENT-H
VARIABLE _UTUI-CG-EXTENT-W
VARIABLE _UTUI-CG-CLIP-TOP
VARIABLE _UTUI-CG-CLIP-LEFT
VARIABLE _UTUI-CG-CLIP-BOTTOM
VARIABLE _UTUI-CG-CLIP-RIGHT
VARIABLE _UTUI-CG-I-ROW
VARIABLE _UTUI-CG-I-COL
VARIABLE _UTUI-CG-I-H
VARIABLE _UTUI-CG-I-W
VARIABLE _UTUI-CG-I-BOTTOM
VARIABLE _UTUI-CG-I-RIGHT

: _UTUI-CG-CLEAR  ( -- )
    0 _UTUI-CG-RGN ! 0 _UTUI-CG-SOURCE-A ! 0 _UTUI-CG-SOURCE-U !
    0 _UTUI-CG-CURRENT ! 0 _UTUI-CG-SAW-DOC !
    0 _UTUI-CG-ORIGIN-ROW ! 0 _UTUI-CG-ORIGIN-COL !
    0 _UTUI-CG-EXTENT-H ! 0 _UTUI-CG-EXTENT-W !
    0 _UTUI-CG-CLIP-TOP ! 0 _UTUI-CG-CLIP-LEFT !
    0 _UTUI-CG-CLIP-BOTTOM ! 0 _UTUI-CG-CLIP-RIGHT !
    0 _UTUI-CG-I-ROW ! 0 _UTUI-CG-I-COL !
    0 _UTUI-CG-I-H ! 0 _UTUI-CG-I-W !
    0 _UTUI-CG-I-BOTTOM ! 0 _UTUI-CG-I-RIGHT ! ;

: _UTUI-CG-FAIL  ( -- 0 0 0 0 0 0 0 0 status )
    _UTUI-CG-CLEAR 0 0 0 0 0 0 0 0 USCOL-S-INVALID ;

: _UTUI-CG-INTERSECT?  ( row col h w -- flag )
    _UTUI-CG-I-W ! _UTUI-CG-I-H !
    _UTUI-CG-I-COL ! _UTUI-CG-I-ROW !
    _UTUI-CG-I-ROW @ _UTUI-CG-I-H @ _UTUI-RS-AXIS-END?
        0= IF DROP 0 EXIT THEN _UTUI-CG-I-BOTTOM !
    _UTUI-CG-I-COL @ _UTUI-CG-I-W @ _UTUI-RS-AXIS-END?
        0= IF DROP 0 EXIT THEN _UTUI-CG-I-RIGHT !
    _UTUI-CG-CLIP-TOP @ _UTUI-CG-I-ROW @ MAX
        _UTUI-CG-CLIP-TOP !
    _UTUI-CG-CLIP-LEFT @ _UTUI-CG-I-COL @ MAX
        _UTUI-CG-CLIP-LEFT !
    _UTUI-CG-CLIP-BOTTOM @ _UTUI-CG-I-BOTTOM @ MIN
        _UTUI-CG-CLIP-BOTTOM !
    _UTUI-CG-CLIP-RIGHT @ _UTUI-CG-I-RIGHT @ MIN
        _UTUI-CG-CLIP-RIGHT !
    -1 ;

\ Return the complete canonical widget origin/extent separately from its
\ exact region-ancestry clip.  The source record contributes semantic UIDL
\ clipping; no physical screen size, renderer allocation, or reflow enters
\ this renderer-neutral geometry.
: _UTUI-CANONICAL-REGION-GEOMETRY
    ( region source-resolved-a source-resolved-u -- origin-row origin-col extent-h extent-w clip-row clip-col clip-h clip-w status )
    _UTUI-CG-SOURCE-U ! _UTUI-CG-SOURCE-A ! _UTUI-CG-RGN !
    _UTUI-CG-SOURCE-A @ _UTUI-CG-SOURCE-U @
        UTUI-RESOLVED-VALID? 0= IF _UTUI-CG-FAIL EXIT THEN
    _UTUI-CG-RGN @ DUP _UTUI-MC-RGN-ACYCLIC? 0= IF
        DROP _UTUI-CG-FAIL EXIT
    THEN
    DUP RGN-ROW _UTUI-CG-ORIGIN-ROW !
    DUP RGN-COL _UTUI-CG-ORIGIN-COL !
    DUP RGN-H DUP 0< IF 2DROP _UTUI-CG-FAIL EXIT THEN
        _UTUI-CG-EXTENT-H !
    RGN-W DUP 0< IF DROP _UTUI-CG-FAIL EXIT THEN
        _UTUI-CG-EXTENT-W !
    _UTUI-CG-ORIGIN-ROW @ _UTUI-CG-EXTENT-H @ _UTUI-RS-AXIS-END?
        0= IF DROP _UTUI-CG-FAIL EXIT THEN _UTUI-CG-CLIP-BOTTOM !
    _UTUI-CG-ORIGIN-COL @ _UTUI-CG-EXTENT-W @ _UTUI-RS-AXIS-END?
        0= IF DROP _UTUI-CG-FAIL EXIT THEN _UTUI-CG-CLIP-RIGHT !
    _UTUI-CG-ORIGIN-ROW @ _UTUI-CG-CLIP-TOP !
    _UTUI-CG-ORIGIN-COL @ _UTUI-CG-CLIP-LEFT !

    _UTUI-CG-RGN @ _UTUI-CG-CURRENT ! 0 _UTUI-CG-SAW-DOC !
    BEGIN _UTUI-CG-CURRENT @ DUP WHILE
        DUP _UTUI-MC-RGN-VALID? 0= IF DROP _UTUI-CG-FAIL EXIT THEN
        DUP _UTUI-RGN @ = IF -1 _UTUI-CG-SAW-DOC ! THEN
        DROP
        _UTUI-CG-CURRENT @ RGN-ROW
        _UTUI-CG-CURRENT @ RGN-COL
        _UTUI-CG-CURRENT @ RGN-H
        _UTUI-CG-CURRENT @ RGN-W
            _UTUI-CG-INTERSECT? 0= IF _UTUI-CG-FAIL EXIT THEN
        _UTUI-CG-CURRENT @ _RGN-O-PARENT + @ _UTUI-CG-CURRENT !
    REPEAT DROP
    _UTUI-CG-SAW-DOC @ 0= IF _UTUI-CG-FAIL EXIT THEN

    _UTUI-CG-SOURCE-A @ @
    _UTUI-CG-SOURCE-A @ 8 + @
    _UTUI-CG-SOURCE-A @ 16 + @
    _UTUI-CG-SOURCE-A @ 24 + @
        _UTUI-CG-INTERSECT? 0= IF _UTUI-CG-FAIL EXIT THEN

    _UTUI-CG-ORIGIN-ROW @ _UTUI-CG-ORIGIN-COL @
    _UTUI-CG-EXTENT-H @ _UTUI-CG-EXTENT-W @
    _UTUI-CG-CLIP-TOP @ _UTUI-CG-CLIP-LEFT @
    _UTUI-CG-CLIP-BOTTOM @ _UTUI-CG-CLIP-TOP @
        2DUP > IF - ELSE 2DROP 0 THEN
    _UTUI-CG-CLIP-RIGHT @ _UTUI-CG-CLIP-LEFT @
        2DUP > IF - ELSE 2DROP 0 THEN
    USCOL-S-OK _UTUI-CG-CLEAR ;

VARIABLE _UTUI-MI-VISITOR
VARIABLE _UTUI-MI-ACTIVE
VARIABLE _UTUI-MI-KIND
VARIABLE _UTUI-MI-CURRENT
VARIABLE _UTUI-MI-WIDGET
VARIABLE _UTUI-MI-SOURCE
VARIABLE _UTUI-MI-GENERATION
VARIABLE _UTUI-MI-ROOT-KEY
VARIABLE _UTUI-MI-SEEN
VARIABLE _UTUI-MI-RESOLVE-STATUS
VARIABLE _UTUI-MI-DST
VARIABLE _UTUI-MI-CAP
VARIABLE _UTUI-MI-BUILDER
CREATE _UTUI-MI-RESOLVED-MEM UTUI-RESOLVED-SIZE 7 + ALLOT

: _UTUI-MI-RESOLVED  ( -- record )
    _UTUI-MI-RESOLVED-MEM 7 + -8 AND ;

: _UTUI-MI-CLEAR  ( -- )
    0 _UTUI-MI-VISITOR ! 0 _UTUI-MI-ACTIVE !
    0 _UTUI-MI-KIND !
    0 _UTUI-MI-CURRENT ! 0 _UTUI-MI-WIDGET !
    0 _UTUI-MI-SOURCE ! 0 _UTUI-MI-GENERATION !
    0 _UTUI-MI-ROOT-KEY ! 0 _UTUI-MI-SEEN !
    0 _UTUI-MI-RESOLVE-STATUS !
    0 _UTUI-MI-DST ! 0 _UTUI-MI-CAP ! 0 _UTUI-MI-BUILDER !
    _UTUI-MI-RESOLVED UTUI-RESOLVED-SIZE 0 FILL
    _UTUI-MC-SCRATCH-CLEAR ;

: _UTUI-MI-SET-CURRENT  ( relation -- )
    DUP _UTUI-MI-CURRENT !
    DUP _UTUI-MCR-WIDGET@ _UTUI-MI-WIDGET !
    DUP _UTUI-MCR-SOURCE@ _UTUI-MI-SOURCE !
    DUP _UTUI-MCR-GENERATION@ _UTUI-MI-GENERATION !
    _UTUI-MCR-ROOT-KEY@ _UTUI-MI-ROOT-KEY ! ;

: _UTUI-MI-CURRENT-SELECTED?  ( -- flag )
    _UTUI-MI-CURRENT @ DUP 0= IF DROP 0 EXIT THEN
    _UTUI-MCR-KIND@
    _UTUI-MI-KIND @ DUP 0= IF
        DROP _UTUI-MC-COLLECTION-KIND?
    ELSE
        =
    THEN ;

: _UTUI-MI-CURRENT-VALID?  ( -- flag )
    _UTUI-MI-ACTIVE @ 0= IF 0 EXIT THEN
    _UTUI-MI-CURRENT @ DUP 0= IF DROP 0 EXIT THEN
    DUP _UTUI-MC-RELATION-VALID? 0= IF DROP 0 EXIT THEN
    DUP _UTUI-MCR-WIDGET@ _UTUI-MI-WIDGET @ =
    OVER _UTUI-MCR-SOURCE@ _UTUI-MI-SOURCE @ = AND
    OVER _UTUI-MCR-GENERATION@ _UTUI-MI-GENERATION @ = AND
    SWAP _UTUI-MCR-ROOT-KEY@ _UTUI-MI-ROOT-KEY @ = AND
    _UTUI-MI-CURRENT-SELECTED? AND ;

: _UTUI-MI-SOURCE-GEOMETRY?  ( -- flag )
    _UTUI-MI-SOURCE @ _UDL-ELEMSZ * _UDL-ELEMS + _UTUI-SIDECAR
    _UTUI-SC-WPTR@ DUP 0= IF DROP 0 EXIT THEN
    _WDG-O-REGION + @ DUP _UTUI-MC-RGN-VALID? 0= IF DROP 0 EXIT THEN
    DUP RGN-ROW _UTUI-MI-RESOLVED @ =
    OVER RGN-COL _UTUI-MI-RESOLVED 8 + @ = AND
    OVER RGN-H _UTUI-MI-RESOLVED 16 + @ = AND
    SWAP RGN-W _UTUI-MI-RESOLVED 24 + @ = AND ;

: _UTUI-MI-VISIT  ( -- )
    _UTUI-MI-SOURCE @ _UTUI-MI-GENERATION @ _UTUI-MI-ROOT-KEY @
    _UTUI-MI-RESOLVED UTUI-RESOLVED-SIZE _UTUI-MI-VISITOR @ EXECUTE ;

: _UTUI-MI-BODY  ( -- status )
    _UTUI-MI-VISITOR @ 0= IF _UTUI-MC-S-INVALID EXIT THEN
    _UTUI-RST-ACTIVE @ IF _UTUI-MC-S-INVALID EXIT THEN
    _UTUI-MC-RELATIONS-READY? 0= IF _UTUI-MC-S-INVALID EXIT THEN
    -1 _UTUI-MI-ACTIVE !
    _UTUI-MC-HEAD @ _UTUI-MI-CURRENT ! 0 _UTUI-MI-SEEN !
    BEGIN _UTUI-MI-CURRENT @ DUP WHILE
        DUP _UTUI-MI-SET-CURRENT DROP
        _UTUI-MI-CURRENT-SELECTED? IF
            _UTUI-MI-SOURCE @ _UDL-ELEMSZ * _UDL-ELEMS +
                _UTUI-RS-RESOLVE DUP _UTUI-MI-RESOLVE-STATUS !
            DUP UTUI-RESOLVED-S-INVALID = IF
                DROP _UTUI-MC-S-INVALID EXIT
            THEN
            DUP UTUI-RESOLVED-S-UNAVAILABLE = IF
                DROP
            ELSE
                UTUI-RESOLVED-S-OK <> IF _UTUI-MC-S-INVALID EXIT THEN
                _UTUI-MI-RESOLVED _UTUI-RS-DST ! _UTUI-RS-WRITE
                0 _UTUI-RS-DST !
                _UTUI-MI-SOURCE-GEOMETRY? 0= IF
                    _UTUI-RS-CLEAR _UTUI-MC-S-INVALID EXIT
                THEN
                _UTUI-RS-VISIBLE @ IF _UTUI-MI-VISIT THEN
            THEN
        THEN
        _UTUI-RS-CLEAR
        _UTUI-MI-CURRENT @ _UTUI-MCR-NEXT@ _UTUI-MI-CURRENT !
        1 _UTUI-MI-SEEN +!
        _UTUI-MI-SEEN @ _UTUI-MC-COUNT @ U> IF
            _UTUI-MC-S-INVALID EXIT
        THEN
    REPEAT DROP
    _UTUI-MI-SEEN @ _UTUI-MC-COUNT @ <> IF
        _UTUI-MC-S-INVALID EXIT
    THEN
    _UTUI-MC-S-OK ;

: _UTUI-MI-CALL  ( -- status )
    ['] _UTUI-MI-BODY CATCH ?DUP IF
        DROP _UTUI-RS-CLEAR _UTUI-MI-CLEAR _UTUI-MC-S-INVALID EXIT
    THEN
    _UTUI-RS-CLEAR _UTUI-MI-CLEAR ;

\ Private, outer-observation-only mounted collection iterator.  The callback
\ sees pointer-free source identity plus one borrowed resolved record; the
\ widget/relation pointers remain in this module's current scope.
: _UTUI-MOUNTED-COLLECTION-EACH-PREFLIGHTED  ( visitor-xt -- status )
    _UTUI-MI-ACTIVE @ IF DROP _UTUI-MC-S-INVALID EXIT THEN
    _UTUI-MI-VISITOR ! 0 _UTUI-MI-KIND ! _UTUI-MI-CALL ;

\ Private, outer-observation-only DATA_GRAPHICS iterator.  Its callback has
\ the same pointer-free `(source generation root-key resolved-a resolved-u)`
\ contract as the collection iterator; current widget authority remains
\ borrowed and private to the geometry/capture words below.
: _UTUI-MOUNTED-DATA-GRAPHICS-EACH-PREFLIGHTED  ( visitor-xt -- status )
    _UTUI-MI-ACTIVE @ IF DROP _UTUI-MC-S-INVALID EXIT THEN
    _UTUI-MI-VISITOR !
...
: _UTUI-MI-CURRENT-COLLECTION?  ( -- flag )
    ...
    _UTUI-MI-CURRENT @ _UTUI-MCR-KIND@
        _UTUI-MC-COLLECTION-KIND? ;

: _UTUI-MI-CURRENT-DATA-GRAPHICS?  ( -- flag )
    _UTUI-MI-CURRENT-VALID? 0= IF 0 EXIT THEN
    _UTUI-MI-CURRENT @ _UTUI-MCR-KIND@
        _UTUI-MC-K-DATA-GRAPHICS = ;

: _UTUI-MOUNTED-COLLECTION-GEOMETRY
    ( -- origin-row origin-col extent-h extent-w clip-row clip-col clip-h clip-w status )
    _UTUI-MI-CURRENT-COLLECTION? 0= IF _UTUI-CG-FAIL EXIT THEN
    _UTUI-MI-WIDGET @ _WDG-O-REGION + @
    _UTUI-MI-RESOLVED UTUI-RESOLVED-SIZE
        _UTUI-CANONICAL-REGION-GEOMETRY ;

: _UTUI-MC-CAPTURE  ( -- bytes status )
    _UTUI-MI-CURRENT @ _UTUI-MCR-KIND@
    DUP _UTUI-MC-K-TEXT-AREA = IF
        DROP
        _UTUI-MI-ROOT-KEY @ _UTUI-MI-DST @ _UTUI-MI-CAP @
        _UTUI-MI-BUILDER @ _UTUI-MI-WIDGET @
        TXTA-TEXT-AREA-CAPTURE EXIT
    THEN
    DUP _UTUI-MC-K-TEXT-GRID = IF
        DROP
        _UTUI-MI-ROOT-KEY @ _UTUI-MI-DST @ _UTUI-MI-CAP @
        _UTUI-MI-BUILDER @ _UTUI-MI-WIDGET @
...
: _UTUI-MOUNTED-COLLECTION-CAPTURE-PREFLIGHTED
    ( destination capacity builder -- bytes status )
    _UTUI-MI-BUILDER ! _UTUI-MI-CAP ! _UTUI-MI-DST !
    _UTUI-MI-CURRENT-COLLECTION? 0= IF 0 USCOL-S-INVALID EXIT THEN
    _UTUI-MC-CAPTURE ;

: _UTUI-MOUNTED-DATA-GRAPHICS-GEOMETRY
    ( -- origin-row origin-col extent-h extent-w clip-row clip-col clip-h clip-w status )
    _UTUI-MI-CURRENT-DATA-GRAPHICS? 0= IF
        _UTUI-CG-CLEAR 0 0 0 0 0 0 0 0 DGRAPH-S-INVALID EXIT
    THEN
    _UTUI-MI-WIDGET @ _WDG-O-REGION + @
    _UTUI-MI-RESOLVED UTUI-RESOLVED-SIZE
        _UTUI-CANONICAL-REGION-GEOMETRY
    DUP USCOL-S-OK = IF DROP DGRAPH-S-OK EXIT THEN
    DROP DGRAPH-S-INVALID ;
...
: _UTUI-MOUNTED-DATA-GRAPHICS-CAPTURE-PREFLIGHTED
    ...
    THEN
    _UTUI-MI-DST @ _UTUI-MI-CAP @
    _UTUI-MI-BUILDER @ _UTUI-MI-WIDGET @
        DGRAPH-DATA-GRAPHICS-CAPTURE ;

\ Check one caller span against every live canonical semantic widget before
\ an upper collector writes any of its caller-owned banks.  The public
\ collection query remains conservative across co-resident DATA_GRAPHICS
\ storage; safety cannot depend on which semantic snapshot writes first.
\ This scan is intentionally independent of effective visibility: a hidden
\ widget is still authoritative live storage and must not be corrupted by an
...
VARIABLE _UTUI-CS-W
VARIABLE _UTUI-CS-REL
VARIABLE _UTUI-CS-REL-SEEN
VARIABLE _UTUI-CS-RGN
VARIABLE _UTUI-CS-ROOT-W

: _UTUI-CS-CLEAR  ( -- )
    0 _UTUI-CS-SPAN-A ! 0 _UTUI-CS-SPAN-U ! 0 _UTUI-CS-I !
    0 _UTUI-CS-N !
    0 _UTUI-CS-ELEM ! 0 _UTUI-CS-SC ! 0 _UTUI-CS-W !
    0 _UTUI-CS-REL ! 0 _UTUI-CS-REL-SEEN !
    0 _UTUI-CS-RGN ! 0 _UTUI-CS-ROOT-W !
    _UTUI-MC-SCRATCH-CLEAR ;
...
: _UTUI-CS-TEXTAREA-WIDGET?  ( -- flag )
    _UTUI-CS-W @ _UTUI-MC-GENUINE-TEXTAREA? ;

: _UTUI-CS-QUERY-WIDGET  ( -- disjoint status )
    _UTUI-CS-W @ DUP _UTUI-MC-GENUINE-DATA-GRAPHICS? IF
        _UTUI-CS-SPAN-A @ _UTUI-CS-SPAN-U @ ROT
            DGRAPH-DATA-GRAPHICS-STORAGE-DISJOINT?
        USCOL-S-OK EXIT
    THEN
    DUP _WDG-O-TYPE + @ WDG-T-DATA-GRAPHICS = IF
        DROP 0 USCOL-S-INVALID EXIT
    THEN
    DUP _UTUI-MC-GENUINE-COLLECTION? 0= IF
        DROP -1 USCOL-S-OK EXIT
    THEN
    _UTUI-MC-COLLECTION-FAMILY@
    DUP USCOL-F-TEXT-AREA = IF
        DROP
        _UTUI-CS-SPAN-A @ _UTUI-CS-SPAN-U @
            TXTA-STORAGE-DISJOINT? 0= IF 0 USCOL-S-OK EXIT THEN
...
: _UTUI-CS-RANGE-DISJOINT?  ( address bytes -- flag )
    _UTUI-CS-SPAN-U @ 0= IF 2DROP -1 EXIT THEN
    DUP 0> 0= IF 2DROP 0 EXIT THEN
    OVER 0= IF 2DROP 0 EXIT THEN
    2DUP MSPAN-NONWRAPPING? 0= IF 2DROP 0 EXIT THEN
    _UTUI-CS-SPAN-A @ _UTUI-CS-SPAN-U @ 2SWAP MSPAN-OVERLAP? 0= ;

: _UTUI-CS-REGION-CHAIN?  ( region -- flag )
    DUP _UTUI-MC-RGN-ACYCLIC? 0= IF DROP 0 EXIT THEN
    _UTUI-CS-RGN !
    BEGIN _UTUI-CS-RGN @ DUP WHILE
        DUP RGN-SIZE _UTUI-CS-RANGE-DISJOINT? 0= IF DROP 0 EXIT THEN
        _RGN-O-PARENT + @ _UTUI-CS-RGN !
    REPEAT DROP -1 ;

: _UTUI-CS-RELATIONS  ( -- disjoint status )
    _UTUI-MC-HEAD @ _UTUI-CS-REL ! 0 _UTUI-CS-REL-SEEN !
    BEGIN _UTUI-CS-REL @ DUP WHILE
        DUP _UTUI-MC-REL-SIZE _UTUI-CS-RANGE-DISJOINT?
            0= IF DROP 0 USCOL-S-OK EXIT THEN
        DUP _UTUI-MCR-WIDGET@ DUP _UTUI-CS-W !
        _WDG-O-REGION + @ _UTUI-CS-REGION-CHAIN?
            0= IF DROP 0 USCOL-S-OK EXIT THEN

        _UTUI-CS-QUERY-WIDGET
        DUP USCOL-S-OK <> IF >R 2DROP 0 R> EXIT THEN
        DROP 0= IF DROP 0 USCOL-S-OK EXIT THEN

        DUP _UTUI-MCR-SOURCE@ _UDL-ELEMSZ * _UDL-ELEMS +
            _UTUI-SIDECAR _UTUI-SC-WPTR@ DUP _UTUI-CS-ROOT-W !
        DUP _WDG-HDR-SIZE _UTUI-CS-RANGE-DISJOINT?
            0= IF 2DROP 0 USCOL-S-OK EXIT THEN
        DROP

        _UTUI-MCR-NEXT@ _UTUI-CS-REL !
        1 _UTUI-CS-REL-SEEN +!
        _UTUI-CS-REL-SEEN @ _UTUI-MC-COUNT @ U> IF
            0 USCOL-S-INVALID EXIT
        THEN
    REPEAT DROP
    _UTUI-CS-REL-SEEN @ _UTUI-MC-COUNT @ <> IF
        0 USCOL-S-INVALID EXIT
    THEN
    -1 USCOL-S-OK ;
...
: _UTUI-CS-ONE  ( -- disjoint status )
    ...
    DUP 7 AND IF DROP 0 USCOL-S-INVALID EXIT THEN
    DUP _WDG-HDR-SIZE MSPAN-NONWRAPPING? 0= IF
        DROP 0 USCOL-S-INVALID EXIT
    THEN
    _UTUI-CS-W !
    _UTUI-CS-W @ _WDG-HDR-SIZE _UTUI-CS-RANGE-DISJOINT?
        0= IF 0 USCOL-S-OK EXIT THEN
    _UTUI-CS-QUERY-WIDGET ;

: _UTUI-CS-BODY  ( -- disjoint status )
    _UTUI-CS-SPAN? 0= IF 0 USCOL-S-INVALID EXIT THEN
    _UTUI-MC-RELATIONS-READY? 0= IF 0 USCOL-S-INVALID EXIT THEN
    _UTUI-DOC-LOADED @ 0= IF
        _UTUI-MC-COUNT @ IF 0 USCOL-S-INVALID ELSE -1 USCOL-S-OK THEN
        EXIT
    THEN
...
: UTUI-COLLECTION-STORAGE-DISJOINT?
    ( address bytes -- disjoint status )
    2DUP TXTA-STORAGE-DISJOINT? 0= IF
...
\ Private raw seam for a DATA_GRAPHICS snapshot collector that already holds
\ the complete UTUI/UIDL resolved observation.  Reuse the one live semantic
\ storage walk above, but preserve DGRAPH's status domain and distinguish an
\ ordinary overlap from malformed input/state.
: _UTUI-DATA-GRAPHICS-STORAGE-DISJOINT-OBSERVED?
    ( address bytes -- disjoint status )
    2DUP _UTUI-CS-SPAN-U ! _UTUI-CS-SPAN-A !
    _UTUI-CS-SPAN? 0= IF
        2DROP _UTUI-CS-CLEAR 0 DGRAPH-S-INVALID EXIT
    THEN
    2DUP DGRAPH-STORAGE-DISJOINT? 0= IF
        2DROP _UTUI-CS-CLEAR 0 DGRAPH-S-OK EXIT
    THEN
    _UTUI-CS-OBSERVED?
    DUP USCOL-S-OK = IF DROP DGRAPH-S-OK EXIT THEN
    2DROP 0 DGRAPH-S-INVALID ;
...
\ seams only while these two authoritative resolved sources remain coherent.
...
\ Finalize the conservative UIDL-TUI-owned span only after optional guard
