...
\ matches the canonical widget's (family, process-lifetime instance) identity,
...
48 CONSTANT _UTUI-MCR-O-FAMILY
...
32 CONSTANT _UTUI-MCT-O-FAMILY
...
: _UTUI-MCR-FAMILY@     ( relation -- value ) _UTUI-MCR-O-FAMILY + @ ;
...
\ The live relation chain is the canonical private index used by an upper
\ collection snapshot.  Keep its exact count and strict unsigned
...
: _UTUI-MC-GENUINE-TEXTGRID?  ( widget -- flag )
    ...
    DUP _WDG-O-TYPE + @ WDG-T-TEXTGRID =
    OVER _WDG-O-DRAW-XT + @ ['] _TGRID-DRAW = AND
...
: _UTUI-MC-RELATION-VALID?  ( relation -- flag )
    ...
    DUP _UTUI-MCR-FAMILY@ _UTUI-MC-V-FAMILY !
    ...
    _UTUI-MC-V-WIDGET @ DUP _UTUI-MC-GENUINE-COLLECTION? 0= IF
        DROP 0 EXIT
    THEN
    DUP _UTUI-MC-COLLECTION-FAMILY@
        _UTUI-MC-V-FAMILY @ <> IF DROP 0 EXIT THEN
    DUP _UTUI-MC-COLLECTION-INSTANCE@
        _UTUI-MC-V-INSTANCE @ <> IF DROP 0 EXIT THEN
    DROP
    _UTUI-MC-V-FAMILY @ _UTUI-MC-FAMILY? 0= IF 0 EXIT THEN
...
: _UTUI-MC-RELATION-DISTINCT?  ( relation -- flag )
    ...
    BEGIN _UTUI-MC-V-INNER @ DUP WHILE
        ...
        DUP _UTUI-MCR-FAMILY@
            _UTUI-MC-V-OUTER @ _UTUI-MCR-FAMILY@ = IF
...
: _UTUI-MC-STAGE-ADD  ( widget -- status )
    ...
    DUP _UTUI-MC-COLLECTION-FAMILY@ DUP 0= IF
        2DROP _UTUI-MC-S-INVALID EXIT
    THEN
    _UTUI-MC-SA-FAMILY !
    _UTUI-MC-COLLECTION-INSTANCE@ DUP 0= IF
        DROP _UTUI-MC-S-INVALID EXIT
    THEN
    ...
    BEGIN _UTUI-MC-SA-CURRENT @ DUP WHILE
        ...
        THEN
        1 _UTUI-MC-SA-SEEN +!
        DUP _UTUI-MCT-FAMILY@ _UTUI-MC-SA-FAMILY @ =
    ...
    _UTUI-MC-SA-FAMILY @ _UTUI-MC-SA-NEW @ _UTUI-MCT-O-FAMILY + !
...
: _UTUI-MC-FIND-RELATION  ( -- status )
    ...
    BEGIN _UTUI-MC-F-CURRENT @ DUP WHILE
        ...
        _UTUI-MC-F-SEEN @ _UTUI-MC-COUNT @ U> IF
            DROP _UTUI-MC-S-INVALID EXIT
        THEN
        DUP _UTUI-MCR-FAMILY@ _UTUI-MC-F-FAMILY @ =
...
: _UTUI-MC-PREPARE-STAGE  ( stage -- status )
    ...
    DUP _UTUI-MCT-FAMILY@ _UTUI-MC-F-FAMILY !
    ...
    _UTUI-MC-P-STAGE @ _UTUI-MCT-FAMILY@
        _UTUI-MC-P-REL @ _UTUI-MCR-O-FAMILY + !
...
: _UTUI-MC-UPSERT  ( widget -- status )
    ...
    _UTUI-MC-U-FAMILY @ _UTUI-MC-F-FAMILY !
    ...
    _UTUI-MC-U-FAMILY @ _UTUI-MC-U-REL @ _UTUI-MCR-O-FAMILY + !
...
: _UTUI-MC-OBS-CANONICAL  ( widget -- )
    DUP _UTUI-MC-GENUINE-COLLECTION? 0= IF DROP EXIT THEN
...
: _UTUI-MC-SCRATCH-CLEAR  ( -- )
    ...
    0 _UTUI-MC-V-INSTANCE ! 0 _UTUI-MC-V-SOURCE !
...
: _UTUI-MOUNTED-COLLECTION-CAPTURE-PREFLIGHTED
    ...
    _UTUI-MI-CURRENT-VALID? 0= IF 0 USCOL-S-INVALID EXIT THEN
    _UTUI-MC-CAPTURE ;
...
: _UTUI-CS-QUERY-WIDGET  ( -- disjoint status )
    _UTUI-CS-W @ DUP _UTUI-MC-GENUINE-COLLECTION? 0= IF
