...
: _RTAPT-PUBLICATION-CONTROL-DELTA-DEFINE?  ( -- flag )
    ...
    _RTAPT-PF-COPY @ _RTAPT-CD.PARENT @ ?DUP IF
        DUP _RTAPT-PF-COPY @ _RTAPT-CD.CONTROL @ U< 0= IF
            DROP 0 EXIT
        THEN
        _RTAPT-PF-CHIGH @ U> IF 0 EXIT THEN
    THEN
    ...
    _RTAPT-PF-COPY @ _RTAPT-CD.CONTENT-UTF8 @
        _RTAPT-UADD? 0= IF DROP 0 EXIT THEN _RTAPT-PF-UTF8 !
    \ Mixed DEFINE/REPLACE deltas retain a nonempty graph-audit phase while
    \ the exact parent forest remains owned by the retained transport.
    _RTAPT-PF-CONTROL-PHASE-COLLECTION _RTAPT-PF-CPHASE ! -1 ;
