...
\ to the sole RTE bridge.  The 160-byte record is a fixed ABI shape, not a
...
\ One call-borrowed LABEL plan describes one complete initial retained root.
\ These are ABI record shapes, never product capacities.
112 CONSTANT RTAPT-LABEL-PLAN-SIZE
128 CONSTANT RTAPT-LABEL-PLAN-ITEM-SIZE
...
: RTAPT-LABEL-PLAN-BYTES  ( -- bytes )  RTAPT-LABEL-PLAN-SIZE ;
: RTAPT-LABEL-PLAN-ITEM-BYTES  ( -- bytes )
    RTAPT-LABEL-PLAN-ITEM-SIZE ;
...
\ Mutation-free initial-LABEL admission plan.  ITEMS-A/ITEMS-U are borrowed
\ only for RTAPT-LABEL-PREFLIGHT's dynamic extent.
: _RTAPT-LP.OWNER        ( p -- a )       ;
...
: _RTAPT-LPI.RGBA          ( i -- a )  80 + ;
: _RTAPT-LPI.H-ALIGN       ( i -- a )  88 + ;
: _RTAPT-LPI.V-ALIGN       ( i -- a )  96 + ;
: _RTAPT-LPI.ELLIPSIZE     ( i -- a ) 104 + ;
: _RTAPT-LPI.TEXT-CAPACITY ( i -- a ) 112 + ;
: _RTAPT-LPI.RESERVED      ( i -- a ) 120 + ;
...
: _RTAPT-LIMITS-VALID-BODY  ( limits -- flag )
    ...
    _RTAPT-LV-L @ _RTAPT-L.OBJECTS @
    _RTAPT-LV-FEATURES @
        RTAPT-F-VECTOR RTAPT-F-IMAGE OR RTAPT-F-INSTRUMENT OR
        RTAPT-F-SERIES OR AND 0<> _RTAPT-POSITIVE-EXACT? 0= IF
        0 EXIT
    THEN
    _RTAPT-LV-L @ _RTAPT-L.PATH-POINTS @
    _RTAPT-LV-FEATURES @ RTAPT-F-VECTOR AND 0<>
        _RTAPT-POSITIVE-EXACT? 0= IF 0 EXIT THEN
    _RTAPT-LV-L @ _RTAPT-L.LABEL-BYTES @
    _RTAPT-LV-FEATURES @ RTAPT-F-INSTRUMENT AND 0<>
        _RTAPT-POSITIVE-EXACT? 0= IF 0 EXIT THEN
    _RTAPT-LV-L @ _RTAPT-L.UTF8-BYTES @
    _RTAPT-LV-FEATURES @ RTAPT-F-INSTRUMENT AND 0<>
        _RTAPT-POSITIVE-EXACT? 0= IF 0 EXIT THEN
    _RTAPT-LV-FEATURES @ RTAPT-F-INSTRUMENT AND IF
        _RTAPT-LV-L @ _RTAPT-L.UTF8-BYTES @
        _RTAPT-LV-L @ _RTAPT-L.LABEL-BYTES @ U< IF 0 EXIT THEN
    THEN
    ...
    THEN
    _RTAPT-LV-FEATURES @ RTAPT-F-INSTRUMENT AND IF
        _RTAPT-LV-L @ _RTAPT-L.LABEL-BYTES @ 304 _RTAPT-UADD? 0= IF
            DROP 0 EXIT
        THEN
...
: _RTAPT-LIMITS-COPY  ( -- limits )
    ...
    _RTAPT-LS-FORMATS-A @ 24 + L@ OVER _RTAPT-L.LABEL-BYTES !
...
\ =====================================================================
\  Mutation-free initial LABEL-plan admission
\ =====================================================================
\
...
\ engine's existing quarantine semantics.
...
: _RTAPT-LPF-HEADER?  ( -- flag )
    _RTAPT-LPF-PLAN @ RTAPT-LABEL-PLAN-SIZE _RTAPT-SPAN? 0= IF
        0 EXIT
    THEN
    ...
    _RTAPT-LPF-ITEMS-A @ _RTAPT-LPF-ITEMS-U @ _RTAPT-SPAN? 0= IF
        0 EXIT
    THEN
    _RTAPT-LPF-ITEMS-U @ RTAPT-LABEL-PLAN-ITEM-SIZE MOD IF 0 EXIT THEN
    _RTAPT-LPF-ITEMS-U @ RTAPT-LABEL-PLAN-ITEM-SIZE /
        DUP _RTAPT-LPF-COUNT !
    DUP 0= SWAP _RTAPT-U32? 0= OR IF 0 EXIT THEN
    _RTAPT-LPF-PLAN @ RTAPT-LABEL-PLAN-SIZE
        _RTAPT-LPF-ITEMS-A @ _RTAPT-LPF-ITEMS-U @
        MSPAN-OVERLAP? IF 0 EXIT THEN
    _RTAPT-LPF-PLAN @ RTAPT-LABEL-PLAN-SIZE _RTAPT-LPF-E @
...
: _RTAPT-LPF-ITEM?  ( item -- flag )
    ...
    _RTAPT-LPF-ITEM @ _RTAPT-LPI.RGBA @ _RTAPT-U32? 0= IF 0 EXIT THEN
    _RTAPT-LPF-ITEM @ _RTAPT-LPI.H-ALIGN @ 2 U>
    _RTAPT-LPF-ITEM @ _RTAPT-LPI.V-ALIGN @ 2 U> OR IF 0 EXIT THEN
    _RTAPT-LPF-ITEM @ _RTAPT-LPI.ELLIPSIZE @
        _RTAPT-LPF-BOOL? 0= IF 0 EXIT THEN
    ...
    _RTAPT-LPF-TEXT-CAP @ _RTAPT-LABEL-DEFINE-COPY-FIXED
        _RTAPT-UADD? 0= IF
        DROP _RTAPT-LPF-CAPACITY-FAIL EXIT
    THEN
...
: _RTAPT-LPF-ARITHMETIC?  ( -- flag )
    ...
    _RTAPT-LPF-COUNT @ 0 ?DO
        _RTAPT-LPF-ITEMS-A @ I RTAPT-LABEL-PLAN-ITEM-SIZE * +
    ...
    _RTAPT-LPF-OPS !
    _RTAPT-LPF-COUNT @ _RTAPT-LABEL-DEFINE-FRAME-FIXED
...
: _RTAPT-LABEL-PREFLIGHT-BODY  ( -- status )
    ...
    _RTAPT-LPF-MAX-TEXT-CAP @
        _RTAPT-LPF-LIMITS @ _RTAPT-L.LABEL-BYTES @ U> IF
        RTAPT-S-CAPACITY EXIT
    THEN
...
: RTAPT-LABEL-PREFLIGHT  ( plan engine -- status )
    _RTAPT-LPF-E ! _RTAPT-LPF-PLAN !
    ['] _RTAPT-LABEL-PREFLIGHT-BODY CATCH ?DUP IF
