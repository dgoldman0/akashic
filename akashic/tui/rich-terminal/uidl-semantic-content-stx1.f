...
: _USSTX-CORRELATE  ( -- status )
    ...
    _USSTX-ENTRY @ _USSTX-ENTRY-U @
        ...
        UTUI-SEMANTIC-S-INVALID EXIT
    THEN
    _USSTX-REVISION @ 0= IF UTUI-SEMANTIC-S-INVALID EXIT THEN
    _USSTX-ENTRY-U @ UTUI-SEMANTIC-ENTRY-HEADER-SIZE U< IF
        UTUI-SEMANTIC-S-INVALID EXIT
    THEN
    _USSTX-ENTRY-U @ 7 AND IF UTUI-SEMANTIC-S-INVALID EXIT THEN
    _USSTX-ENTRY @ UTUI-SEMANTIC-ENTRY-BYTES@
        _USSTX-ENTRY-U @ <> IF UTUI-SEMANTIC-S-INVALID EXIT THEN
    _USSTX-ENTRY @ UTUI-SEMANTIC-ENTRY-FAMILY-ABI@
        USCOL-FAMILY-ABI <> IF UTUI-SEMANTIC-S-INVALID EXIT THEN
    _USSTX-SUMMARY @ USCOL-SUMMARY-ENTRY-BYTES@
        _USSTX-ENTRY-U @ <> IF UTUI-SEMANTIC-S-INVALID EXIT THEN

    _USSTX-ENTRY @ UTUI-SEMANTIC-ENTRY-FAMILY@ DUP _USSTX-FAMILY !
    _USSTX-SUMMARY @ USCOL-SUMMARY-FAMILY@ <> IF
        UTUI-SEMANTIC-S-INVALID EXIT
    THEN
    _USSTX-ENTRY @ UTUI-SEMANTIC-ENTRY-KEY@ DUP 0= IF
    ...
    _USSTX-FAMILY @ _USSTX-TEXT-FAMILY? 0= IF
        UTUI-SEMANTIC-S-UNSUPPORTED EXIT
    THEN
    ...
    _USSTX-SUMMARY @ USCOL-SUMMARY-ITEM-COUNT@ DUP _USSTX-ITEM-COUNT !
        <> IF UTUI-SEMANTIC-S-INVALID EXIT THEN
    _USSTX-SUMMARY @ USCOL-SUMMARY-UTF8-BYTES@ DUP _USSTX-UTF8-U !
        _USSTX-U32? 0= IF UTUI-SEMANTIC-S-INVALID EXIT THEN

    _USSTX-SUMMARY @ USCOL-SUMMARY-STX1-BYTES
    DUP UTUI-SEMANTIC-S-OK <> IF NIP EXIT THEN
    DROP _USSTX-OUT-U !

    _USSTX-CAP @ 0< IF UTUI-SEMANTIC-S-INVALID EXIT THEN
    ...
    THEN
    MSPAN-NONWRAPPING? 0= IF UTUI-SEMANTIC-S-INVALID EXIT THEN
    _USSTX-ENTRY @ _USSTX-ENTRY-U @
...
: _USSTX-PACK-BODY  ( -- bytes status )
    _USSTX-CORRELATE DUP UTUI-SEMANTIC-S-OK <> IF
        0 SWAP EXIT
    THEN
    DROP
    _USSTX-HEADER!
    ...
    _USSTX-OUT-U @ UTUI-SEMANTIC-S-OK ;
