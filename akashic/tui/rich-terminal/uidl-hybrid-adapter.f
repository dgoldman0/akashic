...
\  model into caller-owned A/B banks at a completed draw boundary.  Documents
\  dirtied by their own lifecycle are recaptured; unchanged document slices
\  are validated and copied from the prior published bank.  It does not open a
...
: _RUHA-B-APPEND-DOCUMENT  ( record-u text-u -- status )
...
: _RUHA-B-CAPTURE-CURRENT  ( -- status )
    ...
    _RUHA-B-CAPTURE-TEXT-U @ _RUHA-B-APPEND-DOCUMENT
    DUP RUHA-S-OK = IF
...
\ rebases only those cells to the new snapshot generation.
