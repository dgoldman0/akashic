...
0x7F CONSTANT _RTE-FEATURE-MASK
...
\ Renderer-neutral semantic menu vocabulary.  These values describe meaning
\ above any terminal protocol; a concrete bridge maps each admitted kind and
\ state bit explicitly.
...
\ Geometry is expressed in integer cells relative to the projection root.
\ LABEL and SHORTCUT are borrowed only for the dynamic extent of a CONTROL
\ preflight, definition, or replacement call; the facade retains no pointer.
...
: _RTE-CONTROL.RESERVED    ( control -- a )  152 + ;

160 CONSTANT RTE-CONTROL-SIZE
...
\ Item text is borrowed through each 160-byte item record.  The preflight
\ callback receives only the checked scalar aggregates derived by the neutral
\ plan's sole item-bank pass.  Callback stack: plan count text-bytes,
\ aligned-text-bytes, max-item-text, last-id, context -- status.
...
: RTE-CONTROL-VALID?  ( control -- flag )
    ...
    DUP _RTE-CONTROL.SHORTCUT-A @ SWAP _RTE-CONTROL.SHORTCUT-U @
        _RTE-CONTROL-TEXT? ;
...
\ contiguous parent-first identity, fixed-depth menu graph/order/state, and
\ the exact scalar aggregates consumed by provider admission.  A nonzero
\ FACADE adds storage-authority checks to that same pass; it never triggers a
\ second item traversal.
...
VARIABLE _RTE-CPV-TEXT-BYTES
VARIABLE _RTE-CPV-ALIGNED-TEXT-BYTES
VARIABLE _RTE-CPV-MAX-ITEM-TEXT
VARIABLE _RTE-CPV-LAST-ID
VARIABLE _RTE-CPV-MENUBARS
VARIABLE _RTE-CPV-ROOT-ID
VARIABLE _RTE-CPV-ITEM-TEXT
VARIABLE _RTE-CPV-ITEM-ALIGNED
VARIABLE _RTE-CPV-TEXT-A
VARIABLE _RTE-CPV-TEXT-U
...
VARIABLE _RTE-CPV-CONTROL-TEXT-A
VARIABLE _RTE-CPV-CONTROL-TEXT-U
...
: _RTE-CPV-FINISH  ( x -- x )
    ...
    0 _RTE-CPV-PRIOR-ID !
    0 _RTE-CPV-COUNT !
    0 _RTE-CPV-TEXT-BYTES !
    0 _RTE-CPV-ALIGNED-TEXT-BYTES !
    0 _RTE-CPV-MAX-ITEM-TEXT !
    0 _RTE-CPV-LAST-ID !
    0 _RTE-CPV-MENUBARS !
    0 _RTE-CPV-ROOT-ID !
    0 _RTE-CPV-ITEM-TEXT !
    ...
    0 _RTE-CPV-CONTROL-TEXT-A !
    0 _RTE-CPV-CONTROL-TEXT-U !
...
: _RTE-CPV-TEXT-AUTHORITY?  ( a u -- flag )
    _RTE-CPV-TEXT-U ! _RTE-CPV-TEXT-A !
    _RTE-CPV-TEXT-U @ 0= IF _RTE-CPV-TEXT-A @ 0= EXIT THEN
    _RTE-CPV-FIXED-AUTHORITY @ IF
        _RTE-CPV-CONTROL-TEXT-U @ 0= IF 0 EXIT THEN
        _RTE-CPV-TEXT-A @ _RTE-CPV-CONTROL-TEXT-A @ U< IF 0 EXIT THEN
        _RTE-CPV-TEXT-A @ _RTE-CPV-TEXT-U @ +
        _RTE-CPV-CONTROL-TEXT-A @ _RTE-CPV-CONTROL-TEXT-U @ +
            U> IF 0 EXIT THEN
        -1 EXIT
    THEN
    _RTE-CPV-TEXT-A @ _RTE-CPV-TEXT-U @
    ...
    THEN
    _RTE-CPV-FACADE @ IF
        _RTE-CPV-TEXT-A @ _RTE-CPV-TEXT-U @ _RTE-CPV-FACADE @
            RTE-STORAGE-DISJOINT? 0= IF 0 EXIT THEN
    THEN
    -1 ;
...
: _RTE-CPV-ITEM-TEXT-AUTHORITY?  ( -- flag )
    _RTE-CPV-ITEM @ _RTE-CONTROL.LABEL-A @
    _RTE-CPV-ITEM @ _RTE-CONTROL.LABEL-U @
        _RTE-CPV-TEXT-AUTHORITY? 0= IF 0 EXIT THEN
    _RTE-CPV-ITEM @ _RTE-CONTROL.SHORTCUT-A @
    _RTE-CPV-ITEM @ _RTE-CONTROL.SHORTCUT-U @
        _RTE-CPV-TEXT-AUTHORITY? ;
...
\ One plan may concatenate complete per-document menu forests.  Object IDs
\ remain globally contiguous, while each new root resets only forest-local
\ order and selection state and becomes the lower bound for later parents.
: _RTE-CPV-GRAPH?  ( -- flag )
    _RTE-CPV-ID? 0= IF 0 EXIT THEN
    _RTE-CPV-ITEM @ _RTE-CONTROL.KIND @ RTE-CONTROL-MENU-BAR = IF
        _RTE-CPV-ITEM @ _RTE-CONTROL.PARENT @ IF 0 EXIT THEN
        _RTE-CPV-MENUBARS @ 1 _RTE-UADD? 0= IF DROP 0 EXIT THEN
            _RTE-CPV-MENUBARS !
...
: _RTE-CPV-ITEM-AGGREGATE?  ( -- flag )
    ...
    _RTE-CPV-ITEM @ _RTE-CONTROL.SHORTCUT-U @ _RTE-UADD? 0= IF
        DROP 0 EXIT
    THEN DUP _RTE-CPV-ITEM-TEXT !
    _RTE-CPV-TEXT-BYTES @ SWAP _RTE-UADD? 0= IF DROP 0 EXIT THEN
    _RTE-CPV-TEXT-BYTES !
    _RTE-CPV-ITEM-TEXT @ 7 _RTE-UADD? 0= IF DROP 0 EXIT THEN
    7 INVERT AND DUP _RTE-CPV-ITEM-ALIGNED !
    _RTE-CPV-ALIGNED-TEXT-BYTES @ SWAP _RTE-UADD? 0= IF
        DROP 0 EXIT
    THEN _RTE-CPV-ALIGNED-TEXT-BYTES !
    _RTE-CPV-ITEM-TEXT @ _RTE-CPV-MAX-ITEM-TEXT @ MAX
...
: _RTE-CPV-ITEM?  ( -- flag )
    ...
    _RTE-CPV-ITEM-TEXT-AUTHORITY? 0= IF 0 EXIT THEN
...
VARIABLE _RTE-HPV-CONTROL-TEXT-A
VARIABLE _RTE-HPV-CONTROL-TEXT-U
...
: _RTE-HPV-CONTROL-TEXT-SPAN  ( hybrid -- a u )
    DUP _RTE-HP.CONTROL-TEXT-A @
    SWAP _RTE-HP.CONTROL-TEXT-U @ ;
...
: _RTE-HPV-FIXED-WRAPPER?  ( hybrid facade -- flag )
    ...
    OVER _RTE-HP.CONTROL-PLAN @ 0= IF
        OVER _RTE-HP.CONTROL-TEXT-A @
        2 PICK _RTE-HP.CONTROL-TEXT-U @ OR IF 2DROP 0 EXIT THEN
    THEN
...
: _RTE-HPV-FIXED-CONTROL?  ( hybrid facade -- flag )
    ...
    DUP _RTE-HPV-CONTROL-TEXT-SPAN R@
    ...
    DUP _RTE-HP.CONTROL-TEXT-U @ IF
        DUP RTE-HYBRID-PLAN-SIZE
        2 PICK _RTE-HPV-CONTROL-TEXT-SPAN
            _RTE-HPV-FIXED-DISJOINT? 0= IF DROP R> DROP 0 EXIT THEN
        DUP _RTE-HPV-CONTROL-PLAN-SPAN
        2 PICK _RTE-HPV-CONTROL-TEXT-SPAN
            _RTE-HPV-FIXED-DISJOINT? 0= IF DROP R> DROP 0 EXIT THEN
...
: _RTE-HPV-INIT  ( -- )
    ...
    _RTE-HPV-HYBRID @ _RTE-HP.CONTROL-TEXT-A @
        _RTE-HPV-CONTROL-TEXT-A !
    _RTE-HPV-HYBRID @ _RTE-HP.CONTROL-TEXT-U @
        _RTE-HPV-CONTROL-TEXT-U !
    _RTE-HPV-CONTROL @ IF
...
: _RTE-HPV-CONTROL?  ( -- flag )
    ...
    _RTE-HPV-CONTROL-TEXT-A @ _RTE-CPV-CONTROL-TEXT-A !
    _RTE-HPV-CONTROL-TEXT-U @ _RTE-CPV-CONTROL-TEXT-U !
    _RTE-CONTROL-PLAN-VALID-BODY 0= IF 0 EXIT THEN
    _RTE-HPV-CONTROL-TEXT-U @ 0=
    _RTE-CPV-TEXT-BYTES @ 0<> AND IF 0 EXIT THEN
    _RTE-HPV-CONTROL @ _RTE-HPV-COPY-HEADER
    _RTE-CPV-COUNT @ _RTE-HPV-SUMMARY _RTE-HA.CONTROL-COUNT !
    _RTE-CPV-TEXT-BYTES @ _RTE-HPV-SUMMARY _RTE-HA.CONTROL-TEXT !
    _RTE-CPV-ALIGNED-TEXT-BYTES @
        _RTE-HPV-SUMMARY _RTE-HA.CONTROL-ALIGNED !
    _RTE-CPV-MAX-ITEM-TEXT @ _RTE-HPV-SUMMARY _RTE-HA.CONTROL-MAX !
...
: _RTE-HPV-FINISH  ( status -- status )
    ...
    0 _RTE-HPV-CONTROL-TEXT-A ! 0 _RTE-HPV-CONTROL-TEXT-U !
