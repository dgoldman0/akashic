...
: AHS-CTX-RESTORE  ( slot -- )
    AHS.UCTX @ ASHELL-CTX-SWITCH ;
...
: AHOST-UIDL-READY!  ( xt context host -- )
    DUP >R AHOST.UIDL-READY-CONTEXT !
    R> AHOST.UIDL-READY-XT ! ;

VARIABLE _AHUR-HOST
VARIABLE _AHUR-SLOT
VARIABLE _AHUR-RESULT

: _AHUR-BODY  ( -- )
    _AHUR-HOST @ AHOST.UIDL-READY-XT @ ?DUP 0= IF EXIT THEN
    >R
    _AHUR-HOST @ _AHUR-SLOT @
    _AHUR-HOST @ AHOST.UIDL-READY-CONTEXT @
    R> EXECUTE _AHUR-RESULT ! ;

: _AHOST-UIDL-READY  ( host slot -- ior )
    _AHUR-SLOT ! _AHUR-HOST ! 0 _AHUR-RESULT !
    ['] _AHUR-BODY CATCH ?DUP IF _AHUR-RESULT ! THEN
    0 _AHUR-HOST ! 0 _AHUR-SLOT !
    _AHUR-RESULT @ 0 _AHUR-RESULT ! ;
...
: AHOST-VCOUNT  ( host -- n )
    0 SWAP AHOST.HEAD @
    BEGIN ?DUP WHILE
        DUP AHS-CALLABLE? OVER AHS-VISIBLE? AND IF SWAP 1+ SWAP THEN
        AHS.NEXT @
    REPEAT ;
...
-1410 CONSTANT AHOST-CLOSE-E-PHASE
-1411 CONSTANT AHOST-CLOSE-E-BARRIER

: _AHC-REMEMBER  ( ior -- )
    ?DUP IF _AHC-IOR @ 0= IF _AHC-IOR ! ELSE DROP THEN THEN ;

: _AHC-REMEMBER-BARRIER  ( ior -- )
    DUP 0> IF DROP AHOST-CLOSE-E-BARRIER THEN
    _AHC-REMEMBER ;
...
: _AHQS-REMEMBER  ( ior -- )
    ?DUP IF _AHQS-IOR @ 0= IF _AHQS-IOR ! ELSE DROP THEN THEN ;

: _AHQS-SAVE  ( -- )
    _AHQS-SLOT @ AHS.HAS-UIDL @ IF
        ASHELL-ACTIVE-CTX _AHQS-SLOT @ AHS.UCTX @ = IF
...
: _AHQS-BODY  ( -- )
    _AHQS-SLOT @ AHS.CLOSE-PHASE @
        AHS-CLOSE-PHASE-VALID? 0= IF AHOST-CLOSE-E-PHASE THROW THEN
    _AHQS-SLOT @ AHS.CLOSE-PHASE @
        AHS-CLOSE-S-QUIESCED U< 0= IF EXIT THEN
    _AHQS-SLOT @ AHS.CLOSE-PHASE @ AHS-CLOSE-S-LIVE = IF
        AHS-CLOSE-S-QUIESCING _AHQS-SLOT @ AHS.CLOSE-PHASE !
    ELSE
        _AHQS-SLOT @ AHS.CLOSE-PHASE @
            AHS-CLOSE-S-QUIESCING <> IF AHOST-CLOSE-E-PHASE THROW THEN
    THEN
    _AHQS-SLOT @ AHS.HAS-UIDL @ IF
        _AHQS-SLOT @ AHS-CTX-RESTORE
        UTUI-QUIESCE ?DUP IF
            DROP AHOST-CLOSE-E-BARRIER THROW
        THEN
        _AHQS-SLOT @ AHS-CTX-SAVE
    THEN
...
VARIABLE _AHQA-HOST
VARIABLE _AHQA-ORIGINAL-CTX
VARIABLE _AHQA-IOR

: _AHQA-REMEMBER  ( ior -- )
    ?DUP IF _AHQA-IOR @ 0= IF _AHQA-IOR ! ELSE DROP THEN THEN ;

: _AHQA-BODY  ( -- )
    _AHQA-HOST @ AHOST.HEAD @
    BEGIN ?DUP WHILE
        DUP _AHOST-QUIESCE-SLOT ?DUP IF THROW THEN
        AHS.NEXT @
    REPEAT ;
...
: AHOST-QUIESCE-ALL  ( host -- ior )
    ...
    ['] _AHQA-RESTORE CATCH _AHQA-REMEMBER
    _AHQA-IOR @
    0 _AHQA-HOST ! 0 _AHQA-ORIGINAL-CTX ! 0 _AHQA-IOR ! ;

: _AHC-SHUTDOWN  ( -- )
    _AHC-SLOT @ AHS.CLOSE-PHASE @
        AHS-CLOSE-S-QUIESCED <> IF AHOST-CLOSE-E-PHASE THROW THEN
    _AHC-SLOT @ AHS.HAS-UIDL @ IF _AHC-SLOT @ AHS-CTX-RESTORE THEN
    AHS-CLOSE-S-SHUTDOWN-CLAIMED _AHC-SLOT @ AHS.CLOSE-PHASE !
    _AHC-HOST-ONLY @
    _AHC-SLOT @ AHS.INIT-STARTED @ 0= OR IF EXIT THEN
    _AHC-SLOT @ AHS-ACTIVATE
...
: _AHC-DETACH  ( -- )
    _AHC-SLOT @ AHS.CLOSE-PHASE @
        ...
        AHOST-CLOSE-E-PHASE THROW
    THEN
    _AHC-SLOT @ AHS.HAS-UIDL @ IF
        _AHC-SLOT @ AHS-CTX-RESTORE
        UTUI-DETACH
        _AHC-SLOT @ AHS-CTX-SAVE
    THEN
    AHS-CLOSE-S-DETACHED _AHC-SLOT @ AHS.CLOSE-PHASE !
    0 _AHC-SLOT @ AHS.HAS-UIDL ! ;
...
: _AHOST-CLOSE-SLOT-FORCE  ( slot host -- closed? ior )
    _AHC-HOST ! _AHC-SLOT !
    _AHC-SLOT @ AHS.INST @ _AHC-INST !
    0 _AHC-IOR !
    _AHC-SLOT @ AHS.CLOSE-PHASE @
        AHS-CLOSE-PHASE-VALID? 0= IF
        AHOST-CLOSE-E-PHASE _AHC-REMEMBER
    THEN
    _AHC-SLOT @ AHS.CLOSE-PHASE @ AHS-CLOSE-S-QUIESCED U< IF
        _AHC-SLOT @ _AHOST-QUIESCE-SLOT _AHC-REMEMBER
    THEN
    _AHC-SLOT @ AHS.CLOSE-PHASE @ AHS-CLOSE-S-QUIESCED = IF
        ['] _AHC-SHUTDOWN CATCH _AHC-REMEMBER
    THEN
    _AHC-SLOT @ AHS.CLOSE-PHASE @
        AHS-CLOSE-S-SHUTDOWN-CLAIMED = IF
    ...
    -1 _AHC-IOR @
    0 _AHC-SLOT ! 0 _AHC-HOST ! 0 _AHC-INST ! ;

VARIABLE _AHI-HOST
VARIABLE _AHI-REASON
VARIABLE _AHI-IOR
VARIABLE _AHI-SLOT
VARIABLE _AHI-DECISION
VARIABLE _AHI-CLOSED

: _AHI-REMEMBER  ( ior -- )
    ?DUP IF _AHI-IOR @ 0= IF _AHI-IOR ! ELSE DROP THEN THEN ;

: _AHI-RELAYOUT  ( -- )
    _AHI-HOST @ _AHOST-RELAYOUT ;
...
: AHOST-REQUEST-CLOSE-ID  ( id reason host -- decision )
    ...
    DUP _AHI-SLOT !
    AHS-CALLABLE? IF
        _AHI-SLOT @ _AHI-REASON @ _AHOST-REQUEST-CLOSE-SLOT
    ELSE
        APP-CLOSE-D-ALLOW
    THEN
    _AHI-DECISION !
    _AHI-DECISION @ APP-CLOSE-D-ALLOW = IF
        0 _AHI-IOR !
        0 _AHC-HOST-ONLY !
        _AHI-SLOT @ _AHI-HOST @ _AHOST-CLOSE-SLOT-FORCE
        _AHI-IOR ! _AHI-CLOSED !
        _AHI-CLOSED @ IF
            ['] _AHI-RELAYOUT CATCH _AHI-REMEMBER
            _AHI-IOR @ ?DUP IF THROW THEN
        ELSE
            APP-CLOSE-D-DEFER _AHI-DECISION !
        THEN
    THEN
    _AHI-DECISION @ ;
...
: AHOST-REQUEST-CLOSE-ALL  ( reason host -- decision )
    ...
    BEGIN ?DUP WHILE
        DUP AHS-CALLABLE? IF
            DUP _AHALL-REASON @ _AHOST-REQUEST-CLOSE-SLOT
        ELSE
            APP-CLOSE-D-ALLOW
        THEN
        _AHALL-MERGE
...
: AHOST-DRAIN  ( host -- ior )
    ...
    BEGIN _AHD-HOST @ AHOST.HEAD @ ?DUP WHILE
        _AHD-HOST @ _AHOST-CLOSE-SLOT-FORCE
        _AHD-REMEMBER
        0= IF _AHD-IOR @ EXIT THEN
    REPEAT
    _AHD-IOR @ ;
...
-1407 CONSTANT AHOST-LAUNCH-E-UIDL-READY
...
: _AHL-BODY  ( -- )
    ...
    _AHL-SLOT @ AHS.HAS-UIDL @ IF
        ...
        ELSE
            ...
            THEN
        THEN
        _AHL-HOST @ _AHL-SLOT @ _AHOST-UIDL-READY ?DUP IF
            DUP 0> IF DROP AHOST-LAUNCH-E-UIDL-READY THEN
            THROW
        THEN
    THEN
    -1 _AHL-INIT-STARTED !
    -1 _AHL-SLOT @ AHS.INIT-STARTED !
    _AHL-DESC @ APP.INIT-XT @ ?DUP IF _AHL-INST @ SWAP EXECUTE THEN
    _AHL-SLOT @ AHS.HAS-UIDL @ IF _AHL-SLOT @ AHS-CTX-SAVE THEN ;

VARIABLE _AHR-IOR
VARIABLE _AHR-CLOSED
...
: _AHL-ROLLBACK  ( -- )
    0 _AHR-IOR ! 0 _AHR-CLOSED !
    _AHL-SLOT @ IF
        _AHL-INIT-STARTED @ 0= _AHC-HOST-ONLY !
        _AHL-SLOT @ _AHL-HOST @ _AHOST-CLOSE-SLOT-FORCE
        _AHR-REMEMBER _AHR-CLOSED !
        0 _AHC-HOST-ONLY !
    ELSE
...
: AHOST-RESTORE  ( host -- )
    ...
    DUP AHS-CALLABLE? 0= IF 2DROP EXIT THEN
...
: AHOST-TICK  ( host -- )
    ...
    BEGIN ?DUP WHILE
        DUP AHS-ALIVE? OVER AHS-CALLABLE? AND IF
            DUP >R
