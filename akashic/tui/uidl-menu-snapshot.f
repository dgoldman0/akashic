...
CREATE _UMSN-PROOF-SPANS 4 MSPAN-SET-BYTES ALLOT

\ Collect the four caller spans for one enclosed authority proof.  Their
\ shapes were admitted first, and the prover skips empty optional spans.
: _UMSN-PROOF-SPANS?  ( -- flag )
    4 _UMSN-PROOF-SPANS MSPAN-SET-INIT MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UMSN-WORK-A @ _UMSN-WORK-U @ _UMSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UMSN-WORK-TEXT-A @ _UMSN-WORK-TEXT-U @ _UMSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UMSN-RECORDS-A @ _UMSN-RECORDS-U @ _UMSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UMSN-TEXT-A @ _UMSN-TEXT-U @ _UMSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK = ;
...
: _UMSN-RANGES?  ( -- flag )
    ...
    _UMSN-PROOF-SPANS? 0= IF 0 EXIT THEN
    _UMSN-PROOF-SPANS ['] _UMSN-AUTHORITY-DISJOINT?
