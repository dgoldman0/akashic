...
CREATE _UDGSN-PROOF-SPANS 3 MSPAN-SET-BYTES ALLOT

\ Collect the three caller spans for one enclosed authority proof.  Their
\ shapes are admitted first, and the prover skips empty optional spans.
: _UDGSN-PROOF-SPANS?  ( -- flag )
    3 _UDGSN-PROOF-SPANS MSPAN-SET-INIT MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UDGSN-BUILDER @ UDG-BUILDER-SIZE _UDGSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UDGSN-DESCRIPTORS-A @ _UDGSN-DESCRIPTORS-U @
        _UDGSN-PROOF-SPANS MSPAN-SET-PUSH MSPAN-SET-S-OK <> IF 0 EXIT THEN
    _UDGSN-NATIVE-A @ _UDGSN-NATIVE-U @ _UDGSN-PROOF-SPANS MSPAN-SET-PUSH
        MSPAN-SET-S-OK = ;

\ Caller span shapes and mutual disjointness.  Authority is proved
\ separately, and only when a DATA_GRAPHICS relation can be captured.
: _UDGSN-SHAPES?  ( -- flag )
...
\ Each authority query walks the document's storage.  Clustered caller spans
\ need one enclosed query instead of one per span.
: _UDGSN-AUTHORITY?  ( -- flag )
    _UDGSN-PROOF-SPANS? 0= IF 0 EXIT THEN
    _UDGSN-PROOF-SPANS ['] _UDGSN-AUTHORITY-DISJOINT?
        MSPAN-SET-PROVE-DISJOINT? ;
...
: _UDGSN-CAPTURE-OBSERVED  ( -- descriptor-count native-used status )
    ...
    0 _UDGSN-DIRTY-DESCRIPTOR-U ! 0 _UDGSN-DIRTY-NATIVE-U !
    _UDGSN-SHAPES? 0= IF 0 0 UDGSN-S-INVALID EXIT THEN
    \ Without a mounted DATA_GRAPHICS relation the complete path captures
    \ nothing, so no caller span would be read or written.
    _UTUI-MOUNTED-DATA-GRAPHICS? 0= IF 0 0 UDGSN-S-OK EXIT THEN
    _UDGSN-AUTHORITY? 0= IF 0 0 UDGSN-S-INVALID EXIT THEN
