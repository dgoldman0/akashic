...
inline depth-zero root has zero through four initialized or unwritten extents,
including sparse logical gaps. Before emission, retained transaction
certificate state binds exact release authority. The first transaction commits
the zero-size inode with its old map and `i_blocks` retained and publishes its
membership in either an empty modern orphan union or the first free slot of an
existing authenticated modern-only union. It then uses linked-orphan cleanup
to release every
...
parent size, and parent sector count immutable. The same admitted parent
shapes now also participate in bounded no-victim `RENAME`: old and new parents
are independently linear, depth-zero HTree, or singleton depth-one HTree.
Regular files may move within or across parents, and the canonical empty
directory may move across parents. A same-leaf record that already fits is
rewritten in place; other selected-leaf edits use an aggregate compact-fit
rebuild without splitting, growth, or HTree-key edits. Roots, optional
DX nodes, extent maps, siblings, and parent geometry remain immutable. The
same surface now provides a bounded atomic `MKDIR`: it allocates
...
HTree. It updates parent links, free-space, and `used_dirs` accounting in one
exact `7/0/0` through `9/0/0` transaction. The indexed form mutates only the
selected leaf; its HTree root, optional DX node, extent map, other leaves,
parent size, and parent sector count remain immutable. It also provides bounded
empty-directory `RMDIR` for that exact canonical child shape. Removal atomically
splices the typed parent entry from an authenticated linear, depth-zero HTree,
or singleton depth-one HTree parent. The indexed form mutates only the selected
leaf and leaves the root, optional DX node, extent map, other leaves, parent
size, and parent sector count immutable. Removal
...
root-owned regular inode and an admitted destination parent: either a one-block
linear directory or an existing depth-zero or singleton depth-one HTree with
authenticated slack in the selected checksummed leaf. It adds the typed
directory record, increments the target link count, and updates target and
parent timestamps under exact deduplicated `2/0/0` or `3/0/0` credit, without
allocation, ordered data, revoke, or orphan state.
...
`LINK` and `MKDIR` may reuse authenticated existing slack in an admitted depth-
zero or singleton depth-one HTree. `LINK` does not allocate; `MKDIR` retains its
one child-inode and one child-directory-block allocation. Indexed parent-leaf
splitting/growth, inheritance beyond the explicit root-owned non-setgid
envelope, and broader directory shapes remain gated. Exact one-short credit and unrelated-
owner alias cases refuse without writes or flushes. A committed tear in the new
DX-node home replays all 11 afterimages before a write-free byte-stable remount.
...
8,992-byte binding-owned record. Its 8,264-byte authority/evidence body contains
the common 47 insertion cells, 24-byte hash-authority snapshot, four 1,024-byte
block snapshots, and one 256-byte inode snapshot; RENAME adds 21 scalar cells,
one destination-leaf snapshot, independent new-parent root and DX-node
snapshots, and one new-parent inode snapshot. Record-scoped `OP` and
`OWNER-CTX` cells follow that evidence. The 728-byte tail contains one
...
The XU/XR paths use the following exact sealed vectors in transaction
...
- Same-parent same-leaf RENAME: the common three metadata entries. A
  same-parent cross-leaf RENAME appends
  `_XC-HR-RENAME-NEW-PARENT-DIRECTORY=destination-leaf-home`, for four exact
  semantic roles. Cross-parent regular-file RENAME instead binds
  `_XC-HR-INODE=source-inode-table-home`,
...
indexed-UNLINK slice removed the private `_XU-DIR-SNAPSHOT` and
`_XU-PARENT-SNAPSHOT`: linear XU/XR plus indexed UNLINK and RMDIR began using
the record's existing directory and parent-inode snapshots across cold, dry,
and live planning. At that historical checkpoint the record geometry was
unchanged, the exact `UNLINK` or `RMDIR` operation tag had to agree with
ambient removal mode before an indexed XU shape could bind, and indexed RENAME
remained gated. The later indexed-RENAME slice moved its independent old/new
parent shape and destination authority into the enlarged record described
above. Target-inode and child-directory working evidence remains
facade-private. XH retains its private collector for future work.
...
reserves the facade's 8,992-byte namespace-plan record from `fs-arena`, not the
dedicated writer arena. The base-context-plus-record reservation is 24,560
...
external xattrs. A linear parent has flags exactly `EXTENTS`, one initialized
mapped block, and a complete checksummed dirent chain. Existing slack takes the
original insertion path. If that authenticated block has no usable record,
...
allocation afterimages in the same exact transaction. Depth-one CREATE leaf
splitting, every LINK or MKDIR path requiring conversion, splitting, or growth,
default-ACL inheritance, and non-root credential policy remain gated; none is
...
staging. It snapshots the exact source block, hashes every non-dot live name
...
groups, bitmap homes, and primary-GDT homes are baseline authority. Before
staging, one complete map audit proves the old one-block parent map, then one
three-range filesystem walk proves the old root and both candidates have no
other owner. The proof refuses any ambient owner, map, journal-protocol,
external-xattr-reference, or ownership-certificate scope before publishing
its temporary vector and withdraws that vector on every exit.

The old directory block becomes the checksummed DX root; the two candidates
become independently checksummed leaves. The parent is rewritten to size
3,072 bytes and six 512-byte sectors with one, two, or three resident extents
according only to logical and physical adjacency. The operation allocates the
two leaf blocks and the new file inode in one metadata-only transaction. Exact
credit is the deduplicated topology of primary superblock, participating GDT
and bitmap pages, new/parent inode-table pages, old root, and both leaves:
`8/0/0` through `12/0/0`, never a fixture-fixed nine-home reservation.
...
Non-growing edits are one no-data/no-revoke transaction over the deduplicated
set of primary superblock, primary GDT page, inode bitmap, new inode-table
...
continuation leaf, a full eligible leaf under a saturated HTree root, and
inexact `i_blocks` all refuse without a transaction or cache/accounting
publication.
...
three names into each leaf, using blocks 1351 and 1352 with separator
`0x95d3b562`; the parent has a singleton old-root extent followed by one
length-two extent for the adjacent leaves. The exact nine checkpoint homes are
child inode table W23, parent inode table W24, root W25, the two leaves
W26/W27, block bitmap W28, GDT W29, primary superblock W30, and inode bitmap
W31. Raw after-images, public cache/accounting state, `debugfs`, and pinned
read-only e2fsprogs 1.47.4 `e2fsck` all agree. The clean journey uses
1,552,268,603 of its 1.6-billion-step guard.

A nonadjacent qualification leaves only block 1351 available in group zero and
selects block 8451 in group one. It produces three singleton parent extents and
ten exact homes, including both block bitmaps, while sharing the primary GDT
page. It completes in 1,673,299,481 of the checked-in 1.70-billion guard and
passes the same external oracles. Its complete owner proof also authenticates
the 6,841-block unwritten ballast extent that leaves only the group-zero
singleton available; this real topology accounts for the additional measured
work. In the canonical nine-home case, a published ambient owner range for
candidate 1351 returns `BUSY` before range preparation and preserves the
caller's exact pointer, capacity, count, bounds, and range words. After that
scope is explicitly cleared, a profile with one fewer metadata slot refuses
with `NOSPC` after exact topology measurement and performs no write. A sector
tear on that case's second leaf at W27 retains committed CREATE authority after
four complete checkpoint homes; the fault journey uses 1,376,638,284 steps,
recovery replays all nine after-images in 209,835,075, and the following
...
The parent uses the CREATE slice's root-owned, non-setgid, no-xattr insertion
envelope. It may be a one-block checksummed linear directory, an authenticated
depth-zero HTree, or the singleton depth-one HTree emitted by root growth.
Indexed insertion requires existing slack in the selected checksummed leaf and
binds the complete leaf permutation, live hash policy, route, root, and, at
depth one, the sole DX node across cold, dry, and live passes. The root,
optional DX node, extent map, other leaves, parent size, and parent sector count
remain immutable. Its on-disk link count must be from 2 through 64999 so the
new subdirectory cannot overflow ext4's 65000-link bound. A full selected
indexed leaf returns `VFS-E-NOSPC`; MKDIR never converts, splits, or grows the
parent. Default-ACL or setgid inheritance, non-root credential policy, broader
directory maps, and groups requiring inode-table initialization remain typed
refusal boundaries.
...
selects a clear data-block bit from runtime initialized geometry. The complete
parent-map audit proves that the selected linear block or indexed leaf occurs
exactly once in that parent. A paired whole-filesystem owner proof then excludes
every other inode from both the selected mutable block and the distinct child
candidate. Static-metadata, journal, bitmap, descriptor, superblock, and
inode-table aliases are rejected separately. Dry staging performs that proof
before journal activation, and live staging repeats it and recaptures the
complete parent authority plus candidate from current media.
...
sharing. The parent-directory role binds the selected leaf for indexed MKDIR;
the root, optional DX node, extent map, and other leaves are authenticated
immutable inputs rather than transaction homes. The canonical linear-root
fixture consumes `8/0/0`: child inode-table block 283, parent inode-table block
275, parent directory block 1299, child directory
block 1364, inode bitmap 267, block bitmap 259, primary GDT block 2, and
primary superblock 1. That linear success path validates the cache projection, raw
...
homes, and a checksum-valid `used_dirs` count exceeding the
...
stable and write-free. The linear focused success journey consumes 1,054,460,044
...
The parent may use the authenticated one-block linear-directory envelope, a
depth-zero HTree, or the singleton depth-one HTree emitted by root growth, and
must contain one exact file-type-2 target entry. Its link count must be at least
three. Indexed admission authenticates the complete parent map, HTree root,
primary-super hash authority, active table or sole DX node, and every active
checksummed leaf. It binds the route/hash and selected leaf across cold, dry,
and live passes. Before excluding the parent and child from the paired owner
walk, one complete parent-map pass proves the child release block has zero
references and a second proves the selected leaf has exactly one. The generic
map hook also rejects a child-block alias with the parent's external extent
node. One two-owner reverse-map walk then publishes both distinct directory
ranges and excludes only their two legitimate inode owners. Every other
allocated inode is checked against both ranges in that single walk. Dry and
live staging each repeat the proof and recapture the target, canonical child
block, parent inode, complete HTree authority, and selected leaf before
retaining an after-image.
...
The semantic plan remains the unchanged nine roles in canonical order: child
inode, parent inode, selected parent directory, released-block GDT, released-
block bitmap, primary super, child-directory revoke, inode GDT, and inode
bitmap. For an indexed parent the selected leaf occupies the parent-directory
role; the root, optional DX node, extent map, and sibling leaves are immutable
authenticated inputs rather than transaction homes.

The established linear-parent clean qualification removes inode 33, restores
both allocation counts, the parent link count, and `used_dirs`, preserves every
other parent-directory byte under an independently reconstructed exact splice,
and preserves the freed child-block bytes. Pinned e2fsprogs 1.47.4 `debugfs`
and read-only `e2fsck` accept the result, and the following ordinary remount is
byte-stable and write-free. The canonical checkpoint homes are target inode
W22, parent inode W23, parent directory W24, GDT W25, block bitmap W26,
superblock W27, and inode bitmap W28.

The indexed tranche adds one deliberately compositional runtime result: a
same-session singleton-depth-one `MKDIR` followed by `RMDIR` passed in 625.48
host seconds. It exercised the unchanged canonical 7 META + 1 REVOKE
removal, restored the namespace, free inode/block counts, parent link, and
`used_dirs` while leaving `bg_itable_unused` at its advanced high-water mark,
and left the HTree root, sole DX node, external extent-map node, and sibling
leaves byte-immutable; only the selected leaf was eligible for the RMDIR
splice. Structural contracts and the source-load/plan-core check also pass. No
new indexed crash,
refusal, external-tool, or stable-remount clone is claimed: the established
linear RMDIR lifetime/recovery evidence and indexed topology evidence compose
because the selected-leaf substitution adds no semantic role, write-order, or
recovery branch.

The established linear-parent W7 descriptor tear retains the directory and
both allocations with zero home replay. A committed W25 GDT tear leaves the
first three complete homes plus a torn descriptor; recovery replays all seven
after-images while honoring the child-block revoke, then reaches a write-free
stable remount. Zero-write refusal evidence covers a missing clock, missing
revoke capacity, six rather than seven metadata slots, `used_dirs == 0`, a
noncanonical empty child, an open reference, and hostile third-inode aliases of
either the parent or child directory home. The clean linear RMDIR journey
measures 1,016,967,358 steps and its stable remount 42,878,737 under the
unchanged 1.20-billion runtime watchdog.
...
root-owned, non-setgid directory-insertion envelope. It may be the qualified
one-block linear parent, an authenticated depth-zero HTree, or the singleton
...
dry, and live passes. The root, optional DX node, extent map, directory size and
sector count, and allocation state remain immutable. A full selected indexed
leaf returns `VFS-E-NOSPC`; LINK never splits or allocates. Every parent requires
a loaded cache, an absent destination name, root ownership, no setgid or xattrs,
and unique ownership of the selected mutable block. The target and parent inode
identities must differ; the parent need not be the current directory.

Dry and live staging each authenticate the target inode, parent inode, and
parent directory block before retaining after-images; for indexed LINK the
third block is the selected leaf. The HTree root, optional DX node, extent map,
and other leaves are authenticated immutable inputs, not transaction homes. The
transaction increments and restamps only the target inode, restamps the parent
inode, and inserts and restamps one file-type-1 directory entry. Target size,
mtime, atime, generation, map, `i_blocks`, xattrs, allocation state, and all
free-space and orphan accounts remain unchanged. The exact deduplicated homes
are acquired in target-inode-table, parent-inode-table, and parent-directory
order, giving `2/0/0` when both inode records share a table block and `3/0/0`
otherwise. On the canonical same-parent fixture, the shared target/parent inode-
table home is W16 and the directory home is W17. The distinct-root fixture
writes each of the three homes once in target, parent, directory order.
...
Five focused LINK tests close the original linear envelope. Clean same-parent
linking qualifies the shared vnode, open-FD continuity, exact cache counts,
pinned e2fsprogs 1.47.4 `debugfs` and read-only `e2fsck`, and a byte-stable
write-free ordinary remount. A W7 descriptor tear proves complete provisional
rollback and zero home replay. A committed W17 directory-home tear keeps the
public link and diagnostic, then replays both homes before a stable remount. A
distinct-root link qualifies exact three-home ordering, external inspection,
and stable remount. The fifth test proves zero-write refusal for a missing
clock, a one-metadata-credit profile, a nonregular target, and link-count
saturation.
...
The original same-parent linear envelope is one authenticated, root-owned mutable
...
The original cross-parent linear envelope admits the same regular-file source and two
...
The original linear-parent directory request is deliberately narrower in its
source shape than the regular-file envelope. It admits one source directory with exactly one dentry
...
#### Indexed-parent extension

The indexed-parent extension retains the no-victim operation kinds above while
allowing each old and new parent independently to be a one-block linear
directory, depth-zero HTree, or singleton depth-one HTree. Same-parent regular
files may remain in one selected leaf or move between two selected leaves.
Cross-parent regular files and the canonical empty-directory move may use any
mix of those admitted parent shapes. A source-leaf rebuild removes the exact
entry while emitting every other live entry at minimum aligned length, so the
source may be the first or only live entry. Destination selection scans the
hash route's continuation-eligible leaves in order and chooses the first whose
aggregate compacted live bytes plus the new record fit; a leaf with no live
names is valid. When source and destination are the same leaf and the new
encoded name fits the source record, the exact-record in-place path remains
available. Otherwise one combined rebuild removes the old record and appends
the renamed record; distinct source and destination leaves are rebuilt
independently.

Every indexed rebuild starts from sealed, checksummed authority and restamps
only the selected after-image or after-images. It performs no allocation,
split, growth, or HTree interval-key repair. The root, optional DX node,
complete extent map, unselected leaves, parent size, and parent sector count
remain immutable. The shared 8,992-byte plan seals independent old- and
new-parent shape, locator, route, root, node, inode, and selected-leaf authority
across cold, dry, and live planning. Filesystem hash parameters remain one
shared authenticated baseline, while each name's computed hash is bound to its
parent route. A same-parent same-leaf regular move retains the three roles for
source inode, parent inode, and selected leaf;
a same-parent cross-leaf move adds the destination-leaf role for four. A
cross-parent regular move uses the source inode, both parent inodes, source
leaf, and destination leaf for five roles. The canonical directory move adds
its child-directory role for six. Generic home deduplication still derives the
exact metadata credit and checkpoint order.

Ownership admission is topology-specific rather than weakened to a local
leaf check. A same-parent same-leaf move requires one complete-map hit for the
leaf and then its single-owner proof. A same-parent cross-leaf move requires
one hit for each leaf and then the paired owner proof. Across parents, the old
indexed map, when present, must report `(old leaf = 1, new leaf = 0)` and the
new indexed map, when present, must report
`(new leaf = 1, old leaf = 0)`. Thus two indexed parents form the exact 2x2
map proof; a mixed move applies the corresponding row to its indexed parent.
The filesystem-wide paired owner scan follows in every cross-parent shape.
For a directory move, the canonical child block must additionally have zero
hits in every applicable indexed parent map before its separate child-owner
proof; the existing `..` rewrite and parent-link transfer remain unchanged.

The deliberately lean positive gate uses one source-built session for exactly
two regular moves. It first renames
`/fixture/indexed/new.txt` to `mkdir-depth1` within a singleton depth-one
parent across leaves, with exact homes `[283, 281, 1357, 1497]` under
`4/0/0`. It then moves `/fixture/sparse.bin` from a linear parent to indexed
`/fixture/indexed/moved-000.txt`, with exact homes
`[279, 278, 281, 1345, 1497]` under `5/0/0`.
The selector passed as `1 passed in 774.00s`: its cold source build loaded the
ext4 closure in 1,174,688,163 of 1,600,000,000 steps across 4,057 packed lines
from 13 source units, and the composed indexed-RENAME journey used
2,869,457,042 of 3,600,000,000 steps. Existing linear RENAME recovery,
refusal, external-tool, and stable-remount evidence composes with
the already-qualified indexed authority and generic sealed-home replay; this
extension claims no cloned crash, refusal, external-tool, or stable-remount
matrix.

`VFS-CAP-RENAME-REPLACE` remains absent. Destination victims, replacement,
...
dispatches to the separate depth-zero release path below.
...
### Generalized depth-zero TRUNCATE-to-zero release and canonicalization

The public path accepts a linked positive-size regular file
whose inode flags are exactly `EXTENTS` and whose authenticated inline extent
root is depth zero with any format-valid count from zero through four. A
positive-size count-zero inode is a valid wholly sparse input. Nonempty roots
may begin at any valid logical block and contain arbitrary valid logical gaps;
each entry may be initialized or unwritten and retains its standard decoded
length (initialized 1 through 32,768 blocks, unwritten 1 through 32,767). The
ordered exact physical `{ first, count }` vector and aggregate block count are
authenticated rather than inferred from file size or logical continuity. The
inode may retain one qualified external xattr block, including its `i_blocks`
...
cleanup, and one-home clear requirements. Prospective cleanup credit is derived
from the captured topology. A count-zero target needs one metadata home to
clear its modern slot. A nonempty target starts with the distinct target-inode,
orphan-file, and primary-super homes, then adds one bitmap home for every
touched block group and each distinct primary GDT page needed by those groups.
The target cleanup is therefore exact `1/0/0` for count zero; a nonempty target
uses its computed metadata count with zero data and revoke credit. No singleton
fixture count is a public capacity. Thus the empty-union path has
three synchronously checkpointed transactions, while `ADD_MORE` drains the
complete enlarged union. First, an exact `3/0/0` `ADD_FIRST` transaction
atomically sets the target size and trusted-clock
...
superblock byte-exact. Both modes bind the complete pre-union counts, require
spare geometry-derived runtime-plan capacity, reject a duplicate target, and
rebuild their exact after-images from current media before the first home
write. The prospective JTA ownership certificate binds the filesystem context,
inode and generation, the complete ordered release vector, and the target
inode-table home as an additional one-block ownership range; the latter is not
release authority. One global other-inode walk proves that combined vector, so
count zero still performs a one-range owner proof for the inode home. Its xattr
identity is retained with zero refcount action.
The prospective checkpoint (CP) authority separately seals the exact target
entry count, ordered release vector, aggregate data-block count, and inline-
extents map discriminator. Its unused range tail and all EA-release fields are
zero. Their poststate is exactly the old union plus the new linked zero-size
inode while the old extent ranges and `i_blocks` remain recoverable.

A count-zero input is not required to arrive with zero bytes in its inactive
extent slots. `ADD_FIRST` or `ADD_MORE` canonicalizes all 48 inactive bytes
while publishing size zero, so its later cleanup can be the exact one-home
orphan-slot clear. Final refresh accepts only the canonical empty depth-zero
root. Nonempty inputs retain their complete map through `ADD`; cleanup performs
the canonical empty-root conversion only when it acquires release authority.
...
`i_blocks` to the retained xattr contribution, releases every certified
physical range through all touched block bitmaps, repairs the corresponding
group and global free-block counters, and clears the modern slot. For a
count-zero target, `ADD` already supplied the canonical inode after-image and
cleanup changes only the orphan-file home. The external xattr remains allocated
in both cases. Third, an exact `1/0/0` transaction clears transient
...
zero bytes through every hard-link name, advances free space by the exact sum
of released data-range lengths, and publishes `VN.BLOCKS` as zero or the
retained xattr block's sector contribution. The focused public journeys close
their descriptors while leaving the writer recovery-active, then require a
fresh authenticated mount to perform witnessed deactivation with no ext4 home
replay. The following stable mount performs no writes, and pinned e2fsprogs
1.47.4 read-only `e2fsck` accepts every generalized image. Historical
singleton qualification additionally included `debugfs` acceptance. An
ordinary clean unmount may instead perform the same existing deactivation.

The original singleton qualification fixture supplies the following recovery
evidence. A torn first-transaction descriptor returns the precommit volume
error and restores the old 54-byte public file, four-sector block count, inode
home, and allocated data block. A committed tear of the first inode checkpoint
home returns public truncate success with EOF zero and quarantines the live writer;
...
pinned `e2fsck` accepts the image. At the pre-generalization `109158b`
boundary, the ordinary one-record public journey with the exact counter audit
and stable remount completed in 1,195,226,705 steps under the unchanged
1.2-billion guard. The pre-generalization zero-release,
retained-shrink, policy, orphan-cleanup, and CREATE adjacency capstone passed
all eight selected tests sequentially in 573.98 host seconds; that capstone
contained the original singleton release case.

Final generalized-envelope qualification covers every resident entry count.
Direct seal/abort journeys prove the sparse two-entry `ADD_MORE` vector in
226,531,856 steps, the saturated four-entry `ADD_FIRST` vector in 231,319,776,
and a three-entry vector containing a two-block unwritten extent in
227,974,625, all under the unchanged 800-million guard. Public journeys prove
the canonical singleton at 1,116,644,505 steps, wholly sparse count-zero
canonicalization at 886,800,408, canonical sparse count two at 1,156,748,458,
and a saturated count-four root on compact primary-profile geometry built by
the pinned toolchain at 950,483,090, each under the unchanged 1.2-billion
guard. Every public case performs authenticated recovery/deactivation, a
zero-write stable remount, and
pinned `e2fsck`; the count-zero case also preserves its xattr block byte-exact.
The saturated case begins with the clean journal explicitly activated, then
executes the unchanged public live-stage/emit/checkpoint/drain path; singleton
and sparse cases retain first-activation coverage. Its compact image reduces
the global proof's inode-slot, group, and block-accounting scan geometry while
...
closed/open handling. Directory removal remains the separate RMDIR operation,
and indexed RENAME uses the independently sealed parent authority described in
the RENAME section rather than changing an UNLINK lifetime.
...
same-retained-block TRUNCATE, the original singleton zero-release TRUNCATE,
and TRUNCATE policy refusal: all eight pass in 534.05 host seconds.
...
sequential capstone runs the prior eight cases plus these three final-link
cases: all 11 pass in 949.80 host seconds.
...
5. retain completed same-retained-block shrink and generalized inline
...
8. retain atomic one-block linear-to-HTree conversion and indexed full-leaf
   ...
   evidence; canonical empty-directory RMDIR now admits those same parent
   depths with one same-session singleton-depth-one MKDIR-to-RMDIR success and
   compositional prior recovery evidence; bounded no-victim RENAME now admits
   independently linear, depth-zero, or singleton-depth-one old/new parents,
...
HTree parents—and bounded canonical empty-directory `RMDIR` from those same
admitted parent shapes plus
...
or orphan state. Depth-zero TRUNCATE-to-zero uses exact `3/0/0` `ADD_FIRST` or
`2/0/0` `ADD_MORE` orphan publication, topology-derived linked-orphan cleanup,
and `1/0/0` transient-bit retirement. It accepts a pure-`EXTENTS`, zero-through-
four-entry inline root with arbitrary valid gaps and initialized or unwritten
...
`SYNCFS` returns `BUSY` while the singleton is live. Bounded `RMDIR` accepts an
authenticated linear, depth-zero HTree, or singleton depth-one HTree parent and
only the canonical one-block empty directory produced by MKDIR. The indexed
form mutates only the selected leaf; root, optional DX node, map, and siblings
remain immutable. It
...
checksummed record in an admitted linear or indexed slack leaf, increment and
restamp the target inode, and restamp the parent inode without allocation,
ordered data, revoke, or orphan state. Same-parent same-leaf regular-file
RENAME likewise uses exact deduplicated `2/0/0` or `3/0/0` metadata credit. It
uses an in-place source-record rewrite
...
`[283, 275, 1299, 1377, 1364]`. These RENAME forms now admit each old/new
parent independently as linear, depth-zero HTree, or singleton depth-one
HTree. Same-parent same-leaf and cross-leaf regular moves use three and four
...
`debugfs`/`e2fsck` acceptance. The zero-TRUNCATE release/canonicalization slice adds
authenticated prospective modern-orphan publication for an exact zero-through-
four-entry inline depth-zero vector, topology-derived linked cleanup and data
release, count-zero canonicalization, and transient-bit retirement without
deactivating the writer. JTA retains the xattr identity with zero refcount
action; its checkpoint certificate binds the entry count, physical vector,
aggregate blocks, and map representation with all EA-release fields zero. The
original singleton fixture supplies clean/stable external-tool
acceptance, precommit descriptor rollback, and committed inode-home recovery
through the orphan path. Empty closed final-link UNLINK adds a clean same-mount
CREATE/remove cycle that restores
...
The established linear-parent RMDIR qualification adds a single-walk two-owner
proof, exact parent namespace and inode after-image oracles, inode/block release
with parent-link and `used_dirs` decrement, a revoke-only freed child block, six
zero-write refusal gates plus hostile aliases of each protected owner range, W7
rollback, and W25
...
block atomicity, partial or depth-positive block-releasing truncation, and other
...
- focused 1 KiB coverage exercises one- and two-inode legacy chains, a mixed
  ...
  four-home linked-truncate sealing for the original one-range fixture, post-seal mutation
