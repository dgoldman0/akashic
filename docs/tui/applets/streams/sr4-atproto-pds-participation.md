...
and the current `atproto/xrpc.f` contract rather than treating the removed
AT-local session/repository prototypes or rejected Streams L13 code as the
target architecture.
...
The current SR4 sequence includes:
...
- `a04a843` — the raw AT token adapter and durable P-256 composition
  ...
  secret cleanup;
- `bd58d1f` — the accepted AT grant installed into the existing generic
...
The isolated `sr4-bidirectional` continuation at the merged `a63af04`
baseline adds:

- `5cfe4b4` and `b15d8db` — caller-selected authenticated XRPC methods and
  exact JSON procedure bodies;
- `8342c6f`, `a7a3880`, and `1691252` — caller-owned exact AT codecs and the
  authenticated createRecord owner with detached result storage;
- `b45b166`, `e37afbe`, and `d29fe58` — checked state-free UTC conversion,
  caller-owned monotonic TID clocks, and exact Bluesky text records;
- `3576725` and `89f6ee6` — measurable atomic createRecord body publication
  and admitted composition geometry;
...
`3ad9b0a` keeps the reusable continuation provider-neutral under
`security/oauth2/`. `O2TREQ` owns one address-free authorization-code token
attempt, captures the consumed authorization-code provenance and exact token
endpoint, permits one explicit nonce retry, guards build/provenance loans, and
terminally wipes its secrets. The separate issuer-bound nonce owner validates
...
`bd58d1f` adds no new production owner. The token-success continuation passes
its ephemeral accepted grant directly to the existing provider-neutral
...
That checkpoint's qualification boundary was intentionally narrow. Its
deterministic vault seam persisted one record, so the fixture released the
completed DPoP-key vault before installing the session record. It proved
initial session durability and same-process reconstruction against retained
...
That was the first complete landing-4 authenticated read gate rather than a
live connector. Its focused witness uses deterministic P-256 and lower KDOS
transport seams while exercising the production exchange, HBUF, durability,
response provenance, and semantic admission code.

`51ead3a` advances that operation through an actual authenticated ingress
slice. The state-free `atproto/get-author-feed.f` builder owns only general AT
endpoint and query policy. The per-instance
`streams/atproto-author-feed-connector.f` owner binds the durable session,
builds the exact PDS-proxied request, delegates cooperative exchange, decodes
the bounded feed model, seals the source event, and admits the validated raw
page through the existing compact Streams flow. Its focused serial witness
covers an exact `400 use_dpop_nonce` response and sole retry, nonce rollover
...
Before the isolated bidirectional branch, the first visible multiprotocol
slice was accelerated without waiting for all six landings. Commit `d503a84`
changes the focused Desk profile from the offline
...
Evidence recorded for that historical checkpoint is deliberately narrow:
...
`bd58d1f` carries that accepted grant directly into the generic durable
session owner and proves an ordinary reopen through distinct session/vault
objects. `50e3b24` adds the provider-neutral bounded Authorization-field
builder required by DPoP without placing AT token policy in `akashic/net/`.

`ef53918` supplies the general cached/complete-READDIR lookup semantics needed
...
`51ead3a` keeps the same placement boundary. General author-feed target and
query construction lives under `akashic/atproto/` and depends on neutral
URI/form/HTTP-target libraries. The only new applet-specific module is the
per-instance owner under `akashic/tui/applets/streams/`; it composes the
general AT exchange and bounded feed model with the existing Streams
...
The authenticated presentation files are committed. A historical main-tree
checkpoint also listed seven unrelated dirty runtime/source-owner paths; that
list does not describe this isolated branch and is intentionally omitted here.
Do not stage, restore, rewrite, or delete unrelated changes if work later
...
Current exact-single-full-core, zero-cluster qualification automatically uses
MegaPad `79dedba`'s timing-correct native scheduler fast path. `MachineSession`
still constructs `MegapadSystem`, and Python orchestrates native batches plus
exceptional/MMIO continuations rather than interpreting the normal CPU path
instruction by instruction. This is independent of userland compilation:
source-mode evidence still loads and compiles the Forth sources and does not
claim a precompiled module cache.
...
- The authenticated-read checkpoint passed its static gate and all 55 serial
  phases on one core with 128 MiB of external machine memory and the unchanged
  180,000,000-step phase ceiling. It compiles the production RAM VFS,
  credential vault, P-256 key owner, OAuth session owner, authenticated XRPC
  builder, HTTP request/stream/buffered stack, and their real dependency
  bodies. Only the already-qualified P-256/JWK/DPoP mathematical constructors
  and the remote peer are deterministic seams. Runtime setup, key provision,
  session install, distinct-owner reopen, request build, buffered read, and
  final cleanup used 15,912,128, 24,845,869, 28,687,952, 73,000,000,
  34,780,832, 425,729, and 18,500,000 guest steps respectively. The exact
  sealed request and synthetic `200 {}` response passed with clean stacks and
  complete final owner cleanup. Separately, the focused VFS READDIR-fallback
  lookup contract passed in 117,914,562 guest steps. This evidence does not
  claim live DNS, sockets, TLS, a PDS response, semantic session-DID
  validation, or visible Streams admission.
- The authenticated feed-to-flow checkpoint at `51ead3a` passed its static
  gate and complete sequential vertical on one core with 128 MiB of external
  machine memory and unchanged checked-in phase ceilings. The production
  target builder, authenticated exchange, bounded feed decoder, per-instance
  connector, source event, compact flow, transform, and output sink all
  execute in the witness. The deterministic peer returns one strict
  `400 use_dpop_nonce` challenge and then one exact `200` feed page; the
  second proof receives the first nonce, the request uses the exact
  `did:web:api.bsky.app#bsky_appview` proxy, two items and the cursor decode,
...
`private_key_jwt` path, broad login-hint and malformed-response matrices, the
full media/header/capacity/protected-alias/canary cross-product, durable
restart, and live TLS/browser integration. Direct durable-key proof
composition is no longer deferred: the current public-client wrapper supplies
it while leaving the lower-level raw BUILD usable for other admitted AT
client-authentication profiles. Token-endpoint nonce retry is now covered by
the separate focused token checkpoint rather than the PAR adapter.
...
Authorization-server nonce ownership and retry orchestration are supplied by
the separate DPoP-aware token continuation rather than hidden state in this
state-free wrapper.
...
duplicating it here. Live browser and redirect-route integration plus durable
...
covers initial install plus same-process distinct-owner reopen. The current
authenticated-read checkpoint adds simultaneous key/session persistence and
request construction; refresh/logout and live transport remain.
...
optional `getSession` account-usability fields, cancellation, and live
DNS/socket/TLS/PDS execution. The strict first-response
`400 use_dpop_nonce` challenge and sole retry are now witnessed by the
feed-to-flow checkpoint; the accepted `401` branch and broad
status/challenge permutations remain deferred.
...
`VFS-CACHE-DROP` invariant also remains: dropping a child from a READDIR-only
parent can leave the parent's complete-children flag set, so a later lookup
can return stale `NOENT` without re-enumeration. This is recorded in the VFS
documentation and must be corrected and broadly qualified before the VFS
cache boundary is declared closed; it does not gate the current append-only
credential-vault vertical.
