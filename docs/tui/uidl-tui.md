...
| **Shared label value** | CELL label paint calls `UIDL-TEXT@`, the same neutral value resolver used by UIDL semantic snapshots. Binding precedence and string/integer/boolean coercion therefore do not vary by output path. Snapshot-only capacity declarations never truncate or reject CELL output. |
