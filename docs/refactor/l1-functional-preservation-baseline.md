...
draft, observation and manual-refresh behavior retains its existing evidence
references only while it remains live or protects identified user-created
data.

The former four L13 characterization prerequisites are replaced by one
conditional SR6 prerelease-replacement prerequisite. It applies only when a
replacement milestone touches or displaces a listed current branch. The
required evidence is the smallest focused check needed for behavior that
remains live, or an explicit deletion or user-data export decision. Fixtures
